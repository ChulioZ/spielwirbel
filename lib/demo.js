...
async function seedRound(scoped, spec, userId, locale) {
  ...
  // The colour marker (#1187), written explicitly: createRound already stamped
  // one from the round's id, which would give the three demo rounds whatever
  // colours their random ids hash to — possibly one colour for all three.
  await scoped.setMarker(round.id, spec.marker);
