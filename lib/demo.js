...
async function seedSessions(scoped, round, gameIds) {
  ...
  for (const spec of seed.DEMO_SESSIONS) {
    ...
    /* votes[personId][gameId] = { rating, retire } — the exact shape
       lib/routes/sessions.js persists. A seed rating of 0 is the retirement
       proposal, i.e. the zero of the scale (#797), and is stored the way the
       vote card writes it: the flag, and no rating beside it.

       The seed uses exactly ONE of them, deliberately. Without any, a demo
       account can never show the results screen's zero bar and the feature is
       invisible in the surface built to show the app off; with enough of them
       the round would arrive already nagging the visitor to archive a game,
       which is what the old "retire is always false here" note was protecting
       against. One vote in four
       is 25% — below `retireRecommendations`' SORT_SHARE of 0.5 — and leaves
       that game's raw average at 3.0. Since #893 the rating branch compares the
       SPIELWIRBEL-SCORE against LOW_SCORE rather than that average, and
       {0,4,5,3} scores 1.5, which clears the 1.0 threshold. So no
       recommendation fires — but the margin is now half what the raw mean
       suggested, because the curve weighs the zero at −6. Re-check both numbers
       if you add a second zero; the assertion that actually holds this is
    ...
    memberIds.forEach((mid, seat) => {
      ...
      picked.forEach((gid, gi) => {
        const r = row[gi] != null ? row[gi] : 3;
        votes[mid][gid] = r === 0 ? { rating: null, retire: true } : { rating: r, retire: false };
      });
    });
