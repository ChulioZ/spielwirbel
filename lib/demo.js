 ...
 *    (#215's shared store was closed unshipped), so anything based on
 *    in-memory state would classify the same tenant differently depending on
 *    which replica answered. A real tenant id is 16 hex
...
// The provider-metadata field set (#717), so the seed copies exactly the keys a
// real add would and cannot grow one the repo backends do not know about.
const { assignProviderInfo } = require('../public/js/provider-info-fields');
// The session activity log (#209) — a seeded session carries the same entries a
// played one does, or the demo's Chronik shows an evening nobody started.
const { sessionEvent } = require('./session-events');
// The multi-table split (#796). Both halves of the split route's own logic, so a
// seeded split is by construction one the app would have produced.
const { computeTableProposals, buildChildSessions } = require('./session-split');
const { sessionPeople, MIN_TEAM_SIZE } = require('../public/js/session-people');
...
// The per-user design registry (#1184/#1186) — one list, no copy.
...
// error state.
// Is this USER ROW a guest demo? Split out of the middleware below in #1089, so
// a route that must refuse a demo for only SOME requests (GET
// /api/account/profile/:username, which lets a demo see its own profile) asks the
// same question rather than spelling `user.demo === true` a second time.
//
// It takes the ROW, not an id, on purpose: such a route needs the caller's record
// for its own reasons anyway, and an id-taking predicate would make it read the
// same row twice.
const isDemoUser = (user) => !!user && user.demo === true;

async function refuseDemoAccount(req, res, next) {
  try {
    const user = req.userId ? await repo.getUserById(req.userId) : null;
    if (isDemoUser(user)) return res.status(403).json({ error: 'demo_forbidden' });
    return next();
  } catch (err) {
    return next(err);
  }
...
// Build the three rounds a demo lands in (#427, widened by #953). Everything
// goes through the ordinary tenant-scoped repo mutators (never raw SQL), so the
// demo exercises exactly the code paths a real round does and cannot drift from
// them — which also means a bug that would break a real round breaks the demo
// first, where we see it.
//
// Returns the FIRST round: it is the one the visitor lands in, and the only one
// any caller has ever needed a handle on.
async function seedTenant(tenantId, userId, locale) {
  const scoped = repo.forTenant(tenantId);
  let first = null;
  // Sequential on purpose, like every other multi-write loop here: createRound
  // is one transaction per row in the Postgres backend, and firing three at once
  // on one pooled connection is the shape .claude/rules/postgres-backend.md
  // warns about.
  for (const spec of seed.DEMO_ROUNDS) {
    const round = await seedRound(scoped, spec, userId, locale);
    if (!first) first = round;
  }
  return first;
}

// One seeded round: members, design, tags, shelf, then whatever sessions it
// carries.
async function seedRound(scoped, spec, userId, locale) {
  const text = seed.roundTextFor(spec.key, locale);
  ...
  const round = await scoped.createRound({
    ...
    owner: { name: seed.textFor(locale).ownerSeat, userId },
  });
  if (!round) return null;

  // The world / palette (#903–#905). Stored through the ordinary mutator in the
  // shape lib/routes/marker.js persists, so the home tile's backdrop, emblem
  // and display face resolve exactly as they do for a round somebody styled by
  // hand. Kept until the flip (#1202) retires the worlds.
  await scoped.setBackground(round.id, spec.design);
  // …and the colour marker that replaced it (#1187), derived from that same
  // design rather than hand-picked, so the two can never disagree. It has to be
  // written explicitly: createRound already stamped a marker from the round's
  // id, and a stored marker beats the legacy lookup — so without this the
  // palette round would come out some arbitrary colour while claiming to be
  // Salbei. Any round that gets a legacy design AFTER creation needs the same
  // pairing; the demo is the only thing in the app that still does.
  await scoped.setMarker(round.id, LEGACY_MARKER_INDEX[spec.design.id]);

  // Tags first: a game references them by id, so they have to exist before the
  // games that carry them. Only the keys THIS round's shelf uses, so a round
  // never grows a tag chip that filters nothing.
  const tagIdByKey = {};
  for (const key of seed.tagKeysFor(spec.games)) {
  ...
  for (const game of spec.games) {
    gameIds.push(await seedGame(scoped, round.id, game, tagIdByKey, ownerSeatId, memberIds));
  }

  await seedSessions(scoped, round, gameIds, spec.sessions);
  if (spec.split) await seedSplitSession(scoped, round, gameIds, spec.split, ownerSeatId, locale);
  return round;
}

// One seeded game, including the two off-shelf states.
//
// `providerInfoAt` is stamped at MINT TIME and only when the row actually
// carries resolved metadata. Both halves matter and they fail in opposite
// directions: a fixed historical stamp would be past the TTL, so the lazy
// backfill would hop BGG once per demo per game anyway (the cost #953 set out to
// remove), while stamping a row that has NO metadata would suppress the backfill
// for seven days — and since a demo is purged within one, such a game could
// never fill at all. Stamping only what we actually resolved is the same rule
// backfillProviderInfo applies to its own answers (lib/provider-info.js).
async function seedGame(scoped, rid, spec, tagIdByKey, ownerSeatId, memberIds) {
  const info = assignProviderInfo({}, spec);
  /* `spec.owners` are SEAT INDEXES (lib/demo-seed.js), resolved here because the
     ids only exist once the round has been created. An index past the seat list
     is dropped rather than stored: an ownerId nobody holds would keep the game
     out of every draw (`ownedByParty`) with nothing on screen to explain it. */
  const ownerIds = (spec.owners || []).map((seat) => memberIds[seat]).filter(Boolean);
  const game = await scoped.createGame(
    rid,
    {
      ...
      ...info,
      ...(Object.keys(info).length ? { providerInfoAt: iso(Date.now()) } : {}),
      // A wish is created straight onto the Wunschliste and writes no activity —
      // wanting a game is not an event (lib/repo/json.js's createGame).
      ...(spec.wish ? { wish: true } : { ownerIds }),
    },
    ownerSeatId
  );
  if (!game) return null;
  // Retiring is a separate step because it IS an event: the round archived a
  // game it used to own, and the Chronik should say so. createGame has no
  // `retired` field for exactly that reason.
  if (spec.retired) await scoped.retireGame(rid, game.id, true, ownerSeatId);
  return game.id;
}
...
async function seedSessions(scoped, round, gameIds, specs) {
  const memberIds = round.members.map((m) => m.id);
  for (const spec of specs || []) {
    ...
    /* votes[personId][gameId] = { rating } — the exact shape
       lib/routes/sessions.js persists.

       The seed carries exactly ONE „gar nicht" (a 1), deliberately. Without
       any, a demo account can never show the results screen's leftmost bar or
       the score's „1× gar nicht" reason line, so both are invisible in the
       surface built to show the app off; with enough of them the round would
       arrive already nagging the visitor to archive a game. One 1 among
       {1,4,5,3} scores 1.75 — above `retireRecommendations`' LOW_SCORE of 1.0,
       and a lone dissenter besides, so nothing is proposed. Re-check that if
       you add a second one; the assertion that actually holds it is
       test/retire-score-threshold.test.js, which reads DEMO_SESSIONS directly
    ...
    memberIds.forEach((mid, seat) => {
      ...
      picked.forEach((gid, gi) => {
        votes[mid][gid] = { rating: row[gi] != null ? row[gi] : 3 };
      });
    });
    ...
    await scoped.finishSession(round.id, session.id, {
      ...
      // How it ended when nobody won (#1038). Absent for every session that has
      // winners — the two are mutually exclusive, and passing both would store
      // a state the route refuses.
      ending: spec.ending,
    });
  }
}

/* One evening that was SPLIT across two tables (#796), seeded through the real
   machinery rather than hand-written.

   `computeTableProposals` picks the arrangement and `buildChildSessions` builds
   the children — the same two functions POST /:sid/split calls, in the same
   order (children first, the parent claims them afterwards, because there is no
   id to link before they exist). That is the whole reason this is not a table of
   literals: a hand-written split would be a snapshot of what the objective
   happened to return on the day, and it would silently stop matching the
   recommendation the builder screen re-scores live the first time
   public/js/table-split.js is retuned.

   It also means an infeasible seed degrades to a plain voted session rather than
   to a broken one: no proposal, no children, and the parent stays a session
   whose voting is closed. */
async function seedSplitSession(scoped, round, gameIds, spec, ownerSeatId, locale) {
  const picked = spec.gameIndexes.map((i) => gameIds[i]).filter(Boolean);
  if (picked.length < 2) return;

  const memberIds = round.members.map((m) => m.id);
  // Guest and team ids are minted exactly as resolveGuests/resolveTeams do in
  // lib/routes/sessions.js: a guest id is a key in this session's vote map and
  // in `winnerIds`, so it has to be unique per session rather than derived from
  // anything the seed could repeat.
  const guests = seed.textFor(locale).guests.map((name) => ({
    id: crypto.randomBytes(8).toString('hex'),
    name,
  }));
  const teamPersonIds = spec.team.map((seat) => memberIds[seat]).filter(Boolean);
  const teams = teamPersonIds.length >= MIN_TEAM_SIZE
    ? [{ id: crypto.randomBytes(8).toString('hex'), personIds: teamPersonIds }]
    : [];

  const parent = await scoped.createSession(round.id, {
    events: [sessionEvent('started', ownerSeatId)],
    createdAt: iso(Date.now() - spec.daysAgo * 24 * 60 * 60 * 1000),
    tagIds: null,
    excludeTagIds: null,
    requestedCount: picked.length,
    memberIds,
    // Absent-key parity: the two sessions above grow neither key, this one grows
    // both, and both backends must agree on that (.claude/rules/postgres-backend.md).
  ...
  /* votes[personId][gameId] — over members ∪ guests in sessionPeople order,
     which is the order lib/demo-seed.js's `ratings` rows are written in.

     `guests` is passed explicitly rather than read back off `parent`: the row
     order of that table is defined against the array built above, so resolving
     it through whatever the backend echoed would make a seat's ratings depend on
     a round trip. The two agree today; this way they cannot stop agreeing. */
  const people = sessionPeople(round, { ...parent, guests });
  const votes = {};
  people.forEach((person, seat) => {
    const row = spec.ratings[seat] || [];
    votes[person.id] = {};
    picked.forEach((gid, gi) => {
      votes[person.id][gid] = { rating: row[gi] != null ? row[gi] : 3 };
    });
  });
  await scoped.saveSessionResults(round.id, parent.id, votes);
  await scoped.closeSessionVoting(round.id, parent.id, sessionEvent('voting_closed', ownerSeatId));

  // Re-read: the split machinery takes a stored round and a stored session, and
  // the votes it scores were written by the two calls above.
  const full = await scoped.getRound(round.id);
  const stored = full && (full.sessions || []).find((s) => s.id === parent.id);
  if (!stored) return;
  const proposals = computeTableProposals(full, stored);
  if (!proposals.length) return;
  await scoped.setSessionTableProposals(round.id, parent.id, proposals);

  // The smallest feasible table count, which is the one the builder highlights
  // by default (public/js/table-split.js).
  const started = sessionEvent('started', ownerSeatId);
  const childIds = [];
  for (const blob of buildChildSessions(full, stored, proposals[0].tables, started)) {
    // buildChildSessions stamps `now`, which is right in the route (a split is
    // confirmed the same evening it was drawn) and wrong here, because the
    // parent is BACKDATED. Left alone, the Chronik files the tables under the
    // current month while the evening they belong to sits in the previous one —
    // so the period recap counts plays in a month whose session list does not
    // show them. Inherit the parent's date instead.
    blob.createdAt = stored.createdAt;
    blob.chosenAt = stored.createdAt;
    const child = await scoped.createSession(round.id, blob);
    if (!child) return;
    // Winners are derived from the child rather than declared in the seed: which
    // party sits where is the objective's call, so the only ids guaranteed to be
    // at this table are the ones in the blob it just produced. A table that
    // seated a team lets the team win together, which is the case worth showing.
    const winnerIds = (blob.teams && blob.teams.length)
      ? blob.teams[0].personIds
      : [blob.memberIds[0] || (blob.guests && blob.guests[0] && blob.guests[0].id)].filter(Boolean);
    await scoped.finishSession(round.id, child.id, { finished: true, winnerIds },
      sessionEvent('finished', ownerSeatId));
    childIds.push(child.id);
  }

  await scoped.splitSession(round.id, parent.id, childIds,
    sessionEvent('split', ownerSeatId, { count: childIds.length }));
}
...
async function createDemoAccount(locale, ipHash) {
  ...
  const user = await repo.createUser({
    ...
    disabledReason: null,
    bggUsername: null,
    // #841. A demo may not SET a picture (the upload route refuses one), but the
    // key is written anyway for the absent-key parity below — and so the demo
    // purge's object sweep reads a field that is always there.
    avatar: null,
    ...
    // The per-user design (#1186), written here for the same absent-key parity
    // reason as the two above. The face, and the chooser UNSEEN: a demo is a
    // visitor trying the app, so it is exactly the session the chooser is worth
    // showing — and PATCH /me lets a demo change its design, unlike the avatar
    // and the password (lib/routes/account.js says why).
    design: FACE_DESIGN,
    designChooserSeen: null,
    // The switch-back stamp (#1201), false at birth like at registration. A demo
    // may still set it, and the operator's tile never counts one.
