...
async function seedSessions(scoped, round, gameIds) {
  ...
  for (const spec of seed.DEMO_SESSIONS) {
    ...
    /* votes[personId][gameId] = { rating, retire } — the exact shape
       ...
       against. One vote in four is 25% — below `retireRecommendations`'
       SORT_SHARE of 0.5 — and leaves that game's average at 3.0, well clear of
       LOW_AVG, so no recommendation fires. Re-check those two numbers if you
       add a second zero. */
    const votes = {};
