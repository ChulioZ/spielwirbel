...
async function instanceStatus() {
  ...
  return {
    ...
    // BOTH mail delivery works and IMPRESSUM_ADDRESS is set (see /api/config in
    // lib/app.js); this reports the second half. Presence only, never the value:
    // the address is destined for the public Impressum eventually, but before
    // launch it must not leak early through a panel screenshot (rule 1 above).
    legal: {
