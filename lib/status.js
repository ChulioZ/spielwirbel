 ...
 * ACCOUNTS_ENABLED set without a real SESSION_SECRET, a missing MJ_APIKEY_PRIVATE
...
// Whether two secrets differ, WITHOUT revealing either — only the verdict
// escapes. Deliberately the same shape as lib/admin.js's and lib/auth.js's
// safeEqual (length check, then timingSafeEqual on the raw bytes), so there is
// one "compare two secrets" idiom in the codebase rather than a second variant.
//
// Note what this is NOT: no digest is stored, transmitted or returned, so this
// is an equality test, not password hashing — hashing the operands first (to
// avoid the length short-circuit) would look like a weak password hash to a
// reader and to a scanner, for a side-channel that does not exist here. The only
// thing an unequal-length short-circuit could reveal is whether two secrets share
// a length, to a caller who is already an authenticated operator and who receives
// the equality verdict itself in the response.
//
// An unset `b` counts as distinct: there is nothing to collide with.
...
async function instanceStatus() {
  ...
  return {
    ...
    // Without a Mailjet key lib/mail.js silently routes to the in-memory
    // outbox — verification and reset mails are simply never delivered. BOTH
    // halves of the pair are checked: Mailjet authenticates over HTTP Basic as
    // public:private, so one key alone is a working-looking configuration that
    // delivers nothing — exactly the quiet misconfiguration this card exists
    // to surface.
    mail: {
      configured: isSet('MJ_APIKEY_PRIVATE') && isSet('MJ_APIKEY_PUBLIC'),
      fromSet: isSet('MAIL_FROM'),
      baseUrlSet: isSet('APP_BASE_URL'),
    },

    // The public site footer (Kontakt + the #134 legal pages) renders only when
    // BOTH mail delivery works and the Impressum identity is set (see
    // /api/config in lib/app.js); this reports the identity half. Presence
    // only, never the values: address and e-mail are destined for the public
    // Impressum eventually, but before launch they must not leak early through
    // a panel screenshot (rule 1 above).
    legal: {
      impressumAddressSet: isSet('IMPRESSUM_ADDRESS'),
      impressumEmailSet: isSet('IMPRESSUM_EMAIL'),
    },
    ...
    // Without the BGG application token the board-game lookup answers every
    // search with an empty list — deliberately silent, so the other four
    // providers keep working (.claude/rules/add-game-lookup-provider.md). That
    // silence is exactly why the operator needs to see it here.
    lookup: { bggTokenSet: bgg.tokenSet() },
