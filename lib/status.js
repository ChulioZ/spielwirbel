...
async function instanceStatus() {
  ...
  const mailBudget = mail.budgetState();

  return {
    // Which Node the answering process is actually running (#977). The Dockerfile
    // pins an exact patch, but the pin only describes what the NEXT build will
    // use — nothing else in the app reports what is running now, so an operator
    // could not tell a security release had landed without a Railway console.
    // Read per call like everything else here, and a version string is not a
    // secret: it is the same value `node --version` prints in the container.
    ...
    metrics: {
      ...
      // The #448 daily send budget, as sent/limit like the demo row. Unlike
      // every other metric this is PROCESS state, not database state: each
      // replica books its own counter, so the card shows whichever process
      // answered the request — the panel's row note says so. budgetState()'s
      // `day` string is deliberately not forwarded: it is always today (UTC)
      // by construction, and the metrics payload stays numbers-only.
      mail: { sent: mailBudget.sent, limit: mailBudget.limit },
    },
  };
}
