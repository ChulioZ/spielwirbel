 ...
 * ACCOUNTS_ENABLED set without a real SESSION_SECRET, a missing SMTP_PASS
...
async function instanceStatus() {
  ...
  return {
    ...
    // Without SMTP credentials lib/mail.js silently routes to the in-memory
    // outbox — verification and reset mails are simply never delivered. All
    // three parts are checked: host, user and password are one setting, and a
    // partial set is a working-looking configuration that delivers nothing —
    // exactly the quiet misconfiguration this card exists to surface.
    mail: {
      configured: isSet('SMTP_HOST') && isSet('SMTP_USER') && isSet('SMTP_PASS'),
