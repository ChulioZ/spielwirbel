 ...
 * ACCOUNTS_ENABLED set without a real SESSION_SECRET, a missing SCW_SECRET_KEY
...
async function instanceStatus() {
  ...
  return {
    ...
    // Without SCW_SECRET_KEY lib/mail.js silently routes to the in-memory
    // outbox — verification and reset mails are simply never delivered. The
    // project id is checked too: Scaleway rejects a send without it, so a key
    // on its own is a working-looking configuration that delivers nothing —
    // exactly the quiet misconfiguration this card exists to surface.
    mail: {
      configured: isSet('SCW_SECRET_KEY') && isSet('SCW_PROJECT_ID'),
