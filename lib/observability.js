...
//
// Privacy caveat: unlike the status card, a screenshot of this is NOT guaranteed
...
// carried by the id itself, so it survives the multi-process deployment (#215)
// where anything held in memory would classify the same tenant differently
// depending on which replica answered. lib/demo.js owns the prefix; it is
// duplicated here as a literal ONLY to avoid requiring lib/demo.js (which pulls
// in the repo) from the logging module that everything else requires — the
// require cycle is the reason, and test/demo.test.js pins the two together.
...
function trackEvent(name, { tenantId } = {}) {
  ...
  if (String(tenantId || '').startsWith(DEMO_TENANT_PREFIX)) return;
