...
// shared store was closed unshipped)
// where anything held in memory would classify the same tenant differently
// depending on which replica answered. The predicate lives in lib/demo-tenant.js
// — a dependency-free leaf module — precisely so this file can have it without
// requiring lib/demo.js, which pulls in the repo and would be a cycle from the
// logging module that everything else requires.
...
function errorHandler(err, req, res, next) {
  ...
  // reqPath(), not req.path: this one is forwarded to ERROR_WEBHOOK_URL, i.e. to
  // a third party, so it is the LAST place a vote-link token may appear (#652).
  captureError(err, { method: req.method, path: reqPath(req) });
