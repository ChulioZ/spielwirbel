...
async function backfillProviderInfo(repo, rid, games, { maxBatches = 1, pauseMs = 0 } = {}) {
  ...
  const result = { batches: 0, asked: 0, filled: 0, failed: false };
  for (const [pid, list] of byProvider) {
    ...
    const size = Number.isInteger(provider.MAX_THING_IDS) ? provider.MAX_THING_IDS : list.length;
    let consecutiveFailures = 0;
    for (let i = 0; i < list.length && result.batches < maxBatches; i += size) {
