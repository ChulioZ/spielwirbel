...
/*
 * Instance-wide public statistics (#564) — the scale counters and the six
 ...
 * a fact about the game over its whole life, and reading `plays.year` here would
...
const METRICS = [
   ...
   *
   * BOTH FLOORS DEFAULT TO 0, which is an operator decision (#1035) and not an
   * omission — it publishes whatever it says, because the one durable fact
   * about the instance's shelf is worth stating from the first play. The env
   * vars are kept rather than dropping the `qualifies` clause entirely: 0 is a
   * documented, meaningful `threshold()` value ("publish this metric whatever
   * it says"), and keeping the lever is what lets the card be pulled back live
   * under time pressure without a deploy, exactly like the other five.
   *
   * `plays.all` has been in both backends since #928, where it feeds the
   * Spielwirbel-Score's play lift — so this publishes a number that was already
   * measured and carried, at no cost to either repo.
   */
  {
    key: 'playedAll',
    qualifies: (r) => r.plays.all.count >= threshold('PUBLIC_STATS_MIN_PLAYS_ALL', 0)
      && r.plays.all.tenants >= threshold('PUBLIC_STATS_MIN_PLAY_TENANTS_ALL', 0),
  ...
  },
  {
    ...
    /* AT LEAST ONE RATING, spelled explicitly rather than inferred from a null
       ...
       one that must not depend on an operator's number. */
 ...
 *
 * THE BUDGET IS NOW TIGHT: six metrics x CANDIDATES_PER_METRIC (3) is 18 of
 * PUBLIC_STATS_RESOLVE_MAX's default 20, where five was 15. That still fits,
 ...
 * required to change. A SEVENTH metric does not fit; raise the default with it.
 */
async function podiumsFrom(rows, period) {
...
async function rebuild(now = new Date().toISOString()) {
  ...
  try {
    ...
    const games = await podiumsFrom(rows, periodBoundaries(now));
    payload = {
      generatedAt: now,
      ...(counters ? { counters } : {}),
      ...(games ? { games } : {}),
