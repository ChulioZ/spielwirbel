...
// The Spielwirbel-Score's curve (#893), applied here rather than in the repo's
// SQL — see `scoreTally`'s header for why the aggregate reports a histogram.
const { scoreTally, SCORE_MIN } = require('../public/js/vote-score');
 ...
 */
/*
 * One row's Spielwirbel-Score, or null when it carries no scoreable vote.
 *
...
const METRICS = [
  ...
  })),
  {
    ...
    /* Ranked on the SPIELWIRBEL-SCORE (#893), not the raw mean (#914) — which is
       why the vote-count floor above matters: without it a single 5-star vote
       would top a game with fifty. Unclamped, like every other ranking in the
       app: two games at the display floor are still genuinely different
       disasters and must not tie (core.js's `displayScore`). */
