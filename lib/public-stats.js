...
// Whether the block exists at all. An instance that has not opted in serves a
// 404 and makes no provider call — the DEMO_ENABLED / PRICES_ENABLED shape.
function publicStatsEnabled() {
  return process.env.PUBLIC_STATS_ENABLED === 'true';
}
...
const COUNTERS = [
  { key: 'accounts', env: 'PUBLIC_STATS_MIN_ACCOUNTS', min: 50, of: (m) => m.accounts.total },
  { key: 'rounds', env: 'PUBLIC_STATS_MIN_ROUNDS', min: 50, of: (m) => m.rounds.total },
  { key: 'games', env: 'PUBLIC_STATS_MIN_GAMES', min: 500, of: (m) => m.content.games },
  { key: 'sessions', env: 'PUBLIC_STATS_MIN_SESSIONS', min: 200, of: (m) => m.content.sessionsFinished },
];
...
const METRICS = [
  {
    key: 'mostOwned',
    qualifies: (r) => r.owners >= threshold('PUBLIC_STATS_MIN_OWNERS', 3),
    rank: (r) => r.owners,
    value: (r) => ({ owners: r.owners }),
  },
  ...['d7', 'd30', 'd365'].map((window, i) => ({
    key: ['playedWeek', 'playedMonth', 'playedYear'][i],
    qualifies: (r) => r.plays[window].count >= threshold('PUBLIC_STATS_MIN_PLAYS', 3)
      && r.plays[window].tenants >= threshold('PUBLIC_STATS_MIN_PLAY_TENANTS', 2),
    rank: (r) => r.plays[window].count,
    value: (r) => ({ plays: r.plays[window].count }),
  })),
  {
    key: 'bestRated',
    qualifies: (r) => r.ratings.count >= threshold('PUBLIC_STATS_MIN_RATINGS', 5)
      && r.ratings.tenants >= threshold('PUBLIC_STATS_MIN_RATING_TENANTS', 2),
    // Ranked on the average, which is why the vote-count floor above matters:
    // without it a single 5-star vote would top a game with fifty.
    rank: (r) => r.ratings.sum / r.ratings.count,
    value: (r) => ({
      // Rounded here rather than in the client so every surface shows the same
      // number and no float tail reaches the payload.
      average: Math.round((r.ratings.sum / r.ratings.count) * 10) / 10,
