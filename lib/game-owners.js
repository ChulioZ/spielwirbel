...
async function rememberOwnerPreset(repo, round, rid, userId, ownerIds) {
  ...
  try {
    ...
    /* the add succeeded; the preference is not worth failing it for — but a
       write that keeps failing is still a fault the operator should see (#1148) */
    logger.warn({ event: 'owner_preset_write_failed', message: err.message });
  }
}
