...
// The share of the day's ceiling that only `critical` mail may spend (#618).
//
// Notification mail (the inbox e-mails for round invitations and friend
// requests) joined the same budget, and it is strictly lower-value than the mail
// that makes the app usable at all: a missed notification costs one person one
// delayed reply, while a missed verification mail costs EVERY new user their
// signup. Sharing one undifferentiated bucket would let a busy day of
// notifications spend the registration path's headroom — i.e. buy marginal
// re-engagement with the app's front door.
//
// So the last quarter of the day is critical-only. No env var: MAIL_DAILY_MAX is
// already the tuning dial and the reserve derives from it, so an operator has
// one number to think about rather than two that can be set into conflict.
const NOTIFY_RESERVE_FRACTION = 0.25;
const notifyReserve = () => Math.ceil(dailyMax() * NOTIFY_RESERVE_FRACTION);
...
//
// `kind` selects the floor the send must stay above: `critical` (the default, so
// no existing caller changes) may spend the whole ceiling, `notification` stops
// at the reserve above. An unrecognised value is treated as critical rather than
// refused — a typo at a call site must not silently stop that mail going out.
function bookSend(kind) {
  const today = utcDay();
  if (budget.day !== today) { budget.day = today; budget.sent = 0; }
  const floor = kind === 'notification' ? notifyReserve() : 0;
  if (budget.sent >= dailyMax() - floor) return false;
  budget.sent += 1;
  return true;
}
...
//
// `kind` (optional, #618) is the budget class — see bookSend(). Deliberately not
// recorded on the outbox entry: test/mail.test.js deep-equals that shape.
async function send({ to, subject, text, replyTo, kind }) {
  ...
  if (!bookSend(kind)) {
    ...
    // Two distinct events, because they mean opposite things to the operator: a
    // notification refused by the reserve is the breaker WORKING (registration
    // is still safe), while an exhausted ceiling means signup is affected right
    // now. One shared event would bury the second in the first.
    const event = kind === 'notification' && sent < limit
      ? 'mail_notification_budget_reserved'
      : 'mail_daily_budget_exhausted';
    logger.warn({ event, sent, limit });
  ...
  if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
    ...
    // Subject only — never the body, which carries tokens, and never the
    // recipient, which is personal data (see sendSafe in lib/routes/account.js
    // for the full reasoning). The subject is our own copy and names no person,
    // so it survives: it is what makes this line useful when a self-hosted
    // instance is wondering why nothing arrives. The address is still in the
    // outbox entry above, which is in-memory and operator-only.
    logger.info({ event: 'mail_not_configured', subject });
