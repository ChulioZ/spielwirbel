...
// The three mailed one-time links (#434, #1076). Distinct prefixes so one kind
// of token can never be replayed against another endpoint, even though all
// three hash into the same `tokenHash` field shape.
const VERIFY_TOKEN_VERSION = 'v1';
const RESET_TOKEN_VERSION = 'p1';
// The pending address change. It shares VERIFY_TTL_MS but NOT the `verification`
// record: that key means "this account's CURRENT address is unconfirmed" and the
// login gate reads it, so reusing it would lock a verified account out mid-change.
const EMAIL_TOKEN_VERSION = 'e1';
...
//
// It lives here rather than in lib/routes/account.js (where it was until #418)
// because passkey login has to mint a session in EXACTLY the same way a password
// login does — same TTLs, same rotation, same eviction. A second copy in the
// passkey router would be free to drift, and the drift would be invisible: both
// would keep issuing usable tokens (.claude/rules/shared-constants-across-the-stack.md).
async function issueSession(user) {
  const refreshToken = mintRefreshToken(user.id);
  const { hash } = parseRefreshToken(refreshToken);
  const next = pushRefreshToken(user.refreshTokens, {
    tokenHash: hash,
  ...
  return {
    accessToken: mintAccessToken(user.id),
...
// The mailed verification/reset links (#434). Same "<version>.<uid>.<secret>"
// shape as a refresh token, and for the same reason: the user id rides inside
// the token, so the URL needs no second `uid=` parameter. Only the SECRET is
// hashed at rest, exactly as before — the stored record shape is unchanged, so
// it never encoded which link shape produced it. (That is what let the pre-#434
// `uid=` + bare-token pair keep verifying until its links expired; the fallback
// was removed in #451, once no such link could still match a live record.)
//
// 16 bytes, not the 32 newRawToken() uses, and that is the whole point of the
// issue: the URL has to fit on ONE quoted-printable line. Mail bodies are QP-
// encoded (the German text has umlauts) and QP wraps at 76 columns with a '='
// soft break — which used to land mid-URL, leaving clients whose auto-linkifier
// does not span the break showing half a link. 22 base64url chars keep
// https://spielwirbel.app/v?t=<token> at 70 characters. 128 bits is still ample
// for a single-use token that is time-limited (24 h / 1 h) and rate-limited.
const LINK_SECRET_BYTES = 16;
const newLinkSecret = () => crypto.randomBytes(LINK_SECRET_BYTES).toString('base64url');

const mintLinkToken = (version, userId, secret) => `${version}.${userId}.${secret}`;

// Split a link token into { userId, hash }, or null when it isn't one of ours
// (wrong version, wrong shape) — which the callers turn into a 400. The version
// check is load-bearing: both records hash into the same `tokenHash` field, so
// without it a `v1.` verification link would double as a password-reset link.
function parseLinkToken(version, token) {
  const parts = String(token || '').split('.');
...
// Since #955 it also hands the resolved account id downstream as
// `req.uploadUserId`: the ownership check that follows (lib/upload-access.js)
// needs to know WHOSE request this is, and re-verifying the same token there
// would be a second place that decides what a valid upload credential is.
function requireUploadAccount(req, res, next) {
  const uid = verifyAccessToken(bearerToken(req) || readCookie(req, ACCESS_COOKIE));
...
module.exports = {
  ...
  pushRefreshToken,
  issueSession,
  // Exported for lib/demo.js, which keys its IP hash with the same secret (#502)
  // rather than reading the env var a second time.
  signingSecret,
  hashPassword,
  verifyPassword,
  DUMMY_HASH_PROMISE,
