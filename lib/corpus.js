...
// The recommender's in-process snapshot (#682). Every write below drops it, so a
// freshly uploaded dump or a freshly enriched row is scored on the next request
// instead of waiting out the snapshot's TTL.
...
// How many games the corpus keeps, best-ranked first.
//
// The ceiling is really a budget for the ENRICHMENT, not for storage: each game
// costs 1/20th of an upstream request, so 5000 games is 250 requests (~6 h at the
// default pace) and the figure scales linearly — 20000 games is a full day of
// trickle before the corpus is complete. Raising it is a legitimate call for an
// instance whose groups play outside the well-known band; the upload reports
// `overCap` so the decision can be made against the real number rather than
// guessed at.
...
// How many batches may fail BACK TO BACK before a pass gives up (#774). Two,
// because one failure is a slow answer and the pass should ride it out, while a
// run of them is an outage there is no point spending the rest of the tick on.
// A success resets the count, so this bounds consecutive failures, never the
// total — a tick that hits several unlucky batches still spends its full budget.
 ...
 * { entries, total, filtered, overCap } or the marker string 'invalid_csv'.
 *
 * `filtered` and `overCap` are reported SEPARATELY, and that split is the whole
 * point of the shape. They are not the same fact: a filtered row was never a
 * candidate (an expansion, unranked, too thinly rated), while an over-cap row
 * passed every filter and lost only to BGG_CORPUS_SIZE. Collapsing them into one
 * "dropped" number — which is what shipped first — reports the cap's work as
 * junk filtering, so the operator is told 175,000 rows were unusable when an
 * unknown share of them were ordinary games the cap happened to exclude. That
 * hides the one number that decides whether the cap is set right.
 *
...
function parseRanksCsv(text) {
  ...
  return {
    entries: kept,
    total,
    // Eligible but over the ceiling — the number to raise BGG_CORPUS_SIZE by.
    overCap: entries.length - kept.length,
    filtered: total - entries.length,
  };
}
...
async function ingestCsv(text, { filename, uploadedAt } = {}) {
  ...
  logger.info({
    event: 'bgg_corpus_uploaded', rows, filtered: parsed.filtered, overCap: parsed.overCap,
  });
  return {
    rows, total: parsed.total, filtered: parsed.filtered, overCap: parsed.overCap, ...meta,
  };
}
...
async function enrich({ maxBatches, pauseMs, now } = {}) {
  ...
  const pending = await repo.listCorpusPending(batches * bgg.MAX_THING_IDS, staleBefore);
  if (!pending.length) return result;

  let attempts = 0;
  let consecutiveFailures = 0;
  for (let i = 0; i < pending.length; i += bgg.MAX_THING_IDS) {
    const slice = pending.slice(i, i + bgg.MAX_THING_IDS);
    // Paced off ATTEMPTS, not written batches, so a failed batch is still
    // followed by a pause — a struggling upstream is the last one to hammer.
    if (attempts > 0 && pause > 0) await sleep(pause);
    attempts += 1;
    ...
    try {
      ...
      // A failed batch is skipped, and only a SECOND CONSECUTIVE one stops the
      // pass (#774). Stopping on the first was the right instinct for an outage
      // and the wrong one for a single slow answer — and in production it was
      // always the latter, which cost ~60% of every tick. The counter is what
      // tells the two apart; a success resets it.
      //
      // Either way the batches already written stay written and the skipped
      // rows stay unstamped, so they are simply still owed on the next tick.
      // Every failure is reported, not just the stopping one, so a degrading
      // upstream is visible in the operator panel's log card rather than
      // showing up only as a slow fill.
      // `attempt` is what tells two failures apart: `batch` counts what has been
      // WRITTEN, so the two consecutive failures that end a pass would otherwise
      // both report the same number and read as one line logged twice.
      logger.warn({
        event: 'bgg_corpus_enrich_failed', batch: result.batches, attempt: attempts, message: err.message,
      });
      result.failed = true;
  ...
  if (result.asked) {
    // Only when something was actually stamped: a pass that asked nothing wrote
    // nothing, and dropping the snapshot then would re-read the whole corpus
    // every quarter of an hour once enrichment has caught up.
    corpusCache.invalidate();
    logger.info({ event: 'bgg_corpus_enriched', ...result });
  }
  return result;
}
