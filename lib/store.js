'use strict';

/*
 * Tiny data layer: keeps the whole dataset in memory and persists it to a single
 * file data/data.json. Cover images are stored as files under data/uploads/;
 * data.json only holds the path.
 *
 * All user-specific data lives under one folder (data/) so it can be ignored by
 * git as a whole. Override the location with the DATA_DIR environment variable.
 */

const fs = require('fs');
const path = require('path');
const crypto = require('crypto');
const { logger } = require('./observability');

const ROOT = path.join(__dirname, '..');
const DATA_DIR = process.env.DATA_DIR ? path.resolve(process.env.DATA_DIR) : path.join(ROOT, 'data');
const DATA_FILE = path.join(DATA_DIR, 'data.json');
const UPLOAD_DIR = path.join(DATA_DIR, 'uploads');

// Make sure the data + uploads folders exist (recursive creates DATA_DIR too).
if (!fs.existsSync(UPLOAD_DIR)) fs.mkdirSync(UPLOAD_DIR, { recursive: true });

function loadData() {
  try {
    const parsed = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
    if (!Array.isArray(parsed.rounds)) parsed.rounds = [];
    if (!Array.isArray(parsed.users)) parsed.users = [];
    if (!Array.isArray(parsed.moderationLog)) parsed.moderationLog = [];
    if (!Array.isArray(parsed.feedback)) parsed.feedback = [];
    if (!Array.isArray(parsed.contactNotices)) parsed.contactNotices = [];
    if (!Array.isArray(parsed.inbox)) parsed.inbox = [];
    if (!Array.isArray(parsed.roundGrants)) parsed.roundGrants = [];
    if (!Array.isArray(parsed.invitations)) parsed.invitations = [];
    if (!Array.isArray(parsed.friendships)) parsed.friendships = [];
    if (!Array.isArray(parsed.feedEvents)) parsed.feedEvents = [];
    if (!Array.isArray(parsed.sessionVoteLinks)) parsed.sessionVoteLinks = [];
    if (!Array.isArray(parsed.lastPrices)) parsed.lastPrices = [];
    return parsed;
  } catch (err) {
    /* File missing -> a fresh instance (and every test run), which is normal and
       must stay silent: the ring buffer the operator panel reads holds 200
       entries and this runs at require time on every process.

       Anything ELSE — unreadable, or valid-JSON-shaped garbage — means we just
       started an EMPTY dataset over data that exists, which is the JSON
       backend's worst failure and used to be completely mute (#1148).

       `code`/`name`, NEVER `message`: a JSON.parse error quotes the offending
       bytes back ("Unexpected token 'o', \"not json at\"... is not valid JSON"),
       so the message of a corrupt data.json carries a snippet of the group's own
       data into the logs. Same reason client_error logs `type` and not
       `message` — .claude/rules/client-errors-are-not-instance-faults.md. */
    if (err.code !== 'ENOENT') {
      logger.error({ event: 'data_load_failed', code: err.code || err.name });
    }
    return { rounds: [], users: [], moderationLog: [], feedback: [], contactNotices: [], inbox: [], roundGrants: [], invitations: [], friendships: [], feedEvents: [], sessionVoteLinks: [], lastPrices: [] };
  }
}

// Stable reference: data is never reassigned after loading, only mutated.
const data = loadData();

// Writing is atomic (temp file first, then rename) so data.json is never left
// half-written after a crash.
function saveData() {
  const tmp = DATA_FILE + '.tmp';
  fs.writeFileSync(tmp, JSON.stringify(data, null, 2), 'utf8');
  fs.renameSync(tmp, DATA_FILE);
}

const id = () => crypto.randomBytes(8).toString('hex');

const findRound = (rid) => data.rounds.find((r) => r.id === rid);

// Append an activity entry to a round (for the activity feed).
function pushActivity(round, type, payload) {
  if (!Array.isArray(round.activities)) round.activities = [];
  round.activities.push({ id: id(), type, at: new Date().toISOString(), ...payload });
}

module.exports = { ROOT, DATA_DIR, DATA_FILE, UPLOAD_DIR, data, saveData, id, findRound, pushActivity };
