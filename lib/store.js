...
// One-time migration of the old German field names (aussortiert/aussortieren)
// to English (retired/retire). Runs on load; the next saveData() persists it.
function migrate(data) {
  for (const round of data.rounds) {
    for (const g of round.games || []) {
      if ('aussortiert' in g) { g.retired = g.aussortiert; delete g.aussortiert; }
      if ('aussortiertAt' in g) { g.retiredAt = g.aussortiertAt; delete g.aussortiertAt; }
      if (!('retired' in g)) g.retired = false;
      if (!('retiredAt' in g)) g.retiredAt = null;
    }
    for (const s of round.sessions || []) {
      for (const mid in s.votes || {}) {
        for (const gid in s.votes[mid] || {}) {
          const v = s.votes[mid][gid];
          if (v && 'aussortieren' in v) { v.retire = v.aussortieren; delete v.aussortieren; }
        }
      }
    }
    for (const a of round.activities || []) {
      if (a.type === 'game_aussortiert') a.type = 'game_retired';
    }
  }
  return data;
}
