 ...
 * before it was closed). One named job per entry.
 *
 ...
 *
 * Multi-replica note: production can run more than one instance (#215's shared
 * store was closed unshipped), and then every replica runs this loop and they
 * overlap. That is safe rather than merely tolerated — the purge is idempotent
 * (it re-reads what is expired each tick, and eraseAccount on an
 * already-erased id answers null, which the caller skips), so a double sweep
 * does the work once and no-ops the rest.
 */
...
const JOBS = {
  ...
  // The moderation log's 3-year retention purge (#311). Always enabled, for the
  // reason the two sweeps above are: the promise in the published privacy policy
  // has no feature flag, so the sweep that keeps it must not either.
  //
  // A tick every 15 minutes for a job whose cutoff moves once a year is
  // enormously more often than it needs — and it is still the cheapest option
  // available: one indexed DELETE that matches nothing, against a second timer
  // with its own annual cadence that nothing else in the process would share and
  // that a restart-happy container would keep resetting.
  purgeModerationLog: {
    enabled: () => true,
    run: () => retention.purgeModerationLog(),
  },
  // Freundeskreis feed events past their 12-month retention (#1357). Always
  // enabled, for the reason the retention sweeps above are: the statement in
  // docs/legal/retention.md has no feature flag. Rows written while accounts
  // were on must still age out after ACCOUNTS_ENABLED is switched off.
  //
  // A full scan over the JSONB `at` on Postgres (no indexed column) — cheap at
  // a table the age limit and the per-account ceiling bound, and it is the one
  // query per tick this job adds.
  purgeExpiredFeedEvents: {
    enabled: () => true,
    run: () => feed.purgeExpiredFeedEvents(),
  },
