 ...
 * null/empty rather than throwing. The pure parsers are exported for unit tests
 * (no network needed).
 */
...
// market we ask (#505). Microsoft spells a locale as a language-region tag.
//
// This value is interpolated into the "View on Xbox" URL path, so it may only
// ever come out of this closed table or the env fallback — never out of the
...
// Read per call, never at module load, so a test can drive it
// (.claude/rules/security-middleware.md).
const defaultLocale = () => process.env.XBOX_LOCALE || 'de-de';
...
const marketOf = (locale) => (locale.split('-')[1] || 'us').toUpperCase();
const USER_AGENT =
...
// True if an autosuggest entry is a game rather than an app, a device or other
// storefront noise.
//
// It must test `Metas.ProductType` ('Games'), NOT the sibling `Source` field:
// **Source is LOCALIZED**. Measured live 2026-07-28 on the same query, it comes
// back 'Game' for de-de/en-us/nl-nl but 'Jeu' for fr-fr and 'Juego' for es-es —
// so the original `Source === 'Game'` test silently returned ZERO Xbox results
// for French, Spanish, Italian and Portuguese the moment the locale became
// per-request (#505). Nothing errors; the provider just quietly finds nothing.
// ProductType is stable across every market checked.
//
// Source is still honoured as a fallback for an entry carrying no ProductType,
// so an autosuggest shape that predates this stays readable.
function isGameSuggest(s) {
  const productType = metaValue(s, 'ProductType');
  if (productType) return productType === 'Games';
  return s.Source === 'Game';
}

// Parse an autosuggest response into [{ providerId, title, thumbnail }]. Walks
// every ResultSet, keeps only game suggestions carrying a BigCatalogId. Deduped
// by id; entries without an id or title are skipped. Never throws.
...
function parseDetail(json, externalId, locale = defaultLocale()) {
...
async function search(query, limit = 8, requested) {
  const params = new URLSearchParams({
    market: resolveLocale(requested),
