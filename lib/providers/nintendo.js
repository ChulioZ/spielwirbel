...
// The Solr index is localized per NoE language code, and the requesting user's
// UI language decides which one (#505). The code is interpolated into the
// fetched URL path, so it may only ever come out of this closed table or the env
// fallback — never out of the request. See ./locales.js.
//
// `pt` IS served here (verified live 2026-07-28: it answers with European
// Portuguese, /pt-pt/ store paths). This is Nintendo of EUROPE, so it is pt-PT
// rather than the pt-BR the other three storefronts map `pt` to — European
// Portuguese for a Portuguese-speaking user beats falling back to English.
//
// An unknown code does NOT error: the endpoint answers 200 with an empty doc
// set, i.e. it fails silently as "this provider found nothing" — which is the
// whole reason the value is mapped rather than passed through.
...
const defaultLocale = () => process.env.NINTENDO_LOCALE || 'de';
