...
// The language word that goes with each country code above.
...
// The { cc, l } pair this request should use. A mapped country brings its own
// language; only the unmapped fallback takes both from the env, so a deployment
// that pins STEAM_CC/STEAM_LOCALE to a mismatched pair keeps that pairing.
function steamLocale(requested) {
  const cc = locales.resolveLocale(LOCALES, requested, null);
  if (cc === null) return { cc: defaultCc(), l: defaultLanguage() };
  return { cc, l: LANGUAGES.get(cc) };
}

// The effective upstream locale, as the cache-key component routes/lookup.js
