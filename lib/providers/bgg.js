...
// The corpus enrichment hop's own budget (#774), and it differs on purpose —
// this is not an inconsistency to be tidied away. That hop asks for the heaviest
// document this app ever requests from BGG (20 ids x stats=1), on a 15-minute
// background tick, with no user waiting on the answer; TIMEOUT_MS is tuned for
// the interactive lookup, where the budget is shared with every search. Under
// the 8 s one it aborted mid-batch three times an hour in production and the
// corpus filled at ~40% of the documented rate. Hard-coded rather than env-
// tunable: the corpus already has four BGG_CORPUS_* dials and this one has no
// per-instance reason to move.
const CORPUS_TIMEOUT_MS = 30000;
...
//
// `maxBatches` is how many upstream requests the caller is willing to spend.
// The batch SIZE stays this module's business, so a caller asking for one
// request says exactly that rather than restating 60 — the shelf-wide backfill
// trigger uses it to keep a screen open at one /thing call however big the shelf.
async function gameInfo(ids, { maxBatches = MAX_PARENT_BATCHES } = {}) {
  const batches = Math.min(Math.max(1, maxBatches), MAX_PARENT_BATCHES);
  const asked = [...new Set((ids || []).map((x) => String(x || '').trim()).filter(Boolean))]
    .slice(0, batches * MAX_EXPANSION_BATCH);
  ...
  const items = [];
  for (let i = 0; i < asked.length; i += MAX_EXPANSION_BATCH) {
    const params = new URLSearchParams({ id: asked.slice(i, i + MAX_EXPANSION_BATCH).join(','), stats: '1' });
    items.push(...parseGameInfo(await fetchXml(`${API}/thing?${params.toString()}`)));
  }
  return { items, asked };
}
...
// BGG documents the /thing id limit as **20** ("Maximum 20"), and this hop obeys
// it rather than the 60 its neighbours use — see the note in issue #681 about
// MAX_EXPANSION_BATCH, which is a pre-existing question deliberately left alone.
// Here there is no reason to push it: the corpus fills over hours by design.
const MAX_CORPUS_BATCH = 20;
...
async function corpus(ids) {
  ...
  const xml = await fetchXml(`${API}/thing?${params.toString()}`, CORPUS_TIMEOUT_MS);
