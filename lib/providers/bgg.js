...
// Weight + description off a /thing?stats=1 item's flattened children (#717).
//
// `averageweight` MUST be matched by its exact name: parseItems flattens every
// descendant of <item> into one child list, so the <statistics><ratings> block's
// `average`, `bayesaverage` and `rank` siblings all sit right beside it — and
// rank/average are precisely the community scores the feature deliberately does
// NOT import (the user feedback called them counterproductive).
//
...
// The decoded text is stored unmodified (BGG's licence forbids modifying
// retrieved data); any clamping is display-only.
function infoOf(children) {
  const weightNode = (children || []).find((c) => c.name === 'averageweight');
  const descNode = (children || []).find((c) => c.name === 'description' && c.text);
  return {
...
function parseThing(xml, externalId) {
  ...
  return {
    ...
    // Weight + description (#717): null-shaped like every other field when the
    // body lacks them (a token-absent detail() call, or a game BGG has no
    // weight votes for). weight needs stats=1 on the request; description is in
    // every /thing body.
    ...infoOf(children),
...
// Weight + description for MANY games in one /thing?stats=1 call (#717), for
// the lazy backfill of games added before the fields existed. BGG-only like
...
async function gameInfo(ids) {
  const list = [...new Set((ids || []).map((x) => String(x || '').trim()).filter(Boolean))]
    .slice(0, MAX_EXPANSION_BATCH);
  if (!list.length || !apiToken()) return [];
  const params = new URLSearchParams({ id: list.join(','), stats: '1' });
  return parseGameInfo(await fetchXml(`${API}/thing?${params.toString()}`));
}
