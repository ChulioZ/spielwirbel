...
// Overall budget for one provider call, retries included (see fetchXml). Tuned
// for the INTERACTIVE hops — a search, a single /thing — where a user is waiting
// and the budget is shared with every other lookup on the screen.
const TIMEOUT_MS = 8000;
// The budget for a MULTI-ID /thing?stats=1 batch (#774, widened to gameInfo in
// #828), and it differs on purpose — this is not an inconsistency to be tidied
// away. A full 20-id batch is the heaviest document this app ever requests
// (~471 KB measured, against ~47 KB for one game), and under the 8 s budget it
// aborted mid-batch three times an hour in production: the corpus filled at ~40%
// of the documented rate. Nobody is waiting on either caller — the corpus rides
// a 15-minute tick, the shelf fill runs behind a response already sent.
//
// Hard-coded rather than env-tunable: the corpus already has four BGG_CORPUS_*
// dials and this one has no per-instance reason to move.
const BULK_TIMEOUT_MS = 30000;

// BGG REFUSES MORE THAN 20 IDS ON /thing. Its docs say "Maximum 20"; the server
// enforces it with `400 Cannot load more than 20 items` — measured 2026-08-26 at
// 21 ids and at 60, with and without `stats=1`. 400 is not a retryable status, so
// an over-long batch fails fast and completely.
//
// It is ONE constant for every hop that batches ids, because it is BGG's limit
// rather than a per-hop choice. It used to be stated twice at two different
// values (20 for the corpus, 60 for the other three), and #828 is what that cost:
// every bulk metadata backfill 400'd from the day it shipped, so a round whose
// games came from a collection import never got its provider metadata and the
// „Weitere Filter" controls never appeared. The demo round was the only one that
// worked — nine games, one under-limit request.
const MAX_THING_IDS = 20;
...
// Parse an /xmlapi2/search response into [{ providerId, title, thumbnail, year }],
// best match first. thumbnail is null — search carries no image, the cover
// arrives with the detail on pick. Never throws.
//
// `year` is BGG's own disambiguator: several distinct games share a title
// exactly ("Scout"), and the dropdown offers one row per hit (#790), so without
// it the rows are indistinguishable. It is a picking aid only — nothing stores
// it on the game.
function parseSearch(xml, limit = 8, query = '') {
  ...
  for (const item of parseItems(xml)) {
    ...
    // "0" is BGG's "unknown" here as everywhere else, so toPositiveInt maps it
    // to null rather than rendering a row that reads "Scout (0)".
    const yearNode = item.children.find((c) => c.name === 'yearpublished');
    seen.add(id);
    hits.push({ providerId: id, title, score, year: yearNode ? toPositiveInt(yearNode.attrs.value) : null });
  }
  ...
  return hits.slice(0, limit)
    .map((h) => ({ providerId: h.providerId, title: h.title, thumbnail: null, year: h.year }));
}
...
// even in the German UI — the identifiers-are-not-localized half of
...
// (#717), playing time / age / categories / mechanics / rating (#724).
//
// <description> is deliberately NOT read (#729): BGG's prose is publisher
// marketing copy, English-only for a bilingual UI, and it answered none of the
// question a voter actually has — playing time does, in four characters. Don't
// reinstate it here without reinstating the surfaces that rendered it.
//
...
function parseThing(xml, externalId) {
  ...
  return {
    ...
    // has no votes for). Only `weight`/`rating` need stats=1 on the request; the
    // playtime bounds, the age and the category/mechanic links are in every
    // /thing body — measured 2026-08-09, a no-stats Catan body
...
//
// `timeoutMs` overrides that budget PER CALL (#774), which is how the corpus hop
// gets a wider one without every other caller inheriting it. Note what it does
// NOT change: an abort is a throw, not a status, so it escapes the retry loop
// below entirely — RETRY_STATUS only ever sees an answer BGG actually sent.
async function fetchXml(url, timeoutMs = TIMEOUT_MS) {
...
// shape as the locale tables were,
// .claude/rules/allowlist-request-values-that-reach-a-url.md).
//
...
// the item's default image. BGG-only: not every game database has an equivalent
...
// BGG-only, like collection() and covers(): not every game database has a
// concept of an expansion, so lib/routes/lookup.js treats the presence of this
// function as the capability marker and answers 400 for a provider without it.
//
...
//
// THREE BATCHES, because the quota allows 40 expansions on one game and a save
// that ticks more than MAX_THING_IDS of them must not fail (#828). Before that
// this was one request for up to 60 ids, which BGG answers 400 — surfacing to
// the user as `provider_unreachable` on a perfectly reachable provider.
const MAX_EXPANSION_IDS = 3 * MAX_THING_IDS;

async function expansionDetails(ids) {
  const list = [...new Set((ids || []).map((x) => String(x || '').trim()).filter(Boolean))]
    .slice(0, MAX_EXPANSION_IDS);
  if (!list.length || !apiToken()) return [];
  const out = [];
  for (let i = 0; i < list.length; i += MAX_THING_IDS) {
    const params = new URLSearchParams({ id: list.slice(i, i + MAX_THING_IDS).join(',') });
    out.push(...parseExpansionDetails(await fetchXml(`${API}/thing?${params.toString()}`)));
  }
  return out;
}
...
// collection()/covers(): not every game database has a comparable weight, so the
...
// Sliced per ITEM rather than run over the flat child list, which it was until
// #1005: `parseItems` flattens every descendant into one list, so a two-item
// body's polls would merge. The suggested-players poll is the field that forces
// it — every other field here is a leaf `parseItems` can hand back flat.
function parseGameInfo(xml) {
  const out = [];
  ITEM_BLOCK_RE.lastIndex = 0;
  let m;
  while ((m = ITEM_BLOCK_RE.exec(String(xml || ''))) !== null) {
    const item = parseItems(m[0])[0];
    if (!item) continue;
    const providerId = String(item.attrs.id || '');
    if (!providerId) continue;
    out.push({ providerId, ...infoOf(item.children), ...parsePlayerPoll(m[0]) });
  }
  return out;
}

// ONE batch of metadata, for the lazy backfill in lib/provider-info.js. The
// SIZE is this module's business (BGG's limit); the PACING and the batch count
// belong to the caller, which is what decides how much of BGG's patience one
// screen open or one import may spend. That split is `corpus()`'s exactly, and
// since #828 the two hops are the same shape for the same reason: they request
// the same document.
//
// It REPORTS WHICH IDS IT ASKED ABOUT (#736), and that is the whole reason for
// the `{ items, asked }` shape. The ceiling below silently drops the overflow,
// so a caller that assumed "I passed these ids, therefore they were asked
// about" recorded games as "asked, BGG had nothing" without a request ever
// leaving the process — suppressing them for the full 7-day TTL. Returning the
// covered set makes that structurally impossible instead of leaving the caller
// to re-derive a bound that lives here (.claude/rules/provider-info-is-a-field-set.md).
//
// The BULK budget, not the interactive one, whenever more than a single id is
// asked for: a full batch is the ~471 KB body #774 measured aborting under 8 s.
// A one-id call is the game detail page with a user waiting on it, so it keeps
// the interactive budget — the size of the document is what the two differ over,
// and the id count is exactly that.
async function gameInfo(ids) {
  const asked = [...new Set((ids || []).map((x) => String(x || '').trim()).filter(Boolean))]
    .slice(0, MAX_THING_IDS);
  // No token: nothing was asked, so nothing may be stamped either. That is a
  // deliberate difference from returning an empty list — a tokenless instance
  // must not record its whole shelf as "BGG had nothing", or configuring the
  // token later would leave every game waiting out a TTL for data it could
  // have had immediately.
  if (!asked.length || !apiToken()) return { items: [], asked: [] };
  const params = new URLSearchParams({ id: asked.join(','), stats: '1' });
  const timeout = asked.length > 1 ? BULK_TIMEOUT_MS : TIMEOUT_MS;
  return { items: parseGameInfo(await fetchXml(`${API}/thing?${params.toString()}`, timeout)), asked };
}
...
//
// #828 kept the REQUEST budget at five and let the id ceiling fall from 300 to
// 100 with the batch size, rather than the reverse. Five requests is what the
// paragraph above justifies — this hop sits inside the wishlist listing, with a
// user waiting on it — and fifteen of them is precisely the "minutes" it is
// guarding against. The lost ceiling is theoretical next to what it replaces: at
// 60 ids a batch, every one of these requests had been answered 400.
...
//
// Batched at MAX_THING_IDS like every other /thing hop. Sequential rather than
// parallel on purpose: the batches share one upstream's throttling budget, and
// BGG's terms ask for few requests, not fast ones.
async function expansionParents(ids) {
  const list = [...new Set((ids || []).map((x) => String(x || '').trim()).filter(Boolean))]
    .slice(0, MAX_PARENT_BATCHES * MAX_THING_IDS);
  if (!list.length || !apiToken()) return [];
  const out = [];
  for (let i = 0; i < list.length; i += MAX_THING_IDS) {
    const params = new URLSearchParams({ id: list.slice(i, i + MAX_THING_IDS).join(',') });
...
/* ------------------------- the recommendation corpus ------------------------ */
//
// The per-game attributes the local game corpus scores on (issue #681). BGG's
// XML API has no browse, filter or attribute-search endpoint at all, so a
// "which games would fit this group?" question cannot be asked upstream — the
// app holds its own candidate pool (seeded from the licensed ranks CSV) and
// enriches each row from /thing?stats=1 here.
//
// Two reasons this is its own parser rather than a widened parseGameInfo:
//   - it reads fields that hop never needed (player counts, numweights, the
//     families/designers/implementations link families) and, crucially, the
//     suggested-players POLL, which needs per-item slicing rather than the flat
//     child list every other parser here works from;
//   - parseGameInfo's product is stamped onto a user's own game row, so widening
//     it changes what public/js/provider-info-fields.js writes. These two answer
//     different questions about the same document — though #1005 deliberately
//     narrowed the gap: the suggested-players poll is now on BOTH sides, under
//     the SAME two key names, so a corpus row can fill a shelf game's poll with
//     no upstream hop at all (`corpusPatch`). The rest of the corpus row (player
//     counts, families, designers, implementations) still reaches nobody's game.
//
// Every value is stored VERBATIM as BGG reports it — the licence forbids
// modifying retrieved data (#117/#172), so nothing here translates a category
// name or rescales a weight.

// One <item>…</item> block. The poll below has to be read per item, and
// parseItems deliberately flattens every descendant into one child list — so
// splitting first is the only way a two-item body's polls stay apart. Same
// slice-then-parse shape as parseVersions, and the attribute group uses TAG_RE's
// disjoint alternatives so a '>' inside an attribute value cannot cut it short.
//
// The `(?=[\s>/])` lookahead on each is LOAD-BEARING, not tidiness — every one
// of these four tag names is a prefix of another tag in the same document, and
// without it the opening tag matches the wrong element while the closing tag
// still matches the right one, so the block silently spans the wrong region:
//   <item  -> <items>       (the document wrapper: the whole first game is
//                            swallowed as that wrapper's "attributes" and lost)
//   <poll  -> <poll-summary> (which carries the same name= attribute, so the
//                            name check does not save you)
//   <result -> <results>
// Measured: without it a two-item body parsed as one item.
const ITEM_BLOCK_RE = /<item(?=[\s>/])((?:"[^"]*"|'[^']*'|[^>"'])*)>([\s\S]*?)<\/item>/g;
const POLL_BLOCK_RE = /<poll(?=[\s>/])((?:"[^"]*"|'[^']*'|[^>"'])*)>([\s\S]*?)<\/poll>/g;
const RESULTS_BLOCK_RE = /<results(?=[\s>/])((?:"[^"]*"|'[^']*'|[^>"'])*)>([\s\S]*?)<\/results>/g;
const RESULT_RE = /<result(?=[\s>/])((?:"[^"]*"|'[^']*'|[^>"'])*)>/g;

const VOTE_KINDS = ['Best', 'Recommended', 'Not Recommended'];

// The community's verdict on which player counts a game actually works at —
// which is the single most useful thing BGG knows that a box does not, since a
// 1–4 game is routinely "best at 2" and outright bad at 4.
//
// Derived from the raw <poll> votes rather than read off <poll-summary>, whose
// two values are English PROSE ("Best with 2 players", "Recommended with 1–3
// players", note BGG's own triple-m typo in `recommmendedwith`). Scraping a
// number back out of a sentence would break on the first phrasing change and on
// its en-dash; the votes are numbers already.
//
// The "N+" bucket is DROPPED, not parsed. It means "more players than the box
// allows", and parseInt('4+') is 4 — which would silently merge it into the real
// 4-player row and corrupt both verdicts. Only a bare integer is a player count.
function parsePlayerPoll(chunk) {
  let block = null;
  POLL_BLOCK_RE.lastIndex = 0;
  let p;
  while ((p = POLL_BLOCK_RE.exec(chunk)) !== null) {
    // Three polls ship on every item (suggested_numplayers, suggested_playerage,
    // language_dependence); taking the first would read player ages as seats.
    if (attrsOf(p[1]).name === 'suggested_numplayers') { block = p[2]; break; }
  }
  if (!block) return { bestWith: [], recommendedWith: [] };

  const rows = [];
  RESULTS_BLOCK_RE.lastIndex = 0;
  let m;
  while ((m = RESULTS_BLOCK_RE.exec(block)) !== null) {
    const key = attrsOf(m[1]).numplayers || '';
    if (!/^\d+$/.test(key)) continue;
    const votes = { Best: 0, Recommended: 0, 'Not Recommended': 0 };
    RESULT_RE.lastIndex = 0;
    let r;
    while ((r = RESULT_RE.exec(m[2])) !== null) {
      const a = attrsOf(r[1]);
      const n = parseInt(a.numvotes, 10);
      if (VOTE_KINDS.includes(a.value) && Number.isInteger(n) && n >= 0) votes[a.value] = n;
    }
    rows.push({ players: parseInt(key, 10), votes });
  }

  const topBest = Math.max(0, ...rows.map((r) => r.votes.Best));
  return {
    // A tie keeps both counts rather than picking one arbitrarily. Zero votes
    // anywhere means the poll has not been answered — an empty list, never a 1.
    bestWith: topBest > 0 ? rows.filter((r) => r.votes.Best === topBest).map((r) => r.players) : [],
    // BGG's own reading of the poll: a count is recommended when the positive
    // votes outweigh the negative ones. All-zero rows fail it (0 > 0 is false),
    // so an unanswered poll yields nothing here either.
    recommendedWith: rows
      .filter((r) => r.votes.Best + r.votes.Recommended > r.votes['Not Recommended'])
      .map((r) => r.players),
  };
}

// Parse a /thing?stats=1 body into one corpus row per item. Never throws; a
// field BGG has no data for is null (or an empty list), which is a real state —
// a game nobody has weighted yet must not read as weight 0.
function parseCorpusThing(xml) {
  const out = [];
  ITEM_BLOCK_RE.lastIndex = 0;
  let m;
  while ((m = ITEM_BLOCK_RE.exec(String(xml || ''))) !== null) {
    const providerId = String(attrsOf(m[1]).id || '');
    if (!/^\d+$/.test(providerId)) continue;
    const item = parseItems(m[0])[0];
    const children = item ? item.children : [];
    const node = (name) => children.find((c) => c.name === name);
    const num = (name) => {
      const n = node(name);
      return n ? toPositiveInt(n.attrs.value) : null;
    };
    ...
    out.push({
      providerId,
      // The box art, off the SAME body — `<thumbnail>` is a child of every item
      // this parser already walks, so the cover costs no extra request (#779).
      // It is hotlinked, never re-hosted (.claude/rules/provider-cover-hotlinking.md),
      // and `pickImage` takes the thumbnail rather than `<image>` on purpose:
      // the master is 68 KB – 2 MB and BGG's transform paths are signed, so a
      // right-sized variant can only be picked here, at capture time
      // (.claude/rules/provider-cover-sizing.md).
      //
      // null for a game BGG has no thumbnail for is a SETTLED answer, not a gap
      // to re-ask about — see listCorpusPending's backfill clause, which tests
      // for the key's ABSENCE precisely so a null never re-enters the queue.
      imageUrl: pickImage(children),
      weight: weightNode ? toWeight(weightNode.attrs.value) : null,
      // How many people weighted it — the corpus needs it to know whether a
      // weight is worth believing, which the weight alone cannot say.
      numWeights: num('numweights'),
      minPlayers: num('minplayers'),
      maxPlayers: num('maxplayers'),
      minPlaytime: num('minplaytime'),
      maxPlaytime: num('maxplaytime'),
      minAge: num('minage'),
      categories: linkValues(children, 'boardgamecategory'),
      mechanics: linkValues(children, 'boardgamemechanic'),
      families: linkValues(children, 'boardgamefamily'),
      designers: linkValues(children, 'boardgamedesigner'),
      // "This game is a reimplementation of X" — the one link family that says
      // two entries are the SAME game in different clothes, so a recommender can
      // avoid offering a group the edition they already own.
      implementations: linkValues(children, 'boardgameimplementation'),
      ...parsePlayerPoll(m[0]),
    });
  }
  return out;
...
// One batch. The PACING between batches lives with the caller (lib/corpus.js),
// which is what decides how much of BGG's patience one scheduler tick may spend;
// the batch SIZE is this module's business, the same split gameInfo draws.
//
// This hop was the only one that obeyed BGG's documented limit — the note in
// #681 called the neighbouring 60 "a pre-existing question deliberately left
// alone", and #828 is the answer: it was not a question, it was a bug, and the
// two values are now one MAX_THING_IDS.
//
// Reports back which ids it asked about, like gameInfo (#736): the slice below
// silently drops an over-long list, and a caller that assumed "I passed these,
// therefore they were covered" would stamp rows as enriched without a request
// ever leaving the process.
async function corpus(ids) {
  const asked = [...new Set((ids || []).map((x) => String(x || '').trim()).filter(Boolean))]
    .slice(0, MAX_THING_IDS);
  // No token: nothing was asked, so nothing may be stamped — a tokenless
  // instance must not record its whole corpus as "BGG had nothing", or setting
  // the token later would leave every row waiting out a staleness window for
  // data it could have had at once.
  if (!asked.length || !apiToken()) return { items: [], asked: [] };
  const params = new URLSearchParams({ id: asked.join(','), stats: '1' });
  const xml = await fetchXml(`${API}/thing?${params.toString()}`, BULK_TIMEOUT_MS);
  return { items: parseCorpusThing(xml), asked };
}
...
module.exports = {
  ...
  corpus,
  MAX_THING_IDS,
  // The canonical page for a corpus row (#682). The corpus holds base games
  // only — expansions are dropped at ingest — so the subtype is fixed, unlike
  // parseThing's, which keys the link off the item's own type. It lives here
  // rather than in the consumer so the web host is stated once.
  gameUrl: (externalId) => `${WEB}/boardgame/${encodeURIComponent(String(externalId))}`,
