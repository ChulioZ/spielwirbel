...
function parseCollection(xml) {
  ...
  for (const item of parseItems(xml)) {
    ...
    items.push({
      ...
      // The ONLY thing a collection body says about an expansion (#664): it
      // carries no <link> elements at all, so which base game it belongs to
      // needs the separate expansionParents() hop. Present on every item so the
      // shape stays uniform for the owned shelf, which never sees a true.
