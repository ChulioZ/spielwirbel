 ...
 *
 * Plus one cap that is enforced in EVERY mode, and is the exception to the
 * paragraph below: saved filters per round (lib/routes/saved-filters.js, #1328).
 * It bounds a UI surface rather than abuse — every saved filter is a chip in the
 * hub's CTA row, which holds a handful, not dozens — so a self-hosted round has
 * the same row and gets the same ceiling. Still env-tunable, like the rest.
 *
...
const DEFAULT_SAVED_FILTERS_PER_ROUND = 6;
...
// Max saved session filters one round may hold (MAX_SAVED_FILTERS_PER_ROUND,
// default 6, #1328). Enforced in every mode — see the header for why this one
// is not gated on enforced(). Six is what the hub's chip row carries at phone
// width in two lines without turning into a second menu.
function savedFiltersPerRound() {
  return Number(process.env.MAX_SAVED_FILTERS_PER_ROUND) || DEFAULT_SAVED_FILTERS_PER_ROUND;
}
...
module.exports = {
  ...
  DEFAULT_SAVED_FILTERS_PER_ROUND,
