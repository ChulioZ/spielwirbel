 ...
 *
 * Bounds cost/abuse once public multi-tenant sign-up opens (#219). Three caps:
...
// Middleware for POST /api/rounds/:rid/recommendations — the billed Claude call.
// Runs after the tenant middleware, so req.tenantId is set. Inert unless quotas
// are enforced. Refuses once the month's quota is spent (429) and, on the way
// out, counts only a successful (200) generation against the tenant.
