...
function buildChildSessions(round, session, tables, startedEvent) {
  ...
  return tables.map((tb) => {
    ...
    // The parent's „came without their shelf" marks (#1002), narrowed to the
    // people actually at THIS table. Inherited rather than dropped because the
    // child is the session whose results screen prints „Gehört Anna": losing
    // them here would send one table to fetch a box from somebody who is at
    // another one without it, which is the split-session version of the exact
    // bug the marks exist to prevent.
    const withoutShelfIds = (session.withoutShelfIds || []).filter((x) => memberIds.includes(x));
