...
/*
 * Data-access layer (issue #127 — step 1 of the persistence migration).
 *
 * Routes read and write the app's data ONLY through this async API — never by
 * mutating the in-memory tree from lib/store.js directly. That decoupling is the
 * seam the PostgreSQL backend (a follow-up PR) drops into: every method here is
 * one logical read or write, so a SQL implementation can satisfy the same
 * contract without the routes changing again.
 *
 * This first backend is the existing JSON file store (lib/store.js), so behaviour
 * is unchanged and local dev keeps its zero-dependency start. The API is async
 * (returns Promises) from the outset so swapping in Postgres won't force a second
 * rewrite of every route; Express 5 forwards any rejection to the central error
 * handler (see lib/app.js).
 *
 ...
 * This mirrors a real database (a fetched row is a copy) and keeps the JSON
 * backend honest about the same contract, so the two backends behave alike.
 *
