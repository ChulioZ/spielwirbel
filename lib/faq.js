...
<p>Eine Runde läuft von einem Gerät: Ihr gebt es beim Abstimmen herum, statt
dass sich alle einzeln anmelden.</p>
<p>Wer trotzdem ein Konto hat, kann seinen Platz damit verknüpfen und pro Session
auch vom eigenen Gerät abstimmen. Das ist eine Möglichkeit, keine Voraussetzung —
Namensplätze stimmen unverändert am gemeinsamen Gerät ab.</p>
<p>Und ganz ohne Konto geht es auch: Zu einer laufenden Geräte-Abstimmung lässt
...
<p>A round runs from one device: you pass it around to vote, rather than
everyone signing in separately.</p>
<p>Anyone who does have an account can link their seat to it and, per session,
vote from their own device as well. That is an option, not a requirement —
name-only seats keep voting on the shared device exactly as before.</p>
<p>It also works with no account at all: a running per-device vote can be shared
...
<p>There is no App Store or Google Play listing.</p>`,
  ...
  },
  {
    ...
    de: {
      ...
      a: `<p>Nein. Beim Anlegen eines Spiels sucht das Titelfeld gleichzeitig bei
<strong>BoardGameGeek</strong> (Brettspiele) sowie im <strong>PlayStation
Store</strong>, bei <strong>Steam</strong>, im <strong>Nintendo eShop</strong>
und im <strong>Xbox/Microsoft Store</strong> (digitale Spiele).</p>
<p>Titel, Cover und Spieleranzahl werden dabei übernommen. Von Hand eintragen
geht aber genauso — dann ist völlig egal, was für ein Spiel es ist.</p>`,
    },
    en: {
      q: 'Is it only for board games?',
      a: `<p>No. When you add a game, the title field searches
<strong>BoardGameGeek</strong> (board games) together with the
<strong>PlayStation Store</strong>, <strong>Steam</strong>, the
<strong>Nintendo eShop</strong> and the <strong>Xbox / Microsoft Store</strong>
(digital games).</p>
<p>Title, cover art and player count are filled in for you. Typing a game in by
hand works just as well — and then it can be anything at all.</p>`,
    },
  },
  {
    id: 'export',
...
<p>Einen Export direkt in der App — ein Klick, eine Datei — gibt es derzeit
nicht.</p>`,
    },
    en: {
...
<p>There is currently no one-click export from inside the app itself.</p>`,
    },
  },
  {
    id: 'data',
...
<head>
  ...
  <style>
    :root {
      ...
      --ink: #2b2620;
      --ink-soft: #6b6358;
      --line: color-mix(in srgb, var(--page-bg), #000 9%);
      --sunken: color-mix(in srgb, var(--page-bg), #000 5%);
      --brand-dark: color-mix(in srgb, var(--brand), #000 18%);
      --page-glow: color-mix(in srgb, var(--brand) 7%, transparent);
      --radius-lg: 18px;
