...
router.get('/:rid', async (req, res) => {
  ...
  const out = redactRoundVotes(round);
  res.json(req.grant ? { ...out, shared: true } : out);
});
...
router.patch('/:rid', async (req, res) => {
  ...
  // marker that hides owner-only actions from a grantee — and the same vote
  // redaction, since this response replaces the client's whole round object.
  const out = redactRoundVotes(round);
  res.json(req.grant ? { ...out, shared: true } : out);
});
