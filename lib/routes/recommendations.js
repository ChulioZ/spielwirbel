 ...
 *          spotlights: [{ key, …the same fields…, target? }],
 *          profileGames, linkedGames, minProfileGames, corpusRows, parties,
 *          dismissed: [{ externalId, title, at }] }
 *
 * `spotlights` (#1228) are up to three one-game tiles re-scored with one term
 * altered — `different`, `complexityUp`/`complexityDown`, `hidden` — computed in
 * the same corpus pass as the list and never repeating a title on it (see "the
 * spotlights" in lib/recommend.js).
 *
...
router.get('/', async (req, res) => {
  ...
  const withUrl = (r) => ({ ...r, url: bgg.gameUrl(r.externalId) });
  res.json({
    ...result,
    recommendations: result.recommendations.map(withUrl),
    spotlights: result.spotlights.map(withUrl),
  });
});
