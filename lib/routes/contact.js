...
// Which one-time prompt opened the form (#1172), from the same file the SPA
// builds the link with — a hand-copied list here would silently drop the source
// of every prompt added after it.
const { isFeedbackSource } = require('../../public/js/feedback-link');

const router = express.Router();

// Backtracking-safe email regex (same as lib/routes/account.js): the domain labels
...
const contactSchema = z.object({
  ...
  // Feedback context (#321), mirroring the feedback route retired then: which SPA
  ...
  // Which prompt produced the message (#1172). Lenient like `locale`: an unknown
  // value is dropped, never a 400 — the message is the point, not the label.
  source: z.preprocess(
    (v) => (isFeedbackSource(v) ? v : undefined),
    z.string().optional(),
  ),
});
...
// own copy in public/js/pages/admin.js — the panel is German-only, #268).
...
router.post('/', async (req, res) => {
  ...
  // Same store shape as the feedback route retired in #321 (context.path/locale),
  ...
  if (body.category === 'feedback') {
    const entry = await repo.createFeedback({
      ...
      context: {
        ...
        // Absent for the top-bar button (unprompted), so older rows and new
        // unprompted ones read the same.
