...
/*
 * Add-game lookup: proxies the external game-database providers (BoardGameGeek
 * alone since #744) so the browser never makes the cross-origin call itself.
 *
 * Mounted under /api/rounds/:rid/lookup (mergeParams for rid). It stayed
 * round-scoped after #744 removed the per-round provider setting: every hop
 * below reads or writes that round's games, so the round is a parameter of the
 * question, not just of the permission that used to be checked here.
 *
 *   GET /api/rounds/:rid/lookup/search?provider=bgg&q=catan
 *       -> { results: [{ providerId, title, thumbnail }] }
 *   GET /api/rounds/:rid/lookup/game?provider=bgg&id=13
...
const { getProvider, providerCoverUrl } = require('../providers');
// The edition a picked cover came from (#742), bounded identically to the single
// add in lib/routes/games.js — one normalizer, so the bulk path cannot accept
// what a single add refuses.
const { normalizeEdition } = require('../edition');
const { validateBody } = require('../validate');
const quota = require('../quota');
// Shared with lib/routes/games.js so both write paths agree on what a valid
// owner is, and on where the per-seat preset lives (#971).
...
// The lazy provider-metadata backfill (#717): the import is the bulk entry
// point, where filling at once is the polite option. It is one of five triggers
// — the full list lives in lib/provider-info.js's header, not restated here.
const { startShelfFill } = require('../provider-info');
...
// Resolve the requested provider against the registry. Answers with the
// provider, or an { status, error } the caller returns as-is:
//   - unknown id  -> 400
//   - round gone  -> 404
//
// A retired provider id (`psstore`, `steam`, `nintendo`, `xbox`) is an unknown
// id like any other and gets the 400 — which is what makes a stale tab fail
// cleanly instead of throwing. The round is still read, because every caller
// needs it to exist before it does anything with it.
...
// The cache-key component for this request's language (#505), kept because the
// provider contract still carries it: a provider that answers in the caller's
// language must not serve a French user a German user's hits for the whole TTL.
//
// It keys on the EFFECTIVE provider locale, never on the raw `?lang=`, so BGG —
// which ignores the locale entirely and returns a constant — stays at a single
// entry instead of fragmenting seven ways for byte-identical results.
//
...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    // A third-party outage, not our bug — but „Spiel suchen" is broken app-wide
    // while it lasts, so the operator has to be able to see it (#1148). NEVER
    // log req.query.q: it is user-typed text (product-event-logging.md).
    logger.warn({ event: 'lookup_provider_failed', provider: provider.id, op: 'search', message: err.message });
...
router.get('/game', async (req, res) => {
  ...
  try {
    ...
    logger.warn({ event: 'lookup_provider_failed', provider: provider.id, op: 'detail', message: err.message });
    res.status(502).json({ error: 'provider_unreachable' });
  }
});
...
// provider capability, like collection(): a provider with no per-edition image
// set answers 400 rather than the route pretending every provider has one.
//
...
// detail page. An OPTIONAL capability like covers() and collection(): a provider
// with no concept of an expansion carries no `expansionDetails`, answers 400,
// and the client falls back to the free-text path rather than to an error.
//
...
//
// `editions` (#742) is the second, and it rides the same reasoning one step
// further: the cover the user picked comes from a specific printing, and the
// wish-list price should quote THAT box rather than whichever edition the reader's
// UI language happens to name. Only `languages` does anything — it is allowlisted
// against BGG_EDITION_LANGS on the way out, so an unmapped printing reaches no
// outbound URL — while `name`/`year` are display text, escaped at render.
// `normalizeEdition` caps every field, so the two entry points cannot differ on
// what they accept.
//
  ...
  editions: z
    .record(
      ...
      z.object({
        ...
        year: z.union([z.number(), z.string().max(16)]).nullable().optional(),
        languages: z.array(z.string().max(80)).max(40).optional(),
      })
    )
    .refine((m) => Object.keys(m).length <= 2000, 'too many editions')
    .optional(),
  // Which members own the imported boxes (#971) — ONE selection for the whole
  // batch, like the `wish` flag: an import is one BGG shelf, and a per-game
  // owner picker over 200 rows is not a thing anyone would fill in. Membership
  // is checked in the handler, against the same seats POST …/games uses.
  ownerIds: z.array(z.string()).max(200).optional(),
});
...
router.post('/import', async (req, res) => {
  ...
  const chosenEditions = body.editions || {};
  ...
  const games = result.items
    ...
    .map((g) => {
      ...
      const has = (m) => Object.prototype.hasOwnProperty.call(m, g.externalId);
      const picked = providerCoverUrl(has(chosen) ? chosen[g.externalId] : null);
      // Only read once the picked cover has survived the gate (see below).
      const edition = picked && has(chosenEditions) ? normalizeEdition(chosenEditions[g.externalId]) : null;
      return {
        title: g.title,
        minPlayers: g.minPlayers,
        maxPlayers: g.maxPlayers,
        ...
        image: picked || providerCoverUrl(g.imageUrl),
        // The edition rides ONLY on the picked cover surviving (#742): if the URL
        // was refused the row shows the collection's own default art, and an
        // edition naming a different printing would then describe a box that is
        // not on screen. Absent otherwise, like every other optional key here.
        ...(edition ? { edition } : {}),
        source: {
  ...
  // Owners are stamped on every created row (#971), and never on a WISHLIST
  // import — the round does not own those boxes, so the sheet hides the picker
  // for that status and the route drops anything sent anyway.
  const ownerIds = wish ? [] : resolveOwnerIds(body.ownerIds, round);
  if (ownerIds === null) return res.status(400).json({ error: 'Unknown member' });
  const withOwners = ownerIds.length ? games.map((g) => ({ ...g, ownerIds })) : games;
  const created = await req.repo.createGames(req.params.rid, withOwners, actorMemberId, limits, wish);
  ...
  // Remember the selection on the importer's own seat (#971), exactly as
  // POST …/games does — the two sheets share one picker, so they must share one
  // memory or opening the other one would silently forget it. Not for a
  // wishlist import, whose sheet never showed the picker.
  if (!wish) await rememberOwnerPreset(req.repo, round, req.params.rid, req.userId, ownerIds);
  ...
  // voter. Import time is the polite place — a paced pass over the shelf, long
  // before anyone draws. Fire-and-forget like the session-start trigger; a
  // failure retries via any of the other triggers (lib/provider-info.js).
  //
  // The whole SHELF, not just `created.created`: an import onto a shelf that
  // already held unfilled games should leave neither half behind, and the pass
  // re-reads the round anyway (#828).
  if (created.created.length) {
    startShelfFill(req.repo, req.params.rid).catch(() => {});
  }
  ...
  if (created.created.length && !wish) {
    ...
    /* ONE feed event for the whole import, carrying the count (#1079). Per-game
       events were rejected for the same reason repo.createGames writes one
       Chronik row: each account's feed keeps only MAX_FEED_EVENTS rows, so a
       large import would evict the importer's own history and fill every
       friend's feed end to end.

       `n === 1` stays a plain `game_added`, and that is not a nicety — one game
       imported IS one game added, and the aggregate phrasing would render "and
       0 more games". The first landed game's title and cover ride along either
       way, so the row keeps its picture and the report button keeps a subject.

       The count is what LANDED, not what was requested: `created.created` has
       already had duplicates skipped. */
    const first = created.created[0];
    const n = created.created.length;
    await emitFeedEvent(req.userId, n > 1
      ? { type: 'games_imported', title: first.title, coverUrl: first.image, count: n }
      : { type: 'game_added', title: first.title, coverUrl: first.image });
  }
