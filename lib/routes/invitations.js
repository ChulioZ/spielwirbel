...
// #137: the assignable role ladder, shared with the frontend's invite sheet so
// the picker and this schema cannot offer different values
// (.claude/rules/shared-constants-across-the-stack.md).
const { ROUND_ROLES } = require('../../public/js/round-roles');
...
const sendSchema = z.object({
  ...
  // #137: the role the grant is created with on accept. Omitted => 'editor', so
  // an older client (or any caller that never heard of roles) invites exactly
  // what it always did. 'owner' is not assignable — see the PATCH …/shares route
  // in lib/routes/rounds.js for why ownership is not a grant.
  role: z.enum(ROUND_ROLES.filter((r) => r !== 'owner')).optional(),
});
...
router.post('/', accounts.requireUser, demo.refuseDemoAccount, async (req, res) => {
  ...
  const item = await repo.addInboxItem(invitee.id, {
  ...
  // Also reach the invitee by e-mail (#618). Not awaited: an invitation exists to
  // reach someone who is not in the app, but the SEND must not hold this response
  // open, and a mail hiccup must not fail an invitation that already exists.
  // notifyInboxItem never rejects.
  notify.notifyInboxItem(invitee.id, item);
...
router.post('/:id/accept', accounts.requireUser, async (req, res) => {
  ...
  // #137: the role the inviter fixed at send time. An invitation created before
  // roles existed carries none, so it falls back to 'editor' — which is what such
  // a grantee could already do, so no live invitation changes meaning.
  await repo.createGrant({
    roundId: inv.roundId, ownerTenantId: inv.ownerTenantId, userId: req.userId, memberId, role: inv.role || 'editor',
  });
  await dismissInvitationInbox(req.userId, inv.id);
