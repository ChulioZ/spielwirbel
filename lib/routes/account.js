...
const storage = require('../storage'); // ending a demo frees its cover objects (#502)
const { avatarUpload } = require('../upload');
const { saveAvatar } = require('../avatar');
const mail = require('../mail');
// The terms-change notice (#521). No rendering happens here — this is the
// current revision, recorded when an account accepts. The resolver that applies
// the legacy fallback is read by lib/me-projection.js, which owns the response.
const { TERMS_REVISION } = require('../legal');
// The one description of what a client may see about an account. Every response
// carrying a `user` object answers through it — /me, and the three endpoints that
// START a session, because the client seats `accountUser` from those and only
// refreshes it on the next cold load (#785).
const { meProjection } = require('../me-projection');
// What a username may be — its shape and the handles reserved as impersonations
// of an official account. Required out of public/js on purpose: the register form
// offers both checks, and a second copy here would drift
// (.claude/rules/shared-constants-across-the-stack.md).
const { USERNAME_RE, isReservedUsername } = require('../../public/js/username-policy');
// The „Was ist neu" entry list (#741), required out of public/js for the same
// reason: the SERVER decides which revision a "seen" stamp records, so a client
// cannot claim to have seen an entry that does not exist — and the list it reads
// has to be the very one the screen renders.
const { newsRevision } = require('../../public/js/news');
// The per-user design registry (#1184/#1186). Required out of public/js for the
// same reason as the two above: the picker offers exactly what this route
// validates against, so a hand-copied server list would refuse designs the UI
// shows — the #145/#420 palette bug, one feature over
// (.claude/rules/shared-constants-across-the-stack.md).
const { FACE_DESIGN, DESIGN_CHOOSER_REVISION, isSelectableDesign } = require('../../public/js/designs');
// The switch-back stamp (#1201), decided in the module the operator's tile reads.
const { accountDesign, answeredChooser, switchBackPatch } = require('../account-design');
// The origin mailed links are built on. Shared with lib/notify.js since #618 —
// see the comment there for why it is not re-derived per sender.
const { appBaseUrl: baseUrl } = require('../canonical');
...
// The app-wide public handle (#320). The pattern and its bounds live in
// public/js/username-policy.js, which the register form checks against too — see
// the require above; the reasoning for the narrow charset is there.
const usernameSchema = z.string().regex(USERNAME_RE);
...
// Which designs may be STORED here. Resolved per request rather than hoisted to
// a module constant: NODE_ENV is read at call time in the tests, which run the
// same app object under both settings.
const designSelectable = (id) =>
  isSelectableDesign(id, { production: process.env.NODE_ENV === 'production' });

// Mail failures must never fail the account flow (registration/reset still
// succeed); they are logged for the operator instead.
//
// The recipient address is deliberately NOT logged. It is personal data, and
// the published policy states that request contents are not protocolled and
// that usage events never carry contents (lib/legal.js §3) — an address is
// neither pseudonymous nor absent. The error message alone is what the operator
// needs to see that mail is failing; which address it was does not change the
// diagnosis, and this path is reachable in production on every transport error
// AND on the MAIL_DAILY_MAX breaker. Same shape as lib/notify.js's
// `inbox_notification_failed`. Pinned by test/account-mail-privacy.test.js.
function sendSafe(msg) {
  return mail.send(msg).catch((e) =>
    logger.warn({ event: 'account_mail_failed', message: e.message }));
}
...
router.post('/register', async (req, res) => {
  ...
  // A separate code from `invalid_username`, and from the 409 `username_taken`:
  // the handle is well-formed and free, and the refusal is a policy the person
  // typing it has no way to guess — so the client owes them a message that says
  // which of the three it was. It reveals nothing (the list is static, not
  // account data), so the anti-enumeration rule in the header does not bind here.
  if (isReservedUsername(username)) return res.status(400).json({ error: 'username_reserved' });
  ...
  const user = await repo.createUser({
    ...
    // Always present, so the two backends' jsonb shapes agree from the first
    // row (.claude/rules/postgres-backend.md). Accounts predating #1076 read
    // `undefined`, which every consumer treats as "none" — see meProjection.
    pendingEmail: null,
    reset: null,
    refreshTokens: [],
    ...
    bggUsername: null,
    // The profile picture (#841) — a '/uploads/<key>.webp' path once set.
    // Present-but-null from the start for the same absent-key parity reason as
    // bggUsername above; accounts predating it read the same null out of
    // meProjection rather than being migrated.
    avatar: null,
    // Whether actionable inbox items are also e-mailed (#618). Both default ON:
    // an invitation exists to reach someone who is not in the app, so opt-out is
    // the only default under which the feature does its job. Written here for
    // absent-key parity; accounts predating #618 read the same default from
    // meProjection's `!== false` rather than being migrated.
    notifyRoundInvitations: true,
    notifyFriendRequests: true,
    // #1089. Written true at registration for the same reason the two above are:
    // the stored shape stays explicit, while meProjection's `!== false` is what
    // covers the accounts that predate the key.
    statsVisible: true,
    // Whether the results screen offers the BG Stats push (#485). OFF by
    // default, the opposite of the two above and deliberately so: BG Stats'
    // own integration guidance is to let the user enable the button rather
    // than show a link that dead-ends for everyone who does not use the app,
    // and a website cannot detect whether it is installed.
    bgStats: false,
    // When this account was last sent one, for the per-recipient throttle in
    // lib/notify.js. Null, never a timestamp: "never notified" must read as
    // long ago, or the first item after registration would be suppressed.
    notifiedAt: null,
    ...
    // Which „Was ist neu" revision this account has seen (#741). Stamped at
    // creation so a NEW account starts caught up — without this it would be
    // dotted on its first login about entries describing features it has never
    // known the app to be without. Null while the list is empty, which is the
    // same value an account predating the field resolves to.
    lastSeenNewsRevision: newsRevision(),
    // The per-user design (#1186). Written explicitly at creation for the same
    // absent-key parity reason as the fields above — the two creation sites
    // (here and lib/demo.js) must produce the same key set or the backends
    // diverge (.claude/rules/postgres-backend.md). The value is the face (Der
    // Tisch since #1202), which is also what an unanswered account resolves to.
    design: FACE_DESIGN,
    // Null, not the current revision: a new account has NOT seen the chooser,
    // and stamping it here would mean the one screen that tells people designs
    // exist never reaches anyone who registered after the flip.
    designChooserSeen: null,
    // Whether this account ever went BACK to Klassisch from another design
    // (#1201) — false, not absent, for the same key-set parity. Read as
    // `=== true` by the operator's tile, so an account predating the field
    // counts as never having switched, which is what it did.
    designSwitchedBack: false,
  });
...
// The implementation moved to lib/accounts.js in #418 so the passkey router
// mints sessions through the identical code; this alias keeps the five call
// sites below reading as they did.
const issueTokens = accounts.issueSession;
...
router.post('/login', async (req, res) => {
  ...
  // The FULL projection, not a hand-built subset, because the client seats
  // `accountUser` straight from this response and only refreshes it from /me on
  // the next cold load. #521 added the terms fields here for that reason and
  // #741's news stamp was then forgotten — so the answer is to send what /me
  // sends rather than to remember a fourth field (#785).
  res.json({ ok: true, ...tokens, user: meProjection(user) });
});
...
router.post('/demo', async (req, res) => {
  ...
  // The same projection the login response answers, so `accountUser` has a
  // uniform shape whichever way the session started (#785). `demo` and
  // `demoExpiresAt` are DERIVED from the stored record rather than restated
  // here — createDemoAccount writes both — so the banner cannot end up reading a
  // hand-set flag that disagrees with what /me will report on the next load.
  res.json({ ok: true, ...tokens, user: meProjection(user) });
});
...
/* ------------------------- changing the address (#1076) --------------------- */

/* The address is the account's login identifier and its only recovery channel,
   and it was immutable: a person who lost or retired a mailbox could only ask
   the operator, who had no tool for it either.

   CONFIRM-THEN-SWAP, never swap-then-reverify. The address is replaced only when
   the link mailed to the NEW one is opened, so a typo can never lock anyone out
   and `emailVerified` stays true throughout. A pending change is REPLACED by a
   newer request rather than accumulating, and expires with VERIFY_TTL_MS. */
const pendingEmailRecord = (email, raw) => ({
  email,
  tokenHash: accounts.hashToken(raw),
  expiresAt: iso(Date.now() + accounts.VERIFY_TTL_MS),
  sentAt: iso(Date.now()),
...
// `/e`, a third two-character path, for the reason `/v` and `/r` are terse: the
// whole URL has to fit on one quoted-printable line
// (.claude/rules/mailed-links-must-fit-one-qp-line.md). test/account.test.js
// pins the length — measure it, don't eyeball it.
function mailEmailChange(to, uid, secret) {
  const link = `${baseUrl()}/e?t=${accounts.mintLinkToken(accounts.EMAIL_TOKEN_VERSION, uid, secret)}`;
  return sendSafe({
    to,
    subject: 'Spielwirbel: Neue E-Mail-Adresse best\u00e4tigen / Confirm your new e-mail address',
    text: `Hallo!\n\nDu m\u00f6chtest die E-Mail-Adresse deines Spielwirbel-Kontos auf diese Adresse \u00e4ndern. Bitte best\u00e4tige das, indem du diesen Link \u00f6ffnest (g\u00fcltig 24 Stunden):\n${link}\n\nBis dahin bleibt deine bisherige Adresse aktiv. Falls du das nicht warst, ignoriere diese E-Mail einfach \u2014 es \u00e4ndert sich dann nichts.\n\n---\n\nHi!\n\nYou asked to change your Spielwirbel account's e-mail address to this one. Please confirm by opening this link (valid for 24 hours):\n${link}\n\nUntil then your previous address stays active. If this wasn't you, simply ignore this e-mail \u2014 nothing will change.`,
  });
}

router.post('/change-email', accounts.requireUser, loadNonDemoAccount, async (req, res) => {
  ...
  if (email === user.email) return res.status(400).json({ error: 'same_email' });

  // Same reasoning as change-password: a valid access token may be sitting on an
  // unattended device, and this endpoint moves the account's recovery channel.
  const identity = (user.identities || []).find((i) => i.type === 'password');
  if (!identity || !(await accounts.verifyPassword(identity.hash, String(currentPassword || '')))) {
    return res.status(401).json({ error: 'invalid_credentials' });
  }

  /* Per-ACCOUNT cooldown. This endpoint mails an address the CALLER NAMES, so it
     is exactly the class .claude/rules/mail-sending-endpoints-need-a-per-account-cooldown.md
     covers even though it is authenticated: the IP limiter in lib/app.js would
     let a rotating-address caller flood a stranger's inbox and burn the operator
     mailbox's daily quota with it. */
  if (user.pendingEmail && mailThrottled(user.pendingEmail)) {
    return res.json({ ok: true, pendingEmail: user.pendingEmail.email });
  }

  /* An address belonging to somebody else answers EXACTLY as a free one does,
     stores nothing and sends nothing — the anti-enumeration invariant in
     .claude/rules/user-accounts.md, which binds for an authenticated caller too:
     with DEMO_ENABLED on, anyone is an authenticated caller in one request. The
     person notices the confirmation never arrives, and the pending panel shows
     the address they typed, so a typo is still visible. */
  if (await repo.getUserByEmail(email)) return res.json({ ok: true, pendingEmail: email });

  const raw = accounts.newLinkSecret();
  await repo.updateUser(user.id, { pendingEmail: pendingEmailRecord(email, raw) });
  await mailEmailChange(email, user.id, raw);
  res.json({ ok: true, pendingEmail: email });
});

// Cancel. The only way to stop a link working after a typo — short of waiting
// out the 24 hours — and it costs one field.
router.delete('/change-email', accounts.requireUser, async (req, res) => {
  await repo.updateUser(req.userId, { pendingEmail: null });
  res.json({ ok: true });
});

/* The counterpart of verifyEmail(). UNAUTHENTICATED on purpose: the link is
   opened from a mail client, where requiring a login would only bounce the
   person — and the mailbox IS the proof, which is the whole point of confirming
   there rather than here. */
router.post('/confirm-email', async (req, res) => {
  const cred = accounts.parseLinkToken(accounts.EMAIL_TOKEN_VERSION, (req.body || {}).token);
  if (!cred) return res.status(400).json({ error: 'invalid_token' });
  const user = await repo.getUserById(cred.userId);
  const p = user && user.pendingEmail;
  if (!p || Date.parse(p.expiresAt) <= Date.now()) return res.status(400).json({ error: 'invalid_token' });
  if (!accounts.safeEqual(p.tokenHash, cred.hash)) return res.status(400).json({ error: 'invalid_token' });

  /* Re-checked HERE, not only at request time: the address may have been
     registered by somebody else in between. Postgres's users_email_idx is the
     real backstop — updateUser does not map 23505 — so the write is caught and
     answered the same way, and the dead record is cleared either way so the
     screen stops offering a link that can never work. */
  if (await repo.getUserByEmail(p.email)) {
    await repo.updateUser(user.id, { pendingEmail: null });
    return res.status(400).json({ error: 'invalid_token' });
  }
  const previous = user.email;
  try {
    await repo.updateUser(user.id, { email: p.email, pendingEmail: null });
  } catch (err) {
    // The user is told `invalid_token` (above), which is right for them and
    // wrong for us: a failing write is a database fault dressed as a user
    // error, and it used to be invisible (#1148). No address, and `code`
    // rather than `message` — a 23505 from this index would otherwise be the
    // one log line carrying an e-mail (unique-violation-reports-one-constraint).
    logger.error({ event: 'email_change_write_failed', code: err.code || err.name });
    await repo.updateUser(user.id, { pendingEmail: null });
    return res.status(400).json({ error: 'invalid_token' });
  }

  /* The standard defence against a silent takeover, pointed at the address that
     is LOSING the account. Deliberately NOT at \u201ePasswort vergessen?\u201c like the
     password-changed mail: a reset link goes to the account's address, which is
     no longer this one, so the only channel left is the operator. */
  await sendSafe({
    to: previous,
    subject: 'Spielwirbel: E-Mail-Adresse ge\u00e4ndert / E-mail address changed',
    text: `Hallo!\n\nDie E-Mail-Adresse deines Spielwirbel-Kontos wurde soeben auf ${p.email} ge\u00e4ndert. Diese Adresse hier kann sich nicht mehr anmelden.\n\nFalls du das nicht warst, melde dich bitte sofort \u00fcber das Kontaktformular auf spielwirbel.app \u2014 \u201ePasswort vergessen?\u201c hilft dir nicht mehr weiter, weil der Link an die neue Adresse ginge.\n\n---\n\nHi!\n\nThe e-mail address of your Spielwirbel account was just changed to ${p.email}. This address can no longer sign in.\n\nIf this wasn't you, please contact us immediately via the contact form on spielwirbel.app \u2014 "Forgot password?" will not help any more, because the link would go to the new address.`,
  });
  res.json({ ok: true });
});
...
// Edit the settable parts of the account: the BGG handle (#481) and the two
// inbox-mail opt-outs (#618).
...
router.patch('/me', accounts.requireUser, async (req, res) => {
  ...
  // The two inbox-mail opt-outs (#618). Strictly booleans: the stored value is
  // read back as `!== false`, so anything else — the string 'false', 0, null —
  // would be stored happily and then read as ON forever, i.e. a toggle the user
  // switched off that keeps mailing them. Refusing is the only honest answer.
  for (const field of ['notifyRoundInvitations', 'notifyFriendRequests']) {
    if (body[field] === undefined) continue;
    if (typeof body[field] !== 'boolean') return res.status(400).json({ error: 'invalid_notify_pref' });
    patch[field] = body[field];
  }

  // Whether friends see this account's statistics (#1089). Strictly a boolean for
  // exactly the reason the two above are — it is read back as `!== false`, so a
  // stored string 'false' would be accepted and then read as VISIBLE forever,
  // i.e. a switch the user turned off that keeps showing their record to friends.
  // Its own error code: this is not a mail preference, and a client mapping the
  // refusal to a message would otherwise have to guess which field it was about.
  if (body.statsVisible !== undefined) {
    if (typeof body.statsVisible !== 'boolean') return res.status(400).json({ error: 'invalid_stats_pref' });
    patch.statsVisible = body.statsVisible;
  }

  // The BG Stats push opt-in (#485). Strictly a boolean for the same reason the
  // two above are — the projection reads it back as `=== true`, so a stored
  // string 'true' would be honoured on the way in and read as OFF forever, i.e.
  // a toggle the user switched on that never appears. Its own error code rather
  // than the notify one: this is not a mail preference, and a client mapping the
  // refusal to a message would otherwise have to guess which field it was about.
  if (body.bgStats !== undefined) {
    if (typeof body.bgStats !== 'boolean') return res.status(400).json({ error: 'invalid_bgstats_pref' });
    patch.bgStats = body.bgStats;
  }

  /* The per-user design (#1186). An ALLOWLIST test against the very list the
     picker rendered, so an id this build has never heard of — or one that is
     registered but not `enabled` in production — is refused rather than put on
     <html data-design>. Its own error code, like the three above.

     A DEMO may set it (#427), unlike the avatar and the password: the chooser
     has to work for a visitor trying the app, and a design is a rendering
     preference that costs nothing and discloses nothing. Said explicitly
     because the neighbouring routes refuse a demo and the asymmetry would
     otherwise read as an oversight (.claude/rules/guest-demo-accounts.md). */
  if (body.design !== undefined) {
    if (!designSelectable(body.design)) return res.status(400).json({ error: 'invalid_design' });
    patch.design = body.design;
    // Moving back to Klassisch stamps designSwitchedBack (#1201), judged
    // against what the account was WEARING — lib/account-design.js says why.
    // One extra read, only on a design change.
    const before = await repo.getUserById(req.userId);
    Object.assign(patch, switchBackPatch(before, body.design));
    // A pick on Konto IS an answer to the chooser (#1202). Without the stamp an
    // account that had never answered it would go on resolving to the face —
    // accountDesign() only honours a stored design once the chooser has been
    // answered — so the pick would be saved and not worn. Only when unset: a
    // pick never re-arms or silences a LATER run of the chooser.
    if (before && !answeredChooser(before)) patch.designChooserSeen = DESIGN_CHOOSER_REVISION;
  }
...
/*
 * An account's profile picture is a '/uploads/<key>.webp' path on the user row —
 * the same storage seam and the same auth-gated /uploads mount a game cover uses,
 * so nothing new has to be served and no second upload path exists.
 *
 * STATED DELIBERATELY, because it is a real disclosure decision rather than an
 * omission: /uploads sits behind uploadGate (lib/app.js), which admits ANY
 * authenticated account — it is not tenant-private. That is the right level for a
 * profile picture, since the profile it belongs to already requires a login. But
 * with DEMO_ENABLED on, "any authenticated account" includes anyone who mints a
 * demo in a single request. This is exactly the exposure every game cover has
 * had since #129, and it is why the avatar is re-encoded rather than stored as
 * uploaded (lib/avatar.js).
 */

// Cap on one batch resolution below. Bounds the query rather than the caller: a
// round with more members than this simply resolves in two requests.
const AVATAR_BATCH_MAX = 200;

// Read the account once and refuse a demo (#427) BEFORE multer buffers anything
// and long before the decoder runs — a demo has no friends and no profile
// audience, so a picture buys it nothing, while allowing it would turn an
// unregistered visitor into an anonymous image host whose output every logged-in
// account can fetch (.claude/rules/guest-demo-accounts.md). The account screen
// hides the control in the demo branch; this is the enforcement behind it.
async function loadNonDemoAccount(req, res, next) {
  const user = await repo.getUserById(req.userId);
  if (!user) return res.status(401).json({ error: 'invalid_token' });
  if (user.demo === true) return res.status(403).json({ error: 'demo_account' });
  req.account = user;
...
// multer's cap fires before a single byte is decoded, which is the entire reason
// the avatar instance carries a smaller one than the cover instance. Reported
// under its own code so the toast can state the limit instead of "something went
// wrong" — the one upload failure a user can actually act on.
const receiveAvatar = (req, res, next) => avatarUpload.single('avatar')(req, res, (err) => {
  if (!err) return next();
  if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: 'avatar_too_large' });
  return next(err);
});

router.post('/me/avatar', accounts.requireUser, loadNonDemoAccount, receiveAvatar, async (req, res) => {
  // Sniff + re-encode + persist. A null covers both "did not sniff as an image"
  // and "did not decode as one" — the same answer to the uploader — and in
  // neither case has anything been written.
  const stored = await saveAvatar(req.file);
  if (!stored) return res.status(400).json({ error: 'invalid_image' });

  const previous = req.account.avatar || null;
  const updated = await repo.updateUser(req.userId, { avatar: stored });
  if (!updated) {
    // The account was erased between the read above and this write. Free the
    // object we just wrote rather than orphaning it — nothing references it.
    await storage.remove(stored);
    return res.status(401).json({ error: 'invalid_token' });
  }
  // Reference first, bytes second — the ordering takedownImage and eraseAccount
  // use, so a failed delete can never leave the row pointing at a missing
  // object. Replacing a picture frees the one it replaced (#280); the guard is
  // for the degenerate case of the storage backend handing back the same key.
  if (previous && previous !== stored) await storage.remove(previous);
  res.json(meProjection(updated));
});

// Deliberately NOT demo-gated, unlike the upload: a demo can hold no picture, so
// this is a no-op for one, and refusing an idempotent removal would only make
// the account screen have to special-case a button that already does nothing.
router.delete('/me/avatar', accounts.requireUser, async (req, res) => {
  const user = await repo.getUserById(req.userId);
  if (!user) return res.status(401).json({ error: 'invalid_token' });
  const previous = user.avatar || null;
  const updated = await repo.updateUser(req.userId, { avatar: null });
  if (!updated) return res.status(401).json({ error: 'invalid_token' });
  if (previous) await storage.remove(previous);
  res.json(meProjection(updated));
});

// Resolve account ids the caller already holds to their picture paths, so a
// round screen can render a linked seat's photo WITHOUT the round read growing a
// piece of the users table (#841 §3). Denormalising the path onto the seat was
// rejected for two reasons that outlive the perf argument: it goes stale the
// moment the account changes its picture, and it copies a photo of a person into
// another tenant's stored data, which Art. 17 erasure would then have to sweep
// tenants the account never belonged to.
//
// Authorization is deliberately no narrower than /uploads already is: any
// authenticated account may resolve any id it can NAME, and ids are opaque, so
// this discloses nothing the caller could not already fetch by holding the path.
router.get('/avatars', accounts.requireUser, async (req, res) => {
  const ids = String(req.query.ids || '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean);
  if (!ids.length) return res.json({ avatars: {} });
  if (ids.length > AVATAR_BATCH_MAX) return res.status(400).json({ error: 'too_many_ids' });
  res.json({ avatars: await repo.getUserAvatars(ids) });
});
...
// Acknowledge the current „Was ist neu" revision (#741) — what OPENING the /neu
// screen calls. Visiting the page IS the acknowledgement, exactly like the terms
// banner's dismiss button; there is no separate control to click.
//
// It takes NO body, for the same reason accept-terms does not: the server reads
// `newsRevision()` from the shared list, so a client cannot stamp itself past an
// entry it never received, nor pin itself to an old revision to keep the dot lit.
//
// Note the one case this does not cover: a browser still running a CACHE-stale
// bundle (the shell is served cache-first) holds an older list, so opening /neu
// there stamps it to the server's newest revision including an entry its own
// bundle cannot render. That is the same trade accept-terms makes, and it lasts
// one navigation — a news entry ships with a release, which bumps the shell.
//
// Nothing gates on this value; the dot is a nudge, not a wall.
router.post('/news-seen', accounts.requireUser, async (req, res) => {
  const user = await repo.updateUser(req.userId, { lastSeenNewsRevision: newsRevision() });
  if (!user) return res.status(401).json({ error: 'invalid_token' });
  res.json(meProjection(user));
});

/* Answering the first-start design chooser (#1186).

   ONE request for both halves — the pick and the seen-stamp — because two would
   leave a window in which the chooser has been answered and not recorded: a
   reload there shows it again, on top of the design it just applied.

   The REVISION is stamped server-side and never taken from the body, exactly as
   /news-seen stamps newsRevision(). A client choosing its own value could claim
   to have seen a run that does not exist and silence the chooser permanently —
   including the next one, which is the whole reason this is a revision rather
   than a boolean.

   `design` is optional: „Später entscheiden", Escape and a history Back all send
   nothing and mark it seen alone, which is what makes the promise in the copy
   ("it appears once") true. */
router.post('/design-chooser-seen', accounts.requireUser, async (req, res) => {
  const body = req.body || {};
  const patch = { designChooserSeen: DESIGN_CHOOSER_REVISION };
  const before = await repo.getUserById(req.userId);
  if (body.design !== undefined) {
    if (!designSelectable(body.design)) return res.status(400).json({ error: 'invalid_design' });
    patch.design = body.design;
    // The chooser is the MAIN way back after the flip (#1202 moves everyone to
    // Tisch and this sheet offers „Wie bisher"), so it stamps the switch-back
    // exactly like PATCH /me does (#1201).
    Object.assign(patch, switchBackPatch(before, body.design));
  } else if (before) {
    // „Später entscheiden" keeps what the account is WEARING, and must write it
    // down (#1202): once the chooser is answered, accountDesign() honours the
    // stored design — which for every pre-flip account is a Klassisch it never
    // chose. Declining would otherwise silently undo the flip for that account.
    patch.design = accountDesign(before);
  }
  const user = await repo.updateUser(req.userId, patch);
  if (!user) return res.status(401).json({ error: 'invalid_token' });
  res.json(meProjection(user));
});
