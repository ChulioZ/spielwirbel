...
const { switchBackPatch } = require('../account-design');
...
router.post('/register', async (req, res) => {
  ...
  const user = await repo.createUser({
    ...
    // diverge (.claude/rules/postgres-backend.md). The value is the face, so a
    // new account looks exactly like every existing one until it chooses.
    design: FACE_DESIGN,
...
router.patch('/me', accounts.requireUser, async (req, res) => {
  ...
  if (body.design !== undefined) {
    ...
    Object.assign(patch, switchBackPatch(await repo.getUserById(req.userId), body.design));
  }
