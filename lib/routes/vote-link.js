  ...
  const people = [
    ...all
      ...
      .map(({ m, idx }) => ({
        ...
        guest: false,
        color: colorOf(m, idx),
        // Whether this seat belongs to an account — a BOOLEAN, never the userId
        // itself. #1169 needs to know only "does this person already have the
        // app?", and the header's promise that no account identifier travels is
        // worth more than the convenience of sending one. A raw userId here
        // would hand a stranger holding the link a stable cross-session handle
        // for a person they can already see by name; the boolean tells them
        // nothing they could not infer from the app note's absence.
    ...
    // A guest is never linked by construction: a guest seat exists precisely
    // because that person has no member row to attach an account to.
    ...(session.guests || []).map((g) => ({
      id: g.id, name: g.name, guest: true, color: null, linked: false,
    })),
