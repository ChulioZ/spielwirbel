...
async function openBallot(token) {
  ...
  // The state gate, matching the in-app write's own refusals. A hot-seat session
  // can never have a link (the mint refuses one), but check it here too rather
  // than trusting that: this is the file that must hold on its own.
  if (!session.deviceVoting || session.done || session.cancelled) return null;
