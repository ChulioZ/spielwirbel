...
router.get('/:token', async (req, res) => {
  ...
  const games = (session.gameIds || [])
    ...
    .filter(Boolean)
    // weight/description (#717) and the standard metadata (#724) join the
    // projection so a link voter gets the same "what is this game like?" facts
    // as the hot-seat card. Still built field by field — the whitelist stays the
    // shape that stops a future session field leaking by being forgotten.
    //
