...
// The lazy weight/description backfill (#717): the detail-open trigger lives in
// this router (GET …/:gid/provider-info), the session-start one in sessions.js.
...
// capability; the storefronts have no expansion concept.
//
...
// what the provider says about it. That is what keeps `description` genuinely
// provider-sourced — the reason it joins none of redactText's kinds — since no
// hand-rolled request can write its own text into it.
//
...
router.post('/', upload.single('image'), async (req, res) => {
  ...
  // Weight + description (#717), for wishes too — their detail page shows both.
...
router.patch('/:gid', upload.single('image'), async (req, res) => {
  ...
  // The link-provider sheet's weight/description chips (#717). The request
  // carries BOOLEANS only — the values are resolved server-side from the
  // provider (same trust rule as resolveProviderInfo on create), so a client
  // chooses whether to take the info but never what it says. Tolerant: a failed
  // hop just links without the info and the lazy backfill fills it later.
  //
  // #724's fields (playtime/age/categories/mechanics/rating) get NO chip and are
  // deliberately not written here: the chips exist because weight and
  // description are the two the sheet previews and the user opts into. The rest
  // arrive through the lazy backfill instead — which this stamp defers by up to
  // one PROVIDER_INFO_TTL_MS, the same self-healing delay linking with a chip
  // has always had.
  const wantWeight = b.applyWeight === true || b.applyWeight === 'true';
  const wantDescription = b.applyDescription === true || b.applyDescription === 'true';
  if (source && (wantWeight || wantDescription)) {
    const info = await resolveProviderInfo(source);
    if (info) {
      if (wantWeight && Number.isFinite(info.weight)) patch.weight = info.weight;
      if (wantDescription && info.description) patch.description = info.description;
...
// for a provider-linked game whose weight/description are missing, and the
...
router.get('/:gid/provider-info', async (req, res) => {
  ...
  res.json({
    ...
    description: game.description == null ? null : game.description,
