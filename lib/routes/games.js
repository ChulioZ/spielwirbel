...
const { needsProviderInfo, backfillProviderInfo, startShelfFill } = require('../provider-info');
...
const { assignProviderInfo, UNCHIPPED_PROVIDER_INFO_FIELDS } = require('../../public/js/provider-info-fields');
...
// Owner validation + the per-seat preset memory (#971), shared with the BGG
// import route so the two write paths cannot disagree about what a valid owner is.
const { resolveOwnerIds, rememberOwnerPreset } = require('../game-owners');
...
const createGameSchema = z
  .object({
    ...
    // Which members of the round own the box (#971). Multipart repeats the
    // field exactly like tagIds above, so one value arrives as a bare string;
    // membership in the round's seats is checked in the handler.
...
// for the same absent-≠-empty reason the retired per-round `providers` setting
// had (removed with the storefront providers in #744). An EMPTY array is a client error,
...
// A sanity ceiling on one request's id list — deliberately NOT the quota, which
// is enforced when a game is ADDED. It sits well clear of MAX_GAMES_PER_ROUND's
// default (1000, lib/quota.js) so selecting a full shelf always fits; an
// instance raising that cap above this would have to raise this with it. Past
// the ceiling nothing is at risk anyway — an id the round does not hold is
// refused as 'unknown_game' before any write — so this only stops one request
// carrying an absurd array.
const BULK_GAME_IDS_MAX = 5000;

// Bulk shelf-tidying body (#832): the games a confirmed selection named. Unlike
// moveGamesSchema's, `gameIds` is REQUIRED — there is no "the whole shelf"
// shorthand here, because a missing field must never be read as "retire or
// delete everything".
const bulkGamesSchema = z.object({
  gameIds: z.array(z.string().min(1)).min(1, 'gameIds must not be empty').max(BULK_GAME_IDS_MAX),
});

// Bulk owner assignment (#972). A separate schema rather than an optional field
// on the one above, so `bulk-retire`/`bulk-delete` cannot silently accept an
// `ownerIds` they would then ignore. `ownerIds` is REQUIRED and may be empty:
// unlike `gameIds`, an empty list here is a meaningful instruction ("nobody owns
// these") rather than a caller bug, and it is the only way to clear.
const bulkOwnersSchema = bulkGamesSchema.extend({
  ownerIds: z.array(z.string().min(1)).max(BULK_GAME_IDS_MAX),
});

// Bulk tag delta (#1000). Two lists rather than one `tagIds`, because the action
// ADDS and REMOVES instead of replacing — see the repo mutator for why. Both may
// be empty, and both being empty is a caller bug rather than a meaningful
// instruction (unlike `bulkOwnersSchema`'s empty `ownerIds`, which is the clear),
// so the refine below refuses it rather than reporting a successful no-op.
const bulkTagsSchema = bulkGamesSchema.extend({
  addTagIds: z.array(z.string().min(1)).max(BULK_GAME_IDS_MAX).default([]),
  removeTagIds: z.array(z.string().min(1)).max(BULK_GAME_IDS_MAX).default([]),
}).refine((b) => b.addTagIds.length + b.removeTagIds.length > 0,
...
// The three shelf-state flips take one optional boolean each, defaulting to
// "set it": `{}` and `{ retired: true }` both retire, `{ retired: false }`
// restores. They read the flag off req.body until the 2026-09-06 audit (M-003).
const flagSchema = (name) => z.object({ [name]: z.boolean().optional() });
const retireSchema = flagSchema('retired');
const completeSchema = flagSchema('completed');
const wishSchema = flagSchema('wish');
...
// An oversize paste gets its own 413 code so the client can say what went
// wrong, instead of the generic multer error surfacing as an unexplained
// failure. Same shape as receiveAvatar in lib/routes/account.js (#841); the
// cap itself is COVER_MAX_BYTES, shared with the two paste sites that offer it.
const receiveCover = (req, res, next) => upload.single('image')(req, res, (err) => {
  if (!err) return next();
  if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: 'cover_too_large' });
  return next(err);
});

router.post('/', receiveCover, async (req, res) => {
  ...
  // Owners must be seats of this round (deduped; unknown ids -> 400, #971).
  // A WISH never carries them: the round does not own the game, so "who owns the
  // box" has no answer yet — the picker is hidden on the wish variants of both
  // sheets, and dropping it here is what stops a hand-rolled request storing one.
  const ownerIds = body.wish ? [] : resolveOwnerIds(body.ownerIds, round);
  if (ownerIds === null) return res.status(400).json({ error: 'Unknown member' });
  ...
  // Remember the selection on the adder's own seat (#971) — but not for a wish,
  // whose sheet never showed the picker, or every wish added would silently
  // reset the preset to empty for the next real add.
  if (!body.wish) await rememberOwnerPreset(req.repo, round, req.params.rid, req.userId, ownerIds);
...
// Copy games onto ANOTHER round's shelf (#916) — the sibling of /move-to, and
// deliberately its own route rather than a `mode` flag on that one: the reparent's
// session scrub and vote deletion are the risky half and a copy needs none of it,
// and two entries in lib/round-access.js's table stay separately gateable.
//
// Same request schema and the same refusal contract as the move, so the sheet
// speaks one language for both halves.
router.post('/copy-to', async (req, res) => {
  const round = await req.repo.getRoundMeta(req.params.rid);
  if (!round) return res.status(404).json({ error: 'Round not found' });
  ...
  // Passed down rather than checked here, for /move-to's reason: the number of
  // tags the copy would create is only known once the data layer has built the
  // remap, and the refusal has to be atomic with the copy itself.
  const limits = quota.enforced()
    ? { maxGames: quota.gamesPerRound(), maxTags: quota.tagsPerRound() }
    : null;
  ...
  const result = await req.repo.copyGames(req.params.rid, body.targetRoundId, limits, gameIds);
  if (result === null) return res.status(404).json({ error: 'Round not found' });
  if (result === 'same_round')
    return res.status(400).json({ error: 'Target round must be a different round' });
  if (result === 'unknown_game') return res.status(400).json({ error: 'Unknown game' });
...
router.patch('/:gid', receiveCover, async (req, res) => {
  const round = await req.repo.getRoundMeta(req.params.rid);
  if (!round) return res.status(404).json({ error: 'Round not found' });
  ...
  // Replace the game's owner assignment (#971). Sent as JSON like tagIds above,
  // and cleared the same way: `[]` (or null) stores an empty list, which every
  // reader treats exactly as an absent key. A wish is refused rather than
  // silently ignored — the detail page shows no owner row for one, so a request
  // carrying owners for a wish is a stale or hand-rolled client.
  if (b.ownerIds !== undefined) {
    if (game.wish) return res.status(400).json({ error: 'A wish has no owners' });
    const ownerIds = resolveOwnerIds(b.ownerIds, round);
    if (ownerIds === null) return res.status(400).json({ error: 'Unknown member' });
    patch.ownerIds = ownerIds;
  }
...
// all.
//
// It ANSWERS after one upstream request and then keeps filling (#828). The
// synchronous batch is what makes the controls appear on this open rather than
// the next; the paced pass behind the response is what finishes the shelf, since
// BGG carries at most 20 ids per request and one batch per screen open would
// need eight visits to fill 150 games. Pacing keeps that within what the
// provider's terms ask for (.claude/rules/add-game-lookup-provider.md).
//
...
router.post('/provider-info', async (req, res) => {
  ...
  // Everything the first batch could not reach, paced, behind this response. It
  // re-reads the round itself, so the games just filled above are already out of
  // its way; a pass already running for this round makes it a no-op.
  if (eligible.length > 1) startShelfFill(req.repo, req.params.rid).catch(() => {});
  res.json({
...
router.post('/:gid/retire', async (req, res) => {
  ...
  const body = validateBody(retireSchema, req, res);
  if (!body) return;
  const retired = body.retired !== false; // default: true
...
router.post('/:gid/complete', async (req, res) => {
  ...
  const body = validateBody(completeSchema, req, res);
  if (!body) return;
...
router.post('/:gid/wish', async (req, res) => {
  ...
  const body = validateBody(wishSchema, req, res);
  if (!body) return;
  const wish = body.wish !== false; // default: true
  // The prior flag decides whether this call is an ACQUISITION or a repeat of one
  // (#856) — the route is written to be idempotent, so a client may send the same
  // `{ wish: false }` again and must not announce a second acquisition to friends.
  const before = await req.repo.getGame(req.params.rid, req.params.gid);
  const game = await req.repo.wishGame(req.params.rid, req.params.gid, wish, actorSeat(round, req.userId));
  if (!game) return res.status(404).json({ error: 'Game not found' });
  if (!wish) {
    trackEvent('game_added', { tenantId: req.tenantId });
    // Feed on the transition only. Not gating trackEvent with it is deliberate and
    // is issue #856's stated scope: whether the product counter should count
    // requests or acquisitions is its own question, and the two must not be
    // changed together by accident.
    if (before && before.wish) {
      await emitFeedEvent(req.userId, { type: 'game_added', title: game.title, coverUrl: game.image });
    }
  }
  res.json(game);
...
// Retire several games in one action (#832). The shelf can be FILLED in one
// action — the BGG collection import (#481) — so emptying it had no business
// being 200 requests; the forum report that prompted this was someone
// mid-evaluation who had just imported a collection and found no way back.
//
// Costs `round.write`, the same as POST /:gid/retire: retiring is reversible,
// and doing it to twenty games at once is not a different kind of act.
router.post('/bulk-retire', async (req, res) => {
  const round = await req.repo.getRoundMeta(req.params.rid);
  if (!round) return res.status(404).json({ error: 'Round not found' });

  const body = validateBody(bulkGamesSchema, req, res);
  if (!body) return;

  // Deduped here so both backends see the same list and count, exactly as
  // /move-to does. Membership is checked inside the mutation's own transaction
  // rather than against a snapshot read here, so it is atomic with the write.
  const result = await req.repo.retireGames(
    req.params.rid, [...new Set(body.gameIds)], actorSeat(round, req.userId));
  if (result === null) return res.status(404).json({ error: 'Round not found' });
  if (result === 'unknown_game') return res.status(400).json({ error: 'Unknown game' });
...
// Set who owns many games at once (#972), so a round adopting ownership after
// its shelf is full does not open every detail page in turn.
//
// The four questions .claude/rules/bulk-paths-restate-the-single-path.md makes a
// bulk path re-decide, answered explicitly:
//
//  1. ACTIVITY — none. The single-path owner edit (PATCH …/games/:gid) writes no
//     Chronik row, so a counted one here would announce something the per-game
//     editor stays silent about. This is the one bulk route in the file with no
//     `actorSeat`, and that is why.
//  2. SIDE EFFECTS — none. Owners are ids on the game row: no cover object, no
//     session or feed reference, nothing to scrub or free.
//  3. PERMISSION — `round.write`, the single PATCH's level. Nothing is destroyed
//     and nothing leaves the round, so the co-owner `game.delete` gate that
//     bulk-delete carries would be the wrong kind of act.
//  4. QUOTA — none. No entity is created.
router.post('/bulk-owners', async (req, res) => {
  const round = await req.repo.getRoundMeta(req.params.rid);
  if (!round) return res.status(404).json({ error: 'Round not found' });

  const body = validateBody(bulkOwnersSchema, req, res);
  if (!body) return;

  // The same resolver the add sheet, the BGG import and the single PATCH use, so
  // one of them can never store an id another rejects (lib/game-owners.js).
  const ownerIds = resolveOwnerIds(body.ownerIds, round);
  if (ownerIds === null) return res.status(400).json({ error: 'Unknown member' });

  const result = await req.repo.setGameOwners(
    req.params.rid, [...new Set(body.gameIds)], ownerIds);
  if (result === null) return res.status(404).json({ error: 'Round not found' });
  if (result === 'unknown_game') return res.status(400).json({ error: 'Unknown game' });
...
// Add and remove tags across a selection (#1000) — tagging is the bulk-shaped job
// in the initial organising phase, and it was the one thing the selection bar
// could not do.
//
// The four questions .claude/rules/bulk-paths-restate-the-single-path.md makes a
// bulk path re-decide, answered explicitly:
//
//  1. ACTIVITY — none. The single path (PATCH …/games/:gid with `tagIds`) writes
//     no Chronik row for a tag change, so a counted one here would announce
//     something the per-game editor stays silent about. Hence no `actorSeat`,
//     exactly as bulk-owners.
//  2. SIDE EFFECTS — none. Tags are ids on the game row: no cover object, no
//     session or feed reference, nothing to scrub or free.
//  3. PERMISSION — `round.write`, the single PATCH's level. Nothing is destroyed
//     and nothing leaves the round. Listed in lib/round-access.js, or a grantee
//     is refused outright (UNLISTED = REFUSED).
//  4. QUOTA — none. Assigning consumes nothing; only CREATING a tag counts
//     against the per-round cap, and this route creates none.
router.post('/bulk-tags', async (req, res) => {
  const round = await req.repo.getRoundMeta(req.params.rid);
  if (!round) return res.status(404).json({ error: 'Round not found' });

  const body = validateBody(bulkTagsSchema, req, res);
  if (!body) return;

  // Unknown ids are a 400 exactly as on create and on the single PATCH — a stale
  // client holding a deleted tag must hear about it rather than have its request
  // half-applied.
  const roundTagIds = new Set((round.tags || []).map((tg) => tg.id));
  const add = [...new Set(body.addTagIds)];
  const remove = [...new Set(body.removeTagIds)];
  if ([...add, ...remove].some((x) => !roundTagIds.has(x)))
    return res.status(400).json({ error: 'Unknown tag' });

  const result = await req.repo.setGameTags(
    req.params.rid, [...new Set(body.gameIds)], { add, remove });
  if (result === null) return res.status(404).json({ error: 'Round not found' });
  if (result === 'unknown_game') return res.status(400).json({ error: 'Unknown game' });
...
// Permanently delete several games in one action (#832) — the path someone
// undoing a bad import actually wants.
//
// It accepts games that are still ON the shelf, which DELETE /:gid deliberately
// refuses ('not_archived'). That is the decision this endpoint exists to
// implement: making the user retire 200 games and then delete them is the
// two-step in bulk, i.e. the problem rather than the fix. The guard it gives up
// is replaced by two others — the co-owner `game.delete` capability (below, in
// lib/round-access.js) and a client confirm that names the count and states the
// session-history consequence.
router.post('/bulk-delete', async (req, res) => {
  ...
  const result = await req.repo.deleteGames(
    req.params.rid, [...new Set(body.gameIds)], actorSeat(round, req.userId));
  if (result === null) return res.status(404).json({ error: 'Round not found' });
  if (result === 'unknown_game') return res.status(400).json({ error: 'Unknown game' });

  // Free every deleted cover whose file nothing else still points at — an
  // imported round copies the path, not the file, so the check is per image and
  // not "these games are gone, so their covers are". Skipping it in the bulk
  // path is how a shelf purge silently leaks 200 objects
