 ...
 * it (lib/routes/friends.js). Adding any cross-tenant aggregate here is a new
...
// public/js/views-friends.js links one only from the Freundeskreis and the feed,
// and a demo can hold no friendships (friends.js refuses the send). So the only
// way here is a typed or shared URL.
//
// As MIDDLEWARE, so the refusal lands before the lookup below: a 403 that
// depended on whether the handle existed would make this the username oracle
// the signed-in surface deliberately is not.
router.get('/:username', accounts.requireUser, demo.refuseDemoAccount, async (req, res) => {
  const me = req.userId;
