...
// The frozen legacy-cover host list, for the storefront clean-up action (#981).
const providers = require('../providers');
const { instanceStatus } = require('../status');
const { CSV_BOM, toCsv } = require('../csv');
const { logger, recentLogs } = require('../observability');
...
/* #744 retired the four digital storefronts as lookup providers and deliberately
   kept what was already stored: covers hotlinked from their CDNs keep rendering,
   and `source` links stay as data. That is why `LEGACY_COVER_HOSTS` is frozen
   into the CSP img-src list and why the privacy policy names Sony, Valve,
   Nintendo and Microsoft as recipients at all.
   ...
   THIS CARD IS MEANT TO BE REMOVED once the count reaches 0 in production, the
...
const REDACT_KINDS = ['round', 'game', 'member', 'tag', 'expansion', 'feedback'];
