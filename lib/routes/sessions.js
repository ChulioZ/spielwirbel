...
const { affectedAccounts, badgeSnapshot, announceBadgeCrossings } = require('../badge-feed');
...
// guest caps above. The normalization itself lives in lib/draw-filters.js
// (#1328); this route only needs the count, to decide on the blocking backfill.
const { countMetadataFilters, shelfParty } = require('../../public/js/draw-pool');
...
// The QR encoder for the shared vote link (#1170). Server-side on purpose: it
// keeps an encoder out of the shared frontend scope, and the token never has to
// reach the client to become a picture.
const QRCode = require('qrcode');
// One home for the vote link's path shape, shared with the client that builds
// the same URL to hand around (.claude/rules/shared-constants-across-the-stack.md).
const { votePath } = require('../../public/js/vote-path');
const { appBaseUrl } = require('../canonical');
// The three endings a played session can record when nobody won (#1038). The
// results screen OFFERS them and this route validates by membership, so the two
// sides cannot disagree about what the set is
// (.claude/rules/shared-constants-across-the-stack.md).
...
const { sanitizePersonVotes, sessionParticipantIds, normalizeLegacyVotes } = require('../session-votes');
...
// The filter half of a draw, shared with the saved-filter route (#1328).
...
const startSessionSchema = z.object({
  memberIds: z.preprocess((v) => (Array.isArray(v) ? v.map(String) : []), z.array(z.string())),
  // Seats that are at the table WITHOUT their shelf (#1002). Same lenient
  // coercion as `memberIds`, and it needs no filtering against the round at all:
  // `shelfParty` subtracts this list from the seats, so an id that is not a seat
  // simply subtracts nothing. Absent -> [] -> the seats are passed through
  // unchanged, which is every draw before this and the great majority after it.
  withoutShelfIds: z.preprocess((v) => (Array.isArray(v) ? v.map(String) : []), z.array(z.string())),
  // count, tagIds, excludeTagIds, tagMode, metadata, multiTable — shared with
  // the saved-filter route (#1328), which accepts exactly the same controls.
  ...DRAW_FILTER_FIELDS,
...
// that reads clearest.
// `cancel` takes one optional boolean like the games router's shelf flips;
// `split` takes the table list, whose SHAPE zod checks and whose meaning
// (drawn games, seated people, table count) validateSplitTables still owns.
const cancelSchema = z.object({ cancelled: z.boolean().optional() });
const splitSchema = z.object({ tables: z.array(z.unknown()).optional() });

const finishSchema = z.object({
  // Optional, default "finish": `{ finished: false }` un-finishes. It was read
  // straight off req.body until the 2026-09-06 audit BECAUSE the schema did not
  // name it — zod strips unknown keys, so reading it from the validated body
  // silently turned every un-finish into a finish (test/friends.test.js caught
  // it). A field a handler reads belongs in the schema, never beside it.
  finished: z.boolean().optional(),
  winnerIds: z.preprocess((v) => (Array.isArray(v) ? v.map(String) : []), z.array(z.string())),
  // How the evening ended when nobody won (#1038). Absent means "nothing
  // chosen"; the handler below refuses it alongside a non-empty winner list,
  // which is the invariant that keeps the pair from ever disagreeing.
...
router.post('/', async (req, res) => {
  ...
  // The filter half — tags, tag mode, metadata, multi-table, count — resolved
  // by the one function the saved-filter route shares (#1328). The metadata is
  // normalized against the (possibly just backfilled) shelf's ACTIVE games,
  // because those are the only ones a draw can pick and the setup screen
  // derives its controls from the same set.
  const resolved = resolveDrawFilters(body, round, shelf.games.filter(isActiveGame));
  const { tagIds, excludeTagIds, tagMode, metadata, multiTable, count } = resolved;
  ...
  // can seat some table this group could form. `showStartSession()` applies the
  // identical predicate to the identical count for its preview
  // (.claude/rules/active-games-filter-sites.md).
  // The owner clause (#971) keys off the SEATS that joined, not `playerCount` —
  // guests own nothing (.claude/rules/session-guests-are-not-members.md), so a
  // game whose owners are all away cannot be put on this table. `memberIds` is
  // already resolved against the round above, and it is never empty here (the
  // guard further up falls back to the whole round), so the clause is always
  // applied rather than being conditional on the body having sent seats.
  //
  // Since #1002 a seat can be present without its shelf, so what the clause
  // actually wants is the seats MINUS those people. The subtraction goes through
  // the shared `shelfParty` because the setup screen's live preview does the
  // same one — see its comment for why an all-away party legitimately reduces to
  // `[]`. It changes nothing else the seat means: `memberIds` below is still the
  // full seat list, so the marked member votes, plays, joins a team and wins
  // trophies exactly as before.
  //
  // Resolved against the seats before it is stored, the same drop-unknown-ids
  // rule every other list on this body gets: the blob must not carry an id that
  // names nobody at this table, because the results screen reads it back.
  const withoutShelfIds = memberIds.filter((mid) => body.withoutShelfIds.includes(mid));
  const shelfIds = shelfParty(memberIds, withoutShelfIds);
  const pool = drawPool(shelf, { tagIds, excludeTagIds, tagMode, metadata, playerCount, multiTable, memberIds: shelfIds });
  if (pool.length === 0)
  ...
  // Stored resolved and in the absent-key shape `filterPreset` documents.
  const filters = filterPreset(resolved);
  ...
  const session = await req.repo.createSession(req.params.rid, {
    ...
    // Who among them came WITHOUT their shelf (#1002). Stored — unlike the
    // draw's other inputs, which the pool alone consumes — because the results
    // screen needs it: `boxBringers` must not tell the table to get the box
    // from somebody who said they did not bring it. Same absent-key discipline
    // as `guests`/`teams`, so an ordinary evening's blob is byte-identical to a
    // pre-#1002 one in both backends.
    ...(withoutShelfIds.length ? { withoutShelfIds } : {}),
    ...
    votes: {}, // votes[personId][gameId] = { rating: 1..5 }
...
//
// Both handlers below mint through `mintVoteLink` rather than repeating the
// guard, so the QR picture (#1170) cannot be reachable in any state where the
// link itself is refused. It answers the request itself on a refusal and returns
// null; a caller that reads null must simply return.
async function mintVoteLink(req, res) {
  const round = await req.repo.getRoundMeta(req.params.rid);
  if (!round) { res.status(404).json({ error: 'Round not found' }); return null; }
  const session = await req.repo.getSession(req.params.rid, req.params.sid);
  if (!session) { res.status(404).json({ error: 'Session not found' }); return null; }

  if (session.cancelled) { res.status(400).json({ error: 'Session is cancelled' }); return null; }
  if (session.done) { res.status(400).json({ error: 'voting_closed' }); return null; }
...
router.post('/:sid/vote-link', async (req, res) => {
  const token = await mintVoteLink(req, res);
  if (!token) return;
...
// The same link as a picture to hold up at the table (#1170).
//
// A POST although it only reads to the caller's eye: it mints, exactly like the
// route above, and this router already spells a minting read that way
// (`/:sid/tables`). Idempotent for the same reason — the repo hands back the row
// that exists — so the code drawn here is always the link currently in force,
// and re-minting cannot leave a stale picture behind.
//
// The origin comes from THIS request rather than from APP_BASE_URL: the app
// fetches same-origin, so the Host header is by construction the page the sharer
// is looking at — the very thing the note above says the client knows and the
// server does not. That holds only because the client never names it; don't
// accept an origin from the body, which would make this a QR encoder for
// arbitrary content.
//
// The path shape is required out of public/js/vote-path.js so the picture and
// the shared link cannot drift apart.
router.post('/:sid/vote-link/qr', async (req, res) => {
  const token = await mintVoteLink(req, res);
  if (!token) return;
  const origin = req.get('host') ? `${req.protocol}://${req.get('host')}` : appBaseUrl();
  // Level M is the QR standard's own default: it survives a fingerprint or a
  // thumb over a corner without inflating the module count on a phone screen.
  const svg = await QRCode.toString(origin + votePath(token), {
    type: 'svg',
    errorCorrectionLevel: 'M',
    margin: 2,
  });
  // It draws a live capability. Nothing between here and the table may keep it.
...
router.post('/:sid/results', async (req, res) => {
  ...
  const body = validateBody(saveResultsSchema, req, res);
  if (!body) return;
  // A bundle old enough to still use this route may also be old enough to offer
  // a retirement control, so the one shape #909 removed can still arrive here.
  const votes = normalizeLegacyVotes(body.votes);
...
router.post('/:sid/split', async (req, res) => {
  ...
  const body = validateBody(splitSchema, req, res);
  if (!body) return;
  const tables = body.tables || null;
...
router.post('/:sid/finish', async (req, res) => {
  ...
  if (!body) return;
  const finished = body.finished !== false; // default: true
  let winnerIds = [];
  let ending;
  if (finished) {
    if (session.cancelled)
    ...
    // A session has winners, or an ending, or neither — never both (#1038).
    // Checked against the RAW list rather than the filtered one: a client that
    // sends both must be told so, not silently rescued by an id that happened
    // to fail the allowlist.
    if (body.ending && body.winnerIds.length)
      return res.status(400).json({ error: 'Ending and winners are mutually exclusive' });
    // Undefined when the body carried none — and the backends DELETE the key on
    // that, which is what lets the winner chips replace a stored ending with no
    // extra field: the results screen has no save button, so every chip tap
    // re-POSTs the whole result. No `!winnerIds.length` guard here on purpose:
    // the 400 above already makes the two mutually exclusive, and a second
    // belt-and-braces clause would be a branch no test can reach.
    ending = body.ending;
  }
  // The account-tier Abzeichen (#1389): measure the accounts this request can
  // move BEFORE the mutation, so the diff after it names the tiers it crossed.
  // Accounts mode only (req.userId, as emitFeedEvent), and empty — no read at
  // all — when no seat at the table is linked to an account.
  const badgesBefore = finished && req.userId
    ? await badgeSnapshot(affectedAccounts(round.members, session, winnerIds))
    : null;
  const updated = await req.repo.finishSession(
    req.params.rid,
    req.params.sid,
    { finished, winnerIds, ending },
  ...
  if (finished) {
    ...
    // Before session_played, on purpose: the feed reads newest first, so the
    // play sits above its mark, and an un-finish + re-finish of the same evening
    // leaves the two session_played rows ADJACENT for the read-side collapse
    // (the mark is announced once — lib/badge-feed.js — so it is not repeated
    // between them).
    await announceBadgeCrossings(badgesBefore);
...
router.post('/:sid/cancel', async (req, res) => {
  ...
  const body = validateBody(cancelSchema, req, res);
  if (!body) return;
  const cancelled = body.cancelled !== false; // default: true
