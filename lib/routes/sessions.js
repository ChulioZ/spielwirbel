...
router.delete('/:sid', async (req, res) => {
  ...
  // A `done` session is real shared history and stays co-owner-only, refused with
  // the same code the gate uses so the client's error handling is unchanged.
  const session = await req.repo.getSession(req.params.rid, req.params.sid);
  if (!session) return res.status(404).json({ error: 'Session not found' });
  if (session.done && !can(req.roundRole, 'session.delete')) {
