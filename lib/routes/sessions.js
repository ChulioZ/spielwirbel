...
// Lazy provider-metadata backfill (#717): session start is one of its two
// trigger points, so both voting surfaces read stored values.
...
router.post('/', async (req, res) => {
  ...
  const metadata = normalizeMetadataFilters(
    body.metadata,
    metadataFilterOptions(round.games.filter(isActiveGame))
  );
  ...
  const pool = drawPool(round, { tagIds, excludeTagIds, tagMode, metadata, playerCount });
  if (pool.length === 0)
