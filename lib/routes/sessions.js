...
const startSessionSchema = z.object({
  ...
  // Per-device voting (#209): passed through untouched like `gameId` and read
  // as a strict `=== true` in the handler, so anything else means off. Only the
  // DRAW path honours it — direct-pick has no voting phase to open.
  deviceVoting: z.unknown().optional(),
});
...
router.post('/', async (req, res) => {
  ...
  if (body.gameId != null) {
    ...
    // Same predicate as the draw pool below (#486) — an archived game must not
    // stay playable by id just because the UI no longer offers it. The message
    // still names which archive it is in, which is why the branch survives.
    if (!isActiveGame(game))
      return res
        .status(400)
        .json({ error: game.retired ? 'Game is retired' : 'Game is completed' });
  ...
  const pool = drawPool(round, { tagIds, excludeTagIds, playerCount });
  if (pool.length === 0)
  ...
  const session = await req.repo.createSession(req.params.rid, {
    ...
    ...(teams.length ? { teams } : {}),
    // Per-device voting (#209), same absent-key discipline: an ordinary hot-seat
    // session grows no `deviceVoting` key at all, so every session drawn before
    // this shipped reads as `undefined` — falsy — and every guard below closes
    // on it without a migration.
    ...(body.deviceVoting === true ? { deviceVoting: true } : {}),
...
// Sanitize ONE person's column for the per-device write (#209).
//
...
// this endpoint is called once per person, mid-session, by whichever device that
// person is holding. So it keeps only games this session actually drew — a vote
// for a game that was never on the table would otherwise land in `gameStats()`
// and move a rating average that no screen can explain.
//
...
// asymmetry dropGuestRetireFlags() produces above, for the same reason (#458).
// An entry with neither a rating nor a flag is dropped rather than stored as an
// empty vote, so "submitted nothing" stays distinguishable from "has not voted".
...
// .claude/rules/member-seat-self-claim.md); per-action permissions inside a
// round are #137's job, not this route's.
router.post('/:sid/votes/:pid', async (req, res) => {
  ...
  // Only a session that was drawn with per-device voting takes incremental
  // writes. Anything else is a hot-seat session whose wizard will POST the whole
  // map to /results and overwrite whatever landed here — so accepting the vote
  // would be a promise the next request breaks.
  if (!session.deviceVoting)
    return res.status(400).json({ error: 'not_device_voting' });
...
router.post('/:sid/close', async (req, res) => {
  ...
  const updated = await req.repo.closeSessionVoting(req.params.rid, req.params.sid);
  if (!updated) return res.status(404).json({ error: 'Session not found' });
  res.json(updated);
});
...
router.post('/:sid/results', async (req, res) => {
  ...
  // This route REPLACES the whole vote map, which is right for the hot-seat
  // wizard (its closure holds every person's votes) and destructive for a
  // per-device session, where each column was written separately and this
  // caller has at most its own. Refuse rather than silently discard everyone
  // else's votes — /votes/:pid and /close are that session's path (#209).
  if (stored.deviceVoting) return res.status(400).json({ error: 'device_voting' });
