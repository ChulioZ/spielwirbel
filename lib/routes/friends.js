...
// Resolve a set of account ids to what these two surfaces render for a person:
// the public username (#320) and the profile picture (#841), in one pass over
// small, capped sets — the friends list / a page of feed authors. A missing
// account (edge: mid-erasure) maps to nulls; the frontend shows a neutral
// fallback for the name and the initials avatar for the picture.
//
// The avatar rides HERE rather than through GET /api/account/avatars because
// these responses already resolve the account — the batch endpoint exists for
// the round screens, which hold member.userId and must NOT grow a join into the
// users table (#841 §3). Two ways in, one stored field; nothing is denormalized
// either way.
async function publicProfilesFor(ids) {
  const map = new Map();
  await Promise.all([...new Set(ids)].map(async (uid) => {
    const u = await repo.getUserById(uid);
    map.set(uid, { username: (u && u.username) || null, avatar: (u && u.avatar) || null });
  }));
  return map;
}
...
router.get('/', accounts.requireUser, async (req, res) => {
  ...
  const people = await publicProfilesFor(rows.map((f) => otherParty(f, me)));

  const shape = (f) => ({
    friendshipId: f.id,
    userId: otherParty(f, me),
    username: people.get(otherParty(f, me)).username,
    avatar: people.get(otherParty(f, me)).avatar,
  });
  res.json({
...
//
// Paged since #1357: `?before=<nextCursor>` reads the next page, and `nextCursor`
// is null on the last one. The window/filter/collapse/slice order and where the
// cursor points live in ONE place, lib/feed.js readFeedPage, shared with the
// profile feed. The home screen's compact section reads the first page only.
router.get('/feed', accounts.requireUser, async (req, res) => {
  ...
  const page = since.size
    ? await readFeedPage([...since.keys()], (uid) => since.get(uid), before)
    : { events: [], nextCursor: null };
  const { events } = page;

  const people = await publicProfilesFor(events.map((e) => e.uid));
  res.json({
    ...
    events: events.map((e) => ({
      ...
      title: e.title,
      coverUrl: e.coverUrl,
      // Undefined for the two types that carry none, so JSON.stringify omits it
      // and the client's `ev.count` branch is the same shape on both routes.
      count: e.count,
      // An Abzeichen's tier (#1389) — the catalogue key rides `title`.
      tier: e.tier,
      at: e.at,
      username: people.get(e.uid).username,
      avatar: people.get(e.uid).avatar,
    })),
