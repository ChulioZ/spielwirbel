...
const ROUTE_ROLES = [
  ...
  ['POST', '/games/bulk-owners', 'round.write'],
  ['POST', '/games/bulk-tags', 'round.write'],
  ...
  // Retiring a seat (#1006) is the same class as editing its name: reversible,
  // changes nothing about who may reach the round, and undoable from the
  // member's own page. Deleting one is not — it is the only act in this file
  // that removes a row outright, so it sits with the destructive four.
  ['POST', '/members/:mid/retire', 'round.write'],
  ['DELETE', '/members/:mid', 'round.delete'],
  ...
  // Drawing that same link as a QR code mints it, so it costs exactly what
  // minting costs — never less (#1170).
  ['POST', '/sessions/:sid/vote-link/qr', 'round.write'],
  ...
  // Colour marker (#1187). An ordinary round write: choosing the round's colour
  // is the same kind of act as renaming it.
  ['PATCH', '/marker', 'round.write'],

  // Tags.
  ['POST', '/tags', 'round.write'],
  // Listed BEFORE the parameterised sibling, exactly as the router registers it:
  // `/tags/:tagId` compiles to `^/tags/[^/]+/?$`, which already matches
  // `/tags/order`, so reordering would be granted implicitly by a pattern that
  // is not about it. Stating it keeps the table readable as the answer to "what
  // does this route cost" and survives `/tags/:tagId` ever being narrowed.
  ['PATCH', '/tags/order', 'round.write'],
  ['PATCH', '/tags/:tagId', 'round.write'],
  ['DELETE', '/tags/:tagId', 'round.write'],

  // Saved session filters (#1328). Anyone who can draw in the round may name,
  // rename, reorder and delete its filters — the same floor as the draw itself.
  // `/filters/order` listed before its parameterised sibling, as `/tags/order`.
  ['POST', '/filters', 'round.write'],
  ['PATCH', '/filters/order', 'round.write'],
  ['PATCH', '/filters/:fid', 'round.write'],
  ['DELETE', '/filters/:fid', 'round.write'],
