...
/*
 * The profile's components sorted high to low, as a prefix-sum table — what
 * `attainable` below reads to answer "how good could a candidate of this size
 * possibly be against this shelf?". Computed once per profile, not per candidate:
 * the sort is O(n log n) over the round's own mechanics, the lookup is O(1) over
 * 17k candidates.
 */
