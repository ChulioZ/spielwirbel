 ...
 * pick and a drawn-and-chosen game count the same — the defect this fixes is
 * that direct picks counted ZERO, not that they should outrank a drawn win, and
 * a drawn winner is already ahead on the ladder because it carries the ratings a
 * direct pick never gets.
 *
 ...
 *
 * `playScale` has NO default, on purpose and for the same reason `attainable` is
 * not defensive about a missing `prefix`: a caller that forgot it would silently
 * get the pre-#778 scoring back — a plausible, confident, wrong list with no
 * error anywhere — where an absent argument is a programming error that should
 * throw on the first game. `buildProfile` is the only production caller.
 */
function gameAffinity(round, game, playScale) {
  ...
  const score = ownRating(round, game.id);
...
function buildProfile(round, corpusById) {
  ...
  profiled.forEach((x) => {
    ...
    const affinity = gameAffinity(round, x.game, playScale);
    x.affinity = affinity;
