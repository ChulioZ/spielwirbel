...
function reasonsFrom(profile, entry, scored, stats) {
  ...
  return withoutWeakerTaste(ranked)
    ...
    // being lost — unreachable today (a term only clears the `> NEUTRAL` gate by
    // matching a game in `profile.games`, which is the very list the contributors
    // come from), but the ordering is what keeps it harmless if that changes.
