...
function createApp() {
  ...
  const feedbackLimit = Number(process.env.FEEDBACK_RATE_LIMIT_MAX) || 10;
  ...
  // (useless until mail can deliver) and, with #134, the legal pages (useless
  // until the Impressum address exists), and a partially populated public face
  // reads as broken. Values are read per request like every other env ceiling
  // in this file; mounted ahead of the auth gates because the footer renders on
  // the login page too. Never put anything sensitive in this response.
  app.get('/api/config', (req, res) => {
    const impressum = (process.env.IMPRESSUM_ADDRESS || '').trim() !== '';
    res.json({ footer: mail.isConfigured() && impressum });
  });
  ...
  // Feedback (issue #260) gets its own low ceiling on top of the global one:
  // it's a write anyone signed in can make, and a handful per window is far
  // more than genuine use needs. Read per call like the ceilings above, so a
  // test can drive it and a deployment can re-tune it without a rebuild.
