...
const { requestLogger, healthz, createReadyz, errorHandler, logger } = require('./observability');
...
const { requireUploadOwner } = require('./upload-access');
const { imageCspSources } = require('./providers');
const { createManifestRoute } = require('./web-manifest');
// The user-design registry (#1184). A backend file requiring out of public/js/
// on purpose: the client offers the designs and the server decides which of
// them exist, so there is exactly one list
...
function assetPathSet(dir) {
  try {
    ...
    // too much degrades, one that counts too little stops being a limiter. And
    // say so: the degradation is otherwise invisible (#1148).
    logger.warn({ event: 'asset_scan_failed', code: err.code || err.name });
    return new Set();
  }
}
...
function createApp() {
  ...
  // Browser-fault reports (#1149). 20, not 5: the client caps itself at a
  // handful per page load, so an ordinary tab can produce a few and a shared IP
  // behind one address several — while 20 is still far below what it takes to
  // churn the 200-entry buffer. See routes/client-error.js.
  const clientErrorLimit = Number(process.env.CLIENT_ERROR_RATE_LIMIT_MAX) || 20;
  ...
  // Profile-picture uploads (#841). Its own low cap because this is the app's
  // only endpoint whose cost is DECODING attacker-supplied bytes: the byte cap
  // and the pixel ceiling bound one request, and this bounds how many of them a
  // caller may spend. AUTH_RATE_LIMIT_MAX (100) is the wrong order of magnitude
  // for that, and nobody changes their picture ten times a window.
  const avatarLimit = Number(process.env.AVATAR_RATE_LIMIT_MAX) || 10;
  ...
  // The store is in memory and per process (#215's shared store was closed
  // unshipped; railway.json's numReplicas: 1 is the accepted control), so
  // there is no way to clear such a lockout except waiting it out or
  // restarting. Counting a page load the same as an API write was the
  // asymmetry; exempting the shell fixes it and leaves the ceiling meaningful
  // for everything else. The shell holds no user data (it is the code that is
  // public on the repo anyway), so volumetric
  ...
  // ONE route legitimately needs a bigger body than the rest of the app. The
  // collection import (#481) accepts up to 2000 games plus a cover URL and an
  // edition per game, and the client sends the whole selection in a single
  // request — which a realistic BGG payload (~140-char cover URLs) pushes past
  // the 100 KB default at ~431 games, well inside the schema's own cap. So the
  // transport was refusing bodies the server declared legal, and silently: the
  // parser rejects BEFORE routing, so the route never runs and the user sees a
  // generic failure.
  //
  // Scoped to that exact path instead of raising the global cap, because this
  // parser sits ahead of the auth gate — the 100 KB default is what keeps the
  // whole unauthenticated surface cheap to serve, and only this one path may
  // buffer more (the rate limiter above still bounds how often). An exact-path
  // regex, not a prefix: a sibling route under the same mount must not inherit
  // the ceiling. 1 MB is ~2x the realistic worst case of ~480 KB — the schema
  // caps counts, not bytes, so a byte bound has to live here.
  const importJson = express.json({ limit: '1mb' });
  const IMPORT_BODY_PATH = /^\/api\/rounds\/[^/]+\/lookup\/import$/;
  app.use((req, res, next) => (
    req.method === 'POST' && IMPORT_BODY_PATH.test(req.path)
      ? importJson(req, res, next)
      : next()
  ));

  // Everything else, at body-parser's 100 KB default. Reached by the import path
  // too, where it is a no-op: body-parser skips a request whose body is parsed.
  ...
  // Same placement rule as register's and demo's: BEFORE the account router, or
  // its prefix match fields the request and this never runs. Stacks with
  // authLimiter rather than replacing it, and is deliberately method-agnostic —
  // DELETE costs nothing to serve, but counting it costs a prober rather than a
  // user, who removes a picture once.
  app.use('/api/account/me/avatar', avatarLimiter);
  ...
  // Browser-side fault reports (issue #1149): mounted ahead of the gate for the
  // same reason as the contact form — the faults most worth hearing about are
  // the ones on the login and landing screens, where no session exists yet.
  //
  // Its OWN ceiling, stacking with the global limiter, and low: the client
  // already bounds itself to a handful of reports per page load, so a caller
  // past this is not a browser with a bug but somebody filling the buffer. The
  // ceiling has to be its own rather than the auth one, or an error storm from a
  // shared IP would spend the budget people need to log in with.
  const clientErrorLimiter = rateLimit({
    windowMs: GLOBAL_WINDOW_MS,
    limit: clientErrorLimit,
    standardHeaders: true,
    legacyHeaders: false,
    message: { error: 'rate_limited' },
  });
  app.use('/api/client-error', clientErrorLimiter, require('./routes/client-error'));
  ...
  app.get('/api/config', (req, res) => {
    ...
    res.json({
      ...
      // The per-game expansion ceiling (#1143) — the one quota the client is
      // told BEFORE the write, because the expansions dialog counts the ticked
      // set against it („N von 40 im Regal") instead of letting the user find
      // out from the 403's toast. A documented, operator-set constant, so it is
      // as public as the two flags above.
      //
      // `null` when quotas are inert — lib/quota.js enforces only in accounts
      // mode — rather than the number: a self-hosted round may legitimately hold
      // more than the default, and a bar calling that "over the limit" would be
      // stating a rule the server does not apply.
      expansionsPerGame: quota.enforced() ? quota.expansionsPerGame() : null,
      // Which user designs (#1184) exist on THIS instance, and which one the
      // logged-out surfaces wear. The gate is in code rather than in an env var
      // (operator decision 2026-09-20), and it is the SERVER that applies it:
      // outside production every registered design is selectable so an
      // unfinished one can be built and reviewed, in production only the
      // enabled ones exist at all. Ids only — the client already has the
      // registry, and this is the one thing it cannot work out for itself.
      designs: designs.selectableDesignIds({ production: process.env.NODE_ENV === 'production' }),
      faceDesign: designs.FACE_DESIGN,
    });
  });

  // Instance-wide public statistics (#564): the scale counters and the four
  // game podiums the landing page and /entdecken render. Mounted ahead of the
  // auth gates for the same reason as /api/config — its primary audience is a
  // logged-out visitor.
  //
  // 404 WHEN THE FEATURE IS OFF, not an empty 200: an instance that has not
  // opted in exposes nothing at all, the accounts_disabled / ADMIN_PASSWORD
  // precedent. The payload is whatever the scheduler last built; this handler
  // never computes and never calls a provider, so it cannot be used to drive
  // upstream load. It stays under /api/ deliberately, so it spends from the
  // global per-IP limiter budget and never joins assetPathSet
  // (.claude/rules/security-middleware.md).
  app.get('/api/stats/public', (req, res) => {
    const stats = publicStats.publicStats();
  ...
  // The manifest per design first (#1199) — as open as the file it stands in
  // front of, and falling through to it for the face; lib/web-manifest.js.
  app.get('/manifest.webmanifest', createManifestRoute(ASSET_DIR));
  ...
  // ...and, since #955, only to the tenant that OWNS the object (or a grantee
  // on the round referencing it). The gate above answers "a valid account?";
  // requireUploadOwner answers "whose bytes are these?" — see lib/upload-access.js.
  // It sits in front of storage.serve, so both storage backends are covered by
  // one check rather than two.
  app.use('/uploads', uploadGate, requireUploadOwner, storage.serve);
  ...
  // is retired. The operator read side stays on /api/admin (lib/routes/admin/).
  ...
  // One router, two mounts (#1187): PATCH /marker sets the colour marker,
  // POST /background keeps the retired per-round design writable for an older
  // client until the flip (#1202). See lib/routes/marker.js.
  app.use('/api/rounds/:rid/marker', require('./routes/marker').marker);
  app.use('/api/rounds/:rid/background', require('./routes/marker').background);
  ...
  // Games the round does not own, scored from the BGG corpus (#682). Read-only
  // and round-scoped like everything above it.
...
// assetPathSet is exported for test/fault-logging.test.js only: its catch is
// the limiter's fail-closed branch, which is unreachable through createApp with
// a readable public/ tree.
module.exports = { createApp, assetCacheHeaders, assetPathSet };
