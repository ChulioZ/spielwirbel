...
function createApp() {
  ...
  // same set isAllowedImageUrl trusts for downloads) so the browser can render
  // provider covers in the add-game/link previews and lookup thumbnails — see
  ...
  // the Claude API — routes/recommendations.js). Two layers stack on the route:
  // this blunt per-IP burst cap (RECS_RATE_LIMIT_MAX), then the per-tenant
  // monthly SPEND cap (#139, quota.recommendationsGuard, mounted below) which
  // bounds the billed spend per tenant rather than per IP.
  ...
  app.use('/api/rounds/:rid/recommendations', recommendationsLimiter, quota.recommendationsGuard, require('../routes/recommendations'));
