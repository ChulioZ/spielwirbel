...
function createApp() {
  ...
  // same-site <img> GETs). requireAuth is a no-op when auth is disabled. The
  // storage backend serves the bytes — off local disk by default, streamed from
  // object storage when S3 is configured (issue #128) — behind this same gate.
  app.use('/uploads', auth.requireAuth, storage.serve);
