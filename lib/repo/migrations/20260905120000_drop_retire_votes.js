...
exports.up = async (knex) => {
  ...
  try {
    ...
    /* THE VERIFICATION MUST RUN INSIDE THIS WINDOW, and getting that wrong is
       the same trap the rewrite itself is about, one statement over: a SELECT
       issued after FORCE is restored is filtered by the tenant policy and
       returns nothing, so the check would pass by construction — vacuously in
       production and honestly against a local superuser, i.e. green exactly
       where it cannot help.

       It also has to be here for a second reason. The ALTER above holds an
       ACCESS EXCLUSIVE lock on `sessions` for the rest of this transaction, so
       nothing else can write to the table while we count. Outside the window a
       vote committed by the still-serving previous container between the UPDATE
       and the count would fail this migration — and therefore the deploy — over
       a row the next boot would simply clean up. */
