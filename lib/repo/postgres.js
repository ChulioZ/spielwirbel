...
// Clear EVERY game's cover and provider source link, across every tenant, and
// report what changed. The one-time operator sweep from #172 (see the JSON
// backend for the why). Same split as takedownImage and for the same reason: the
// admin escape only widens SELECT, so it finds the rows and the ordinary
// tenant-scoped tx does the writing. An UPDATE under atx() would match zero rows
// and report a clean sweep that changed nothing.
async function purgeAllCovers() {
  const targets = await atx((trx) =>
    trx('games')
      .whereRaw("data->>'image' IS NOT NULL")
      .orWhereRaw("data->>'source' IS NOT NULL")
      .select('id', 'tenant_id', 'data'));
  let images = 0;
  let sources = 0;
  for (const t of targets) {
    const patch = {};
    if (t.data.image) patch.image = null;
    if (t.data.source) patch.source = null;
    const rows = await qt(t.tenant_id, (trx) =>
      trx('games')
        .where({ id: t.id, tenant_id: t.tenant_id })
        ...
        .returning('id'));
    if (rows[0]) {
      if (patch.image !== undefined) images += 1;
      if (patch.source !== undefined) sources += 1;
    }
  }
