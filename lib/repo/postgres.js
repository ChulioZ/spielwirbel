...
async function getRound(rid) {
  // Fetch the round and its four child collections CONCURRENTLY. Unlike
  // childrenOf's transaction callers (a single client can't run concurrent
  // queries), getRound reads on the pool, so each query gets its own connection
  // and they run in parallel — collapsing ~5 sequential round-trips into one.
  ...
  const [r, members, games, sessions, activities] = await Promise.all([
  ...
  return assemble(r.rows[0], {
    members: members.rows, games: games.rows, sessions: sessions.rows, activities: activities.rows,
  });
}
