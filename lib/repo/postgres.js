 ...
 *    a JS ARRAY into a Postgres array literal — corrupting `recommendation_runs`
 *    (an array) and any array payload. Stringifying uniformly sidesteps the
 *    footgun; pg casts the text to jsonb on assignment.
...
// Costs BEGIN/SET/COMMIT round trips (~a millisecond over the deploy's private
// network); independent qt() calls still parallelize — each knex.transaction()
// checks out its own pooled connection.
...
async function listRounds(tenant) {
  const rounds = await qt(tenant, (trx) =>
    trx('rounds').where({ tenant_id: tenant }).orderBy('seq').select('id', 'name', 'background', 'recommendation_runs'));
  if (rounds.length === 0) return [];
  const ids = rounds.map((r) => r.id);
  const [members, games, sessions] = await Promise.all([
    qt(tenant, (trx) => trx('members').whereIn('round_id', ids).andWhere({ tenant_id: tenant }).orderBy('seq').select('id', 'round_id', 'data')),
    qt(tenant, (trx) => trx('games').whereIn('round_id', ids).andWhere({ tenant_id: tenant }).orderBy('seq').select('id', 'round_id', 'data')),
    qt(tenant, (trx) => trx('sessions').whereIn('round_id', ids).andWhere({ tenant_id: tenant }).orderBy('seq').select('id', 'round_id', 'data')),
  ]);
  const group = (rows) => {
    const m = new Map();
...
async function getRound(tenant, rid) {
  ...
  // single-transaction callers (one connection can't run concurrent queries),
  // getRound reads via independent qt() calls, so each gets its own connection
  // and they run in parallel — collapsing the sequential round-trips into one
  // batch. That round-trip count dominates latency to a hosted Postgres (a
  // sequential fetch measured ~1s there); listRounds already fetches this way.
  const [r, members, games, sessions] = await Promise.all([
    qt(tenant, (trx) => trx('rounds').where({ id: rid, tenant_id: tenant }).select('id', 'name', 'background', 'recommendation_runs')),
    qt(tenant, (trx) => trx('members').where({ round_id: rid, tenant_id: tenant }).orderBy('seq').select('id', 'data')),
    qt(tenant, (trx) => trx('games').where({ round_id: rid, tenant_id: tenant }).orderBy('seq').select('id', 'data')),
    qt(tenant, (trx) => trx('sessions').where({ round_id: rid, tenant_id: tenant }).orderBy('seq').select('id', 'data')),
  ]);
  if (!r[0]) return null;
  return assemble(r[0], { members, games, sessions });
}
...
async function createRound(tenant, { name, members, importFromRoundId }) {
  return tx(tenant, async (trx) => {
    ...
    const round = await trx('rounds').where({ id: rid }).select('id', 'name', 'background', 'recommendation_runs');
    return assemble(round[0], await childrenOf(trx, tenant, rid));
  });
}

async function deleteRound(tenant, rid) {
  const n = await qt(tenant, (trx) => trx('rounds').where({ id: rid, tenant_id: tenant }).del());
  return n > 0;
}
...
async function listActivities(tenant, rid) {
  const [round, acts] = await Promise.all([
    qt(tenant, (trx) => trx('rounds').where({ id: rid, tenant_id: tenant }).first('id')),
    qt(tenant, (trx) => trx('activities').where({ round_id: rid, tenant_id: tenant }).orderBy('seq').select('id', 'data')),
  ]);
  if (!round) return null;
  return acts.map(withId);
}
...
async function saveRecommendationRuns(tenant, rid, runs) {
  // runs is an ARRAY — J() it (a raw array binding would become a Postgres array
  // literal, not jsonb; see the header note).
  const rows = await qt(tenant, (trx) =>
    trx('rounds').where({ id: rid, tenant_id: tenant }).update({ recommendation_runs: J(runs) }).returning('recommendation_runs'));
  return rows[0] ? rows[0].recommendation_runs : null;
}
