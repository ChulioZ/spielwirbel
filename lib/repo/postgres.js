...
const SUMMARY_OBJ = `jsonb_build_object(
    ...
    /* The open sessions the home resume zone offers (#842), newest first and
       ...
       calls out for the vote-scale corpus aggregate: the predicate lives in
...
async function publicGameAggregates(now = new Date().toISOString()) {
  ...
  return atx(async (trx) => {
    ...
    const ratings = await trx({ s: 'sessions' })
      ...
      .whereRaw('g.round_id = s.round_id')
      /* The 0–5 TILE this vote landed on, or NULL when it is not a vote at all.
         This expression is the one place the shared public/js/vote-scale.js rule
         has to be restated, because SQL cannot require it
         (.claude/rules/shared-constants-across-the-stack.md); #914 collapsed the
         old separate WHERE and CASE into it so the admission test and the tile
         are by construction the same decision, instead of two spellings of it.

         RETIREMENT WINS over a stored rating, matching `effectiveRating` —
         legacy rows can carry both, and pre-#797 this read
         `jsonb_typeof(…'rating') = 'number'` alone, dropping a retire-only vote
         entirely. Compared as jsonb, not `->>'retire' = 'true'`: the text form
         also matches the STRING "true", which `effectiveRating`'s `=== true`
         rejects — and the legacy /results route stores a member's column
         unvalidated.
         ...
         than merely sit beside it. `in (0,1,2,3,4,5)` covers range and
      ...
      .crossJoin(trx.raw(`lateral (select case
          when vote.val->'retire' = 'true'::jsonb then 0
          else (case when jsonb_typeof(vote.val->'rating') = 'number'
                     then (case when (vote.val->>'rating')::float8 in (0,1,2,3,4,5)
