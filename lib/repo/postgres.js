...
async function createGame(tenant, rid, fields, actorMemberId) {
  return tx(tenant, async (trx) => {
    ...
    // Provider-sourced info (#717) — see the JSON backend; keys stay absent
    // when unresolved (absent-key parity).
    if (Number.isFinite(fields.weight)) data.weight = fields.weight;
    if (typeof fields.description === 'string' && fields.description) data.description = fields.description;
...
// Mirrors the JSON backend's method of the same name (#717) — see it for why
// values only accrete and the attempt is stamped unconditionally. mergeData
...
async function setGameProviderInfo(tenant, rid, gid, info) {
  const patch = { providerInfoAt: new Date().toISOString() };
  if (Number.isFinite(info.weight)) patch.weight = info.weight;
  if (typeof info.description === 'string' && info.description) patch.description = info.description;
  const rows = await qt(tenant, (trx) =>
    trx('games').where({ id: gid, round_id: rid, tenant_id: tenant }).update({ data: mergeData(patch) }).returning(['id', 'data']));
  return rows[0] ? withId(rows[0]) : null;
}
