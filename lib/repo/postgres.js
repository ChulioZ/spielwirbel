...
// background is always present (may be null); recommendationRuns and tags only
// when they have ever been written (matches the JSON model, where those keys
// are absent until saveRecommendationRuns/addTag run). tenant_id is scoping
...
const ROUND_OBJ = "jsonb_build_object('id', id, 'name', name, 'background', background, 'recommendation_runs', recommendation_runs, 'tags', tags)";
const READ_SQL = {
  // bindings: [tenant]
  list: `
  ...
  return tx(tenant, async (trx) => {
    ...
    if (importFromRoundId) {
      ...
      for (const row of src) {
        const gid = newId();
        const data = { title: row.data.title, image: row.data.image, retired: false, retiredAt: null };
    ...
    const round = await trx('rounds').where({ id: rid }).select('id', 'name', 'background', 'recommendation_runs', 'tags');
