...
// always present, tags/lastSessionFilters only when ever written.
...
async function publicGameAggregates(now = new Date().toISOString()) {
  ...
  return atx(async (trx) => {
    ...
    const ratings = await trx({ s: 'sessions' })
      ...
      .whereRaw('g.round_id = s.round_id')
      .whereRaw("jsonb_typeof(vote.val->'rating') = 'number'")
      .whereRaw(`g.data->'source'->>'externalId' is not null`)
      .whereRaw(`g.data->'source'->>'provider' is not null`)
      .whereRaw('s.tenant_id not like ?', [demoLike])
      .groupByRaw(`g.data->'source'->>'provider', g.data->'source'->>'externalId'`)
      .select(
        ...
        trx.raw(`sum((vote.val->>'rating')::float8) as rating_sum`),
...
async function listCorpusPending(limit, staleBefore) {
  const rows = await knex('bgg_corpus')
    .where((b) => b
      ...
      // carries the reasoning for why this tests KEY ABSENCE and not a null
      // value. Change one, change both.
      //
      ...
      // verbatim, so the three ANDed terms would only group correctly by
      // operator precedence — and a future edit adding an OR inside would
      // silently widen the whole queue to every row.
      .orWhereRaw("(data->'info' is not null and data->'info' <> 'null'::jsonb and not (data->'info' \\? 'imageUrl'))"))
