 ...
 *   rounds(id, tenant_id, name, background jsonb, marker smallint, tags jsonb,
 *          dismissed_recommendations jsonb, saved_filters jsonb)
...
const { ERASURE_ACTIONS } = require('../erasure-actions');
...
// Which tenants the Funktionsnutzung card leaves out (#1174). Read per call —
// see the module header for why it must not be cached.
const { excludedTenants } = require('../admin-exclusions');
// The design tile's tally (#1201), shared with the JSON backend — one fold, so
// the two cannot resolve a stored design differently.
const { tallyDesignAdoption } = require('../account-design');

// Calendar week/month/year boundaries for publicGameAggregates (#964) — the
// same derivation the JSON backend uses, so the two cannot disagree.
...
const { assignProviderInfo } = require('../../public/js/provider-info-fields');
const { importGame, copyGame, importTags, mergeTagsInto } = require('./import-copy');
// The saved-filter list operations (#1328) — the same functions the JSON backend
// runs, here inside the FOR UPDATE that holds the list.
const {
  insertSavedFilter, renameSavedFilter, removeSavedFilter, permuteSavedFilters,
} = require('./saved-filters');
// See the JSON backend's deleteMember: the palette is needed to freeze the
// colours a removed row would otherwise shift.
...
// background and marker are always present (may be null); tags,
// lastSessionFilters and dismissedRecommendations only when they have ever been
// written (matches the
...
function assemble(round, children) {
  ...
  if (round.savedFilters != null) out.savedFilters = round.savedFilters;
  return out;
}
...
const ROUND_OBJ = "jsonb_build_object('id', id, 'name', name, 'background', background, 'marker', marker, 'tags', tags, 'lastSessionFilters', last_session_filters, 'dismissedRecommendations', dismissed_recommendations, 'savedFilters', saved_filters)";
...
const ROUND_META_OBJ = "jsonb_build_object('id', id, 'name', name, 'background', background, 'marker', marker, 'tags', tags, 'lastSessionFilters', last_session_filters, 'savedFilters', saved_filters)";
...
const SUMMARY_OBJ = `jsonb_build_object(
    ...
    'members', (
      SELECT coalesce(jsonb_agg(
        jsonb_build_object('id', m.id, 'name', m.data->'name')
          ...
          || (CASE WHEN jsonb_exists(m.data, 'userId')
                THEN jsonb_build_object('userId', m.data->'userId') ELSE '{}'::jsonb END)
    ...
    'lastPlayed', (
      ...
      LIMIT 1),
    /* The open sessions the home resume zone offers (#842), newest first and
       capped at the JSON backend's OPEN_SESSIONS_CAP — restated as a bare
       LIMIT because SQL cannot require that constant.

       This is the shape .claude/rules/shared-constants-across-the-stack.md
       calls out for the vote-score corpus aggregate: the predicate lives in
       public/js/session-outcome.js, the JSON backend requires it, and this side
       has to say the same thing by hand. Two clauses are load-bearing:

       (No backticks in here: this comment is inside a template literal.)

       - the split-parent exclusion tests jsonb_typeof(...) = 'array', which is
         the exact equivalent of the JS Array.isArray — a plain
         coalesce(..., '[]') would ERROR on a non-array value rather than
         reading it as "no children", and a legacy session carries no key at all;
       - the game join is gated on done, so a 'voting' ticket carries a null
         title and cover BY CONSTRUCTION, exactly as the JSON backend's
         s.done && s.chosenGameId guard does. The draw stays secret until
         everyone has rated. */
    'openSessions', (
      SELECT coalesce(jsonb_agg(
        jsonb_build_object(
          'id', s.id,
          'stage', CASE WHEN (s.data->>'done')::boolean IS TRUE
            THEN 'results' ELSE 'voting' END,
          'at', s.data->'createdAt',
          'gameTitle', g.data->'title',
          'image', g.data->'image')
        ORDER BY s.data->>'createdAt' DESC, s.seq ASC), '[]'::jsonb)
      FROM sessions s
      LEFT JOIN games g
        ON (s.data->>'done')::boolean IS TRUE
        AND g.id = s.data->>'chosenGameId'
        AND g.round_id = r.id AND g.tenant_id = _t.v
      WHERE s.round_id = r.id AND s.tenant_id = _t.v
        AND s.id IN (
          SELECT s2.id FROM sessions s2
          WHERE s2.round_id = r.id AND s2.tenant_id = _t.v
            AND (s2.data->>'finished')::boolean IS NOT TRUE
            AND (s2.data->>'cancelled')::boolean IS NOT TRUE
            AND jsonb_array_length(CASE
                  WHEN jsonb_typeof(s2.data->'childSessionIds') = 'array'
                    THEN s2.data->'childSessionIds' ELSE '[]'::jsonb END) = 0
          ORDER BY s2.data->>'createdAt' DESC, s2.seq ASC
          LIMIT 3))
  )`;
const READ_SQL = {
  // bindings: [tenant]
...
// otherwise — background and marker always present, tags/lastSessionFilters
// only when ever written.
...
async function createRound(tenant, { name, members, owner, importFromRoundId, limits }) {
  return tx(tenant, async (trx) => {
    const rid = newId();
    await trx('rounds').insert({
      // Stamped at creation from the id and stored, never derived on read —
      // see createRound in the JSON backend for why (#1187).
      id: rid, tenant_id: tenant, name, background: null, marker: markerIndexFromId(rid),
    });
    if (owner) {
    ...
    if (importFromRoundId) {
      // Active games only, with the full copy field set (#921) — and only from a
      // round of the same tenant. The source round's own tag list is read the
      // same way, so a tenant-crossing id yields no row and copies nothing.
      const srcRound = await trx('rounds')
        .where({ id: importFromRoundId, tenant_id: tenant })
        .first('tags');
      const src = await trx('games')
        ...
        .orderBy('seq')
        .select('data');
      const importing = src.map((row) => row.data);
      const { tags, remap } = importTags(srcRound && srcRound.tags, importing, limits, newId);
      // `tags` is an ARRAY — J() it (see the header note on the JSONB footgun).
      // Only written when tags were created, so a round whose imported games
      // carry none keeps the column NULL (absent-key parity).
      if (tags.length) {
        await trx('rounds').where({ id: rid, tenant_id: tenant }).update({ tags: J(tags) });
      }
      for (const game of importing) {
        const data = importGame(game, newId(), remap, newId);
        const { id: gid, ...rest } = data;
        await trx('games').insert({ id: gid, round_id: rid, tenant_id: tenant, data: J(rest) });
        await addActivity(trx, tenant, rid, 'game_added', { gameId: gid, title: rest.title });
      }
    }
    // Aliased so assemble() sees the same camelCase key ROUND_OBJ builds.
    const round = await trx('rounds')
      .where({ id: rid })
      .select('id', 'name', 'background', 'marker', 'tags', 'last_session_filters as lastSessionFilters',
        'dismissed_recommendations as dismissedRecommendations', 'saved_filters as savedFilters');
...
// Batch id -> profile-picture path (#841). See the JSON backend for why an
// unknown id is simply absent rather than an error.
async function getUserAvatars(ids) {
  const want = [...new Set(ids || [])];
  if (!want.length) return {};
  const rows = await knex('users').whereIn('id', want).select('id', 'data');
  const out = {};
  for (const r of rows) out[r.id] = r.data.avatar || null;
  return out;
}
...
// See the JSON backend for the contract, `kind`, and why the blank guard is
...
async function findImageOwner(image) {
  ...
  const rows = await atx((trx) =>
    trx('games')
      ...
      .first());
  if (!rows) {
    // #841: not a cover — it may be an account's profile picture. `users` is
    // global and un-scoped (no RLS), so this needs neither atx() nor a tenant
    // transaction; the base instance is the whole story.
    const u = await knex('users').whereRaw("data->>'avatar' = ?", [image]).select('id', 'data').first();
    if (!u) return null;
    return {
      kind: 'account',
      image,
      tenantId: u.data.tenantId || null,
      userId: u.id,
...
async function takedownImage(image) {
  ...
  // #841: the same path may instead be an account's profile picture. NOTE the
  // missing tenant transaction is deliberate and not an oversight next to the
  // per-tenant loop above: `users` is global with no RLS policy at all, so
  // there is no tenant to set and qt() would have nothing to scope. Counted
  // into the same total — "references cleared", not "games changed".
  const avatars = await knex('users')
    .whereRaw("data->>'avatar' = ?", [image])
    .update({ data: mergeData({ avatar: null }) })
    .returning('id');
  cleared += avatars.length;
  return cleared;
}

/* ------------------- Cover re-encode backfill (#867) ----------------------- */
/*
 * See the JSON backend for the contract and for why this pair is GAME COVERS
 * ONLY where takedownImage also reaches avatars.
 *
 * The read/write split is takedownImage's, for takedownImage's reason: the
 * admin policy is FOR SELECT, so the cross-tenant READ runs under atx(), and
 * the WRITE goes through the ordinary tenant-scoped qt() path. A rewrite
 * attempted under the escape would match zero rows and report a success that
 * changed nothing.
 */

async function listGameImages() {
  const rows = await atx((trx) =>
    trx('games').whereRaw("data->>'image' is not null").distinct(knex.raw("data->>'image' as image")));
  return rows.map((r) => r.image).filter(Boolean);
}

/* Every stored path the app still references (#941) — game covers plus account
   profile pictures, as one union. See the JSON backend for why it is one method
   and not two.

   The games half runs under atx() (it is cross-tenant by nature, like
   listGameImages); `users` is un-scoped, so a plain read is correct there. */
async function referencedImages() {
  const out = new Set(await listGameImages());
  const rows = await knex('users')
    .whereRaw("coalesce(data->>'avatar', '') <> ''")
    .select(knex.raw("data->>'avatar' as avatar"));
  for (const r of rows) if (r.avatar) out.add(r.avatar);
  return [...out];
}

// How much room the database takes (#941). Global, absent from TENANT_METHODS
// like every operator-only method — and worth having: the Railway volume was
// still at the 0.5 GB trial ceiling as of the 2026-08-04 settings review.
async function databaseSize() {
  const row = await knex.raw('select pg_database_size(current_database())::bigint as n');
  const n = row && row.rows && row.rows[0] ? Number(row.rows[0].n) : NaN;
...
async function roundContent(roundId) {
  return atx(async (trx) => {
    const round = await trx('rounds').where({ id: roundId })
      .first('id', 'name', 'tenant_id', 'tags', 'saved_filters as savedFilters');
    if (!round) return null;
    ...
    return {
      ...
      // See the JSON backend (#1328).
      filters: (round.savedFilters || []).map((f) => ({ id: f.id, name: f.name })),
    };
  });
}
...
async function redactText({ kind, roundId, id: targetId }, replacement) {
  ...
  return tx(tenant, async (trx) => {
    ...
    if (kind === 'filter') {
      // A saved filter's name (#1328): the same read-modify-write as a tag —
      // `saved_filters` is one jsonb array, so J() it.
      const row = await trx('rounds').where({ id: roundId, tenant_id: tenant })
        .first('saved_filters as savedFilters');
      const list = (row && row.savedFilters) || [];
      const hit = list.find((f) => f.id === targetId);
      if (!hit) return null;
      await trx('rounds').where({ id: roundId, tenant_id: tenant })
        .update({ saved_filters: J(list.map((f) => (f.id === targetId ? { ...f, name: replacement } : f))) });
      return done(targetId, hit.name);
    }
 ...
 * from everything; no `now`, since #1124 left it with no date window at all).
...
async function instanceMetrics() {
  ...
  /* #1174 — the tenants the Funktionsnutzung card leaves out, and the predicate
     every adoption figure carries. See lib/repo/json.js's instanceMetrics for
     WHY the exclusion reaches `adoption`, `designAdoption` and `social` but never
     `content`, `rounds`, `accounts` or `peaks`; this comment covers only what is
     Postgres-specific.

     ONE JSON STRING BINDING, and the obvious alternative is SILENTLY WRONG.
     Knex does not expand an array binding for a raw `?` — it hands the array to
     node-pg, which serializes it as a Postgres array LITERAL, so the comparison
     is against one string rather than against a list. Measured on PG 18:

       select 'x' not in (?)                                     -- ['x'] -> TRUE
       select 'x' <> ALL(?)                                      -- ['x'] -> false
       select 'x' not in (select jsonb_array_elements_text(?))    -- '["x"]' -> false

     The first form keeps the row it was written to exclude, with no error — so
     the card would quietly report the operator's own tenants after all, and only
     on the one instance that set the variable. `<> ALL(?)` is correct too; the
     jsonb form is used because it reads identically in a WHERE and inside a
     FILTER, which is where most of these sit.

     An EMPTY list makes the subquery empty, so the predicate is TRUE for
     everybody — deliberately the same SQL the excluding path runs, so CI
     exercises the real expression rather than a `true` shortcut it would never
     take in anger. */
  const EXCL_LIST = excludedTenants();
  const EXCL = JSON.stringify(EXCL_LIST);
  // The same list as a Set, for the one figure reduced in JS rather than in SQL
  // (the per-tenant round counts, which are already fetched row-per-tenant).
  const exclSet = new Set(EXCL_LIST);
  const inExcl = (col) => `${col} in (select jsonb_array_elements_text(?::jsonb))`;
  const kept = (col) => `${col} not in (select jsonb_array_elements_text(?::jsonb))`;
  const keptTenant = kept('tenant_id');
  const keptUser = kept("coalesce(data->>'tenantId', '')");

  /* The funnel's predicates, restated from the JS the JSON backend requires
     (public/js/session-outcome.js + its two local helpers). Each one mirrors a
     single line there; the repo contract test compares the two backends' output
     for the same seeded data, which is what keeps them honest.

     `jsonb_typeof` before `jsonb_array_length` everywhere, for the reason the
     ownerIds filter below carries: the key is ABSENT on an ordinary session, and
     a legacy row holding a non-array would ERROR the whole query rather than
     being skipped. */
  const arr = (key) => `case when jsonb_typeof(data->'${key}') = 'array'
    then data->'${key}' else '[]'::jsonb end`;
  // A split parent is not counted at all — its tables are (sessionChildIds).
  const NOT_SPLIT = `jsonb_array_length(${arr('childSessionIds')}) = 0`;
  // Two PEOPLE with at least one rating. A person key alone is a seat that was
  // asked and never answered, which is why the empty object is excluded.
  const RATERS = `(select count(*) from jsonb_each(
    case when jsonb_typeof(data->'votes') = 'object' then data->'votes' else '{}'::jsonb end
  ) as v(person, games) where jsonb_typeof(v.games) = 'object' and v.games <> '{}'::jsonb)`;
  /* sessionEnding(): only a PLAYED session (not split, not cancelled, finished)
     can have recorded how it ended, and the endings are an ALLOWLIST — an
     unknown value reads as unrecorded, exactly as the shared module does. */
  const HAS_RESULT = `coalesce(data->>'cancelled', '') <> 'true'
    AND data->>'finished' = 'true'
    AND (jsonb_array_length(${arr('winnerIds')}) > 0
         OR data->>'ending' IN ('lost', 'noWinner', 'ongoing'))`;
  ...
  const accounts = await knex('users')
    ...
    .select(
      ...
      /* `coalesce(…, '') <> ''`, never `is not null`: the takedown path clears
         an avatar to JSON null, and the JSON backend's `!!u.avatar` is falsy for
         null, undefined AND the empty string. `is not null` would count a
         cleared-to-'' row that the other backend does not, and the two would
         report different numbers from the same data (#941). */
      knex.raw("count(*) FILTER (WHERE coalesce(data->>'avatar', '') <> '')::int as with_avatar"),
      /* Containment, not a scan of the array: `identities` holds one entry per
         credential and `@>` asks "is there an element with this type", which is
         the JSON backend's `.some(i => i.type === 'passkey')` exactly. A NULL
         `identities` yields NULL rather than false, which FILTER drops — the
         same direction the JSON side's Array.isArray guard fails in. */
      knex.raw(`count(*) FILTER (
        WHERE data->'identities' @> '[{"type":"passkey"}]'::jsonb
          AND ${keptUser})::int as with_passkey`, [EXCL]),
      knex.raw(`count(*) FILTER (
        WHERE coalesce(data->>'bggUsername', '') <> ''
          AND ${keptUser})::int as with_bgg`, [EXCL]),
      /* The adoption block's own account figures (#1174): its denominator, the
         Konto-Bild share the card divides (the instance-wide `with_avatar`
         above stays where it is, in the accounts block), and the BG Stats
         opt-in — which is OFF at registration, so this counts real choices. */
      knex.raw(`count(*) FILTER (WHERE ${keptUser})::int as adoption_total`, [EXCL]),
      knex.raw(`count(*) FILTER (
        WHERE coalesce(data->>'avatar', '') <> ''
          AND ${keptUser})::int as adoption_with_avatar`, [EXCL]),
      knex.raw(`count(*) FILTER (
        WHERE data->>'bgStats' = 'true'
          AND ${keptUser})::int as with_bgstats`, [EXCL]),
    )
    .first();

  const keptOwner = kept("coalesce(data->>'ownerTenantId', '')");

  const shared = await knex('round_grants')
    .whereRaw("coalesce(data->>'ownerTenantId', '') not like ?", [demoLike])
    .whereRaw(keptOwner, [EXCL])
  ...
  const friendships = await knex('friendships')
    ...
    // …and the same for an EXCLUDED tenant's accounts, through the same join —
    // the JSON backend builds a Set of their ids for exactly this reason.
    .whereNotExists((qb) => qb
      .select(knex.raw('1'))
      .from('users')
      .whereRaw('users.id in (friendships.user_lo, friendships.user_hi)')
      .whereRaw(inExcl("coalesce(users.data->>'tenantId', '')"), [EXCL]))
    .count('* as n')
    .first();

  /* PLAIN knex, NOT atx() — and that is a property of the table, not an
     oversight. `session_vote_links` is deliberately not tenant-scoped and not
     under RLS (its own migration says why: the caller of /vote/:token is not
     authenticated, so there is no app.tenant_id to scope by, and an RLS-scoped
     lookup would match zero rows on every request). It therefore carries its
     tenant inside `data` rather than as a column, which is what the demo filter
     reads. Verified against lib/repo/migrations/20260805090000_session_vote_links.js.

     countDistinct over the PAIR, matching the JSON backend: the unique index on
     (round_id, session_id) already makes minting idempotent, so this is the same
     number either way — but counting it the same way on both sides means a stray
     row could not make them disagree. */
  const voteLinks = await knex('session_vote_links')
    .whereRaw("coalesce(data->>'tenantId', '') not like ?", [demoLike])
    .whereRaw(kept("coalesce(data->>'tenantId', '')"), [EXCL])
    .select(knex.raw('count(distinct (round_id, session_id))::int as n'))
    .first();

  /* Which design accounts wear (#1201). PLAIN knex: `users` carries no RLS
     (lib/repo/migrations/20260719000000_initial_schema.js lists the five RLS
     tables, and no later migration adds it), exactly like the account figures
     above.

     Grouped by the RAW stored pair and folded in JS rather than resolved in
     SQL, because the resolution is the registry's `enabled` gate against
     NODE_ENV — lib/account-design.js, the function /me answers with. Restating
     that in SQL would be a second copy of the gate, i.e. a card that could
     report an account on a design its owner is not shown. The raw keys never
     leave this function: tallyDesignAdoption keys its output by offered id only.

     `data->'designSwitchedBack' = 'true'::jsonb` matches the JSON boolean
     alone, which is the other backend's `=== true` exactly; `->> = 'true'`
     would also count a stored STRING 'true'.

     `answered` is whether the chooser was ever answered (#1202): the fold needs
     it to resolve the flip's lazy default, under which an account that never
     answered wears the face whatever it stores. `->>` reads a JSON null as SQL
     NULL, so coalescing to '' and testing for non-empty is the other backend's
     `Boolean(u.designChooserSeen)` exactly. */
  const designRows = await knex('users')
    .whereRaw("coalesce(data->>'tenantId', '') not like ?", [demoLike])
    .whereRaw(keptUser, [EXCL])
    .select(
      knex.raw("data->>'design' as design"),
      knex.raw("coalesce(data->>'designChooserSeen', '') <> '' as answered"),
      knex.raw("coalesce(data->'designSwitchedBack' = 'true'::jsonb, false) as switched"),
      knex.raw('count(*)::int as n'),
    )
    .groupByRaw("data->>'design', coalesce(data->>'designChooserSeen', '') <> '', "
      + "coalesce(data->'designSwitchedBack' = 'true'::jsonb, false)");
  ...
  const rounds = await atx(async (trx) => {
    ...
    const perTenant = await trx('rounds')
      ...
      .groupBy('tenant_id')
      // The tenant rides along so the ADOPTION round total is reduced out of
      // this same read rather than fetched again — one scan, so the card's
      // denominator cannot disagree with the peak beside it about which rounds
      // were counted.
      .select('tenant_id', trx.raw('count(*)::int as n'));
      ...
      .select(
        ...
        trx.raw(`count(*) FILTER (
          WHERE (data->>'retired')::boolean IS NOT TRUE
            AND (data->>'completed')::boolean IS NOT TRUE
            AND (data->>'wish')::boolean IS NOT TRUE)::int as active`),
        // A provider LINK, not a cover — a hand-typed game can carry a pasted
        // image and a linked one can have had its cover cleared.
        trx.raw(`count(*) FILTER (WHERE ${keptTenant})::int as adoption_n`, [EXCL]),
        trx.raw(`count(*) FILTER (
          WHERE coalesce(data->'source'->>'provider', '') <> ''
            AND ${keptTenant})::int as linked`, [EXCL]),
        /* The two cover kinds are mutually exclusive: `/uploads/` is bytes this
           instance stores, anything else is hotlinked (#172). `like` needs no
           ESCAPE here — the pattern is a literal with one trailing wildcard. */
        trx.raw(`count(*) FILTER (
          WHERE coalesce(data->>'image', '') like '/uploads/%'
            AND ${keptTenant})::int as own_cover`, [EXCL]),
        trx.raw(`count(*) FILTER (
          WHERE coalesce(data->>'image', '') <> ''
            AND coalesce(data->>'image', '') not like '/uploads/%'
            AND ${keptTenant})::int as provider_cover`, [EXCL]),
        /* jsonb_typeof before jsonb_array_length: the key is ABSENT on a game
           that never used the feature, and jsonb_array_length(NULL) is NULL
           rather than 0 — which FILTER drops, but a legacy row holding something
           that is not an array would ERROR the whole query. */
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(${arr('ownerIds')}) > 0
            AND ${keptTenant})::int as with_owners`, [EXCL]),
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(${arr('expansions')}) > 0
            AND ${keptTenant})::int as with_expansions`, [EXCL]),
      )
      .first();
    ...
    const tags = await trx('rounds')
      .whereRaw('tenant_id not like ?', [demoLike])
      .select(
        trx.raw("coalesce(max(jsonb_array_length(coalesce(tags, '[]'::jsonb))), 0)::int as n"),
        // The adoption half of the same scan: how many rounds define a tag at
        // all (#238), which the fullest-round maximum beside it cannot answer.
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(coalesce(tags, '[]'::jsonb)) > 0
            AND ${keptTenant})::int as rounds_with`, [EXCL]),
        // Rounds with at least one saved filter (#1328) — same scan, same gate.
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(coalesce(saved_filters, '[]'::jsonb)) > 0
            AND ${keptTenant})::int as saved_with`, [EXCL]),
      )
      .first();
    ...
    const sessions = await trx('sessions')
      ...
      .select(
        ...
        // `guests` and `teams` are ABSENT keys on an ordinary session by design
        // (#532/#575) — see the ownerIds note above for why the typeof guard is
        // not optional.
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(${arr('guests')}) > 0
            AND ${keptTenant})::int as with_guests`, [EXCL]),
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(${arr('teams')}) > 0
            AND ${keptTenant})::int as with_teams`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant})::int as adoption_total`, [EXCL]),
        /* The funnel (#1174). Seven INDEPENDENT shares of `started`, not a
           shrinking pipeline: a direct-pick session is born closed and chosen
           with no votes at all, so „bewertet" is not a predecessor of
           „Abstimmung beendet". Every line restates one branch of the JSON
           backend's walk; the repo contract compares the two. */
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT})::int as f_started`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT} AND ${RATERS} >= 2)::int as f_rated`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT} AND data->>'done' = 'true')::int as f_closed`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT}
          AND coalesce(data->>'chosenGameId', '') <> '')::int as f_chosen`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT} AND data->>'finished' = 'true')::int as f_played`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT} AND ${HAS_RESULT})::int as f_result`, [EXCL]),
        trx.raw(`count(*) FILTER (WHERE ${keptTenant}
          AND ${NOT_SPLIT} AND data->>'cancelled' = 'true')::int as f_cancelled`, [EXCL]),
      )
      .first();

    /* Rounds by how many sessions they FINISHED. Grouped per round and then
       banded, because the question is about rounds rather than sessions.

       Only `one` and `many` come from here: a round that never held a session
       has no row in this group-by at all, and one whose sessions all went
       unfinished lands in neither band — both are „keine", which is therefore
       derived by subtraction from the adoption round total. Counting `none`
       here instead would silently drop the first kind, which is precisely the
       round the tile exists to count. */
    const finishedBands = await trx
      .select(
        trx.raw('count(*) FILTER (WHERE n = 1)::int as one'),
        trx.raw('count(*) FILTER (WHERE n >= 2)::int as many'),
      )
      .from(trx('sessions')
        .whereRaw('tenant_id not like ?', [demoLike])
        .whereRaw(keptTenant, [EXCL])
        .groupBy('round_id')
        .select(trx.raw("count(*) FILTER (WHERE data->>'finished' = 'true')::int as n"))
        .as('per_round'))
      .first();

    /* Accounts that never started anything (#1174). Inside atx() because it
       reads `rounds`, which is RLS-scoped — a plain query would match zero rows
       and report that EVERY account is roundless, with no error and a perfectly
       plausible-looking number (.claude/rules/admin-kennzahlen-card.md). `users`
       carries no RLS, so joining it in here costs nothing.

       The tenant fallback mirrors the JSON backend's `tenantOf`: a legacy row
       with no tenantId belongs to 'default', and nullif catches the empty
       string that coalesce alone would keep. */
    const roundless = await trx('users')
      .whereRaw("coalesce(data->>'tenantId', '') not like ?", [demoLike])
      .whereRaw(keptUser, [EXCL])
      .whereNotExists((qb) => qb
        .select(trx.raw('1'))
        .from('rounds')
        .whereRaw("rounds.tenant_id = coalesce(nullif(users.data->>'tenantId', ''), 'default')"))
      .select(trx.raw('count(*)::int as n'))
      .first();

    // Rounds USING each shelf state — "do groups reach for the archive and the
    // wishlist at all", which the games totals above do not answer.
    const shelfUse = await trx('games')
      .whereRaw('tenant_id not like ?', [demoLike])
      .select(
        trx.raw(`count(distinct round_id) FILTER (
          WHERE data->>'retired' = 'true' AND ${keptTenant})::int as retired`, [EXCL]),
        trx.raw(`count(distinct round_id) FILTER (
          WHERE data->>'completed' = 'true' AND ${keptTenant})::int as completed`, [EXCL]),
        trx.raw(`count(distinct round_id) FILTER (
          WHERE data->>'wish' = 'true' AND ${keptTenant})::int as wish`, [EXCL]),
        /* The UNION, which the three above cannot be summed into: a round with
           an archived game AND a wish is one round using the shelf, not two. */
        trx.raw(`count(distinct round_id) FILTER (
          WHERE (data->>'retired' = 'true'
             OR data->>'completed' = 'true'
             OR data->>'wish' = 'true')
            AND ${keptTenant})::int as any_shelf`, [EXCL]),
      )
      .first();
    ...
    return {
      ...
      roundCounts: perTenant.reduce(
        (acc, r) => ({
          total: acc.total + r.n,
          max: Math.max(acc.max, r.n),
          adoption: acc.adoption + (exclSet.has(r.tenant_id) ? 0 : r.n),
        }),
        { total: 0, max: 0, adoption: 0 },
      ),
      games: games.n,
      adoptionGames: games.adoption_n,
      finishedBands,
      roundless: roundless.n,
      activeGames: games.active,
      gamesLinked: games.linked,
      gamesWithOwnCover: games.own_cover,
      gamesWithProviderCover: games.provider_cover,
      gamesWithOwners: games.with_owners,
      gamesWithExpansions: games.with_expansions,
      members: members.n,
      maxGamesPerRound: fullestRound ? fullestRound.n : 0,
      maxTagsPerRound: tags.n,
      roundsWithTags: tags.rounds_with,
      roundsWithSavedFilters: tags.saved_with,
      sessions,
      shelfUse,
    };
  });
  ...
  return {
    accounts: {
      ...
      withAvatar: accounts.with_avatar,
    },
    rounds: {
      total: rounds.roundCounts.total,
    },
    /* `activeGames`, `members` and `sessionsFinished` are no longer on the
       operator card (#1124) but are NOT dead — lib/public-stats.js reads exactly
       these three for the public landing counters (#564), where an absent field
       fails its threshold and blanks the block with no error. */
    content: {
      games: rounds.games,
    ...
    adoption: {
      roundsWithRetired: rounds.shelfUse.retired,
      roundsWithCompleted: rounds.shelfUse.completed,
      roundsWithWish: rounds.shelfUse.wish,
      roundsWithAnyShelf: rounds.shelfUse.any_shelf,
      roundsWithTags: rounds.roundsWithTags,
      roundsWithSavedFilters: rounds.roundsWithSavedFilters,
      gamesLinked: rounds.gamesLinked,
      gamesWithOwnCover: rounds.gamesWithOwnCover,
      gamesWithProviderCover: rounds.gamesWithProviderCover,
      gamesWithOwners: rounds.gamesWithOwners,
      gamesWithExpansions: rounds.gamesWithExpansions,
      sessionsWithGuests: rounds.sessions.with_guests,
      sessionsWithTeams: rounds.sessions.with_teams,
      sessionsWithVoteLink: voteLinks.n,
      accountsWithPasskey: accounts.with_passkey,
      accountsWithBggUsername: accounts.with_bgg,
      // The card's own denominators and the two new account figures — see the
      // JSON backend for what each one means and why it is not `accounts.total`.
      accountsTotal: accounts.adoption_total,
      roundsTotal: rounds.roundCounts.adoption,
      gamesTotal: rounds.adoptionGames,
      sessionsTotal: rounds.sessions.adoption_total,
      accountsWithAvatar: accounts.adoption_with_avatar,
      accountsWithoutRound: rounds.roundless,
      accountsWithBgStats: accounts.with_bgstats,
      funnel: {
        started: rounds.sessions.f_started,
        rated: rounds.sessions.f_rated,
        closed: rounds.sessions.f_closed,
        chosen: rounds.sessions.f_chosen,
        played: rounds.sessions.f_played,
        result: rounds.sessions.f_result,
        cancelled: rounds.sessions.f_cancelled,
      },
      roundsByFinished: {
        // Derived, not counted — see the finishedBands query for why `none`
        // cannot come out of the group-by.
        none: rounds.roundCounts.adoption
          - rounds.finishedBands.one - rounds.finishedBands.many,
        one: rounds.finishedBands.one,
        many: rounds.finishedBands.many,
      },
    },
    designAdoption: tallyDesignAdoption(designRows),
    social: {
...
async function publicGameAggregates(now = new Date().toISOString()) {
  /* CALENDAR periods on the Europe/Berlin clock (#964), taken from the SHARED
     helper rather than from date_trunc('week' | 'month' | 'year', now AT TIME
     ZONE 'Europe/Berlin'). Postgres would do it natively and Monday-first, but
     then the two backends would hold two independent implementations of one
     boundary — and test/support/repo-contract.js asserts they agree to the hour
     across both DST transitions. One derivation, handed to both, cannot drift. */
  const period = periodBoundaries(now);
  const demoLike = `${DEMO_TENANT_PREFIX}%`;
  ...
  return atx(async (trx) => {
    ...
    // A play is a session joined to the game it settled on. The join is
    ...
    //
    // Dated by `createdAt`, matching the JSON backend and every other surface:
    // `finishedAt` is rewritten on every winner-chip re-POST, so bucketing on it
    // moved an old play into the current period (#1059).
    //
    // TWO SETS SHARE THIS READ (#1329). The period and all-time counts are
    // FINISHED plays — "most played" tiles, dated by `createdAt`. The `lift`
    // pair is the Regal's `playCounts` (public/js/vote-score.js): every
    // NON-CANCELLED session with a chosen game, finished or not, which is what
    // makes the Discover score's play lift the shelf's own. So the WHERE admits
    // the union and each count carries its own FILTER; a row can then carry
    // zero finished plays, which is exactly the JSON backend's shape.
    //
    // A split evening is not counted twice: its parent never carries a
    // `chosenGameId` (the routes refuse `/choice` on a split parent and `/split`
    // on a chosen one), so only the child tables count, once each.
    const playedAt = "coalesce(s.data->>'createdAt', '')";
    const finished = "s.data->>'finished' = 'true'";
    const liftable = "coalesce(s.data->>'cancelled', 'false') <> 'true'";
    const plays = await trx({ s: 'sessions' })
      .join({ g: 'games' }, 'g.id', trx.raw("s.data->>'chosenGameId'"))
      .whereRaw('g.round_id = s.round_id')
      .whereRaw(`(${finished} or ${liftable})`)
      ...
      .select(
        ...
        trx.raw(`count(*) FILTER (WHERE ${finished} and ${playedAt} >= ?)::int as plays_week`, [period.week]),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${finished} and ${playedAt} >= ?)::int as tenants_week`, [period.week]),
        trx.raw(`count(*) FILTER (WHERE ${finished} and ${playedAt} >= ?)::int as plays_month`, [period.month]),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${finished} and ${playedAt} >= ?)::int as tenants_month`, [period.month]),
        trx.raw(`count(*) FILTER (WHERE ${finished} and ${playedAt} >= ?)::int as plays_year`, [period.year]),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${finished} and ${playedAt} >= ?)::int as tenants_year`, [period.year]),
        // ALL-TIME finished plays, for the „Dauerbrenner" card (#1035). No
        // `createdAt` cutoff — the JSON backend spells the empty cutoff because
        // it shares one loop over its window map; `playedAt >= ''` admits every
        // row, so the two forms are the same set.
        trx.raw(`count(*) FILTER (WHERE ${finished})::int as plays_all`),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${finished})::int as tenants_all`),
        // The play LIFT (#1329): the Regal's definition, finished or not.
        trx.raw(`count(*) FILTER (WHERE ${liftable})::int as plays_lift`),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${liftable})::int as tenants_lift`),
      );
    ...
    const ratings = await trx({ s: 'sessions' })
      ...
      .whereRaw('g.round_id = s.round_id')
      /* The 1–5 TILE this vote landed on, or NULL when it is not a vote at all.
         This expression is the one place the vote SCALE has to be restated in
         SQL, because SQL cannot require public/js/vote-score.js
         (.claude/rules/shared-constants-across-the-stack.md); #914 collapsed a
         separate WHERE and CASE into it so the admission test and the tile are
         by construction the same decision, instead of two spellings of it.

         It carried a `retire`-wins arm between #797 and #909, while a
         retirement proposal was the zero of a 0–5 scale. #909 removed the
         option AND rewrote the stored votes
         (lib/repo/migrations/20260905120000_drop_retire_votes.js), so a vote is
         its rating and nothing else — which is why this reads `rating` alone
         rather than also ignoring a flag that no longer exists in the data.

         Nested CASEs, not an `AND`: Postgres may reorder the arms of a
         conjunction, so the `jsonb_typeof` check has to GUARD the cast rather
         than merely sit beside it. `in (1,2,3,4,5)` covers range and
         integrality at once — a stray 3.5 is not a tile the vote card can
         produce, and `tileValue` skips it on the JS side for the same reason,
         which is what keeps `count` equal to the buckets it totals. */
      .crossJoin(trx.raw(`lateral (select case
          when jsonb_typeof(vote.val->'rating') = 'number'
            then (case when (vote.val->>'rating')::float8 in (1,2,3,4,5)
                       then (vote.val->>'rating')::int end)
        end as tile) as t`))
      .whereRaw('t.tile is not null')
      .whereRaw(`g.data->'source'->>'externalId' is not null`)
      .whereRaw(`g.data->'source'->>'provider' is not null`)
      .whereRaw('s.tenant_id not like ?', [demoLike])
      .groupByRaw(`g.data->'source'->>'provider', g.data->'source'->>'externalId'`)
      .select(
        trx.raw(`g.data->'source'->>'provider' as provider`),
        trx.raw(`g.data->'source'->>'externalId' as external_id`),
        /* A per-tile HISTOGRAM, not a rating sum (#914). The sum was only ever
           useful for a raw mean, and the app ranks on the Spielwirbel-Score —
           whose curve lives in public/js/vote-score.js, which SQL cannot
           require(). Summing `TILE_VALUE` here would hand-restate six numbers
           that file documents as expected to be retuned, so a retune would move
           every screen except the public podium. A histogram carries no curve:
           lib/public-stats.js scores it through `scoreTally`, in JS, where the
           shared module is requirable
           (.claude/rules/shared-constants-across-the-stack.md). */
        // Indexed by rating, so `tile0` is a permanent zero — the same hole
        // vote-score.js's TILE_VALUE carries, kept for the same reason: every
        // consumer indexes the histogram by the rating itself.
        ...[0, 1, 2, 3, 4, 5].map((k) => trx.raw(`count(*) FILTER (WHERE t.tile = ?)::int as tile${k}`, [k])),
    ...
    const rowFor = (provider, externalId) => {
      ...
      if (!row) {
        row = {
          ...
          owners: 0,
          plays: {
            week: { count: 0, tenants: 0 },
            month: { count: 0, tenants: 0 },
            year: { count: 0, tenants: 0 },
            all: { count: 0, tenants: 0 },
            lift: { count: 0, tenants: 0 },
          },
          ratings: { tiles: [0, 0, 0, 0, 0, 0], count: 0, tenants: 0 },
        };
        rows.set(key, row);
      }
      return row;
    ...
    for (const r of plays) {
      const row = rowFor(r.provider, r.external_id);
      row.plays = {
        week: { count: r.plays_week, tenants: r.tenants_week },
        month: { count: r.plays_month, tenants: r.tenants_month },
        year: { count: r.plays_year, tenants: r.tenants_year },
        all: { count: r.plays_all, tenants: r.tenants_all },
        lift: { count: r.plays_lift, tenants: r.tenants_lift },
      };
    }
    for (const r of ratings) {
      const row = rowFor(r.provider, r.external_id);
      row.ratings = {
        tiles: [r.tile0, r.tile1, r.tile2, r.tile3, r.tile4, r.tile5],
        count: r.rating_count,
        tenants: r.rating_tenants,
      };
    }
    return [...rows.values()];
  });
...
async function eraseAccount(uid) {
  ...
  let rounds = 0;
  const images = new Set();
  // Collected from the row we already read, BEFORE the delete below — otherwise
  // the object is billable and unreachable forever (#841,
...
/* The 3-year retention purge (#311). Same contract as the JSON backend's: drop
   entries dated before `beforeIso`, keep every ERASURE_ACTIONS record regardless
   of age, and keep an entry that carries no `at` at all.

   `at` is compared as TEXT for the same reason `logFilter` below does it — it is
   always written as an ISO-8601 UTC string, which sorts lexicographically, and a
   cast would turn one malformed historical value into an error that fails the
   WHOLE purge rather than one row that simply doesn't match.

   `whereNotNull` on a JSON path needs the `->>` form too: a missing key yields
   SQL NULL, and `data->>'at' < ?` is already NULL (not true) for such a row, so
   the null-safety is implicit — the explicit clause is here to say so rather
   than to change the result. The table is un-scoped and carries no RLS, so this
   needs none of the `atx()`/tenant plumbing the round data does
   (.claude/rules/admin-moderation-surface.md). */
async function purgeModerationLog(beforeIso) {
  return knex('moderation_log')
    .whereRaw("data->>'at' < ?", [beforeIso])
    .whereNotIn(knex.raw("coalesce(data->>'action', '')"), ERASURE_ACTIONS)
    .del();
}
 ...
 * from lib/routes/contact.js, the read/decide side only from
 * lib/routes/admin/notices.js.
 */
...
// The corpus rows for a set of BGG ids (#829) — see the JSON backend for why
// this is the one corpus read keyed by externalId.
//
// Plain `knex`, no `tx()`/`qt()`: the table carries no `tenant_id`, so there is
// no `app.tenant_id` for a statement here to set (.claude/rules/tenancy-rls.md).
async function getCorpusEntries(externalIds) {
  const want = [...new Set((externalIds || []).map((x) => String(x)))];
  if (!want.length) return [];
  const rows = await knex('bgg_corpus')
    .whereIn('external_id', want)
    .select('external_id', 'rank', 'enriched_at', 'data');
  return rows.map(corpusRow);
}
...
// One set and one retention rule for both backends (#1357: age, not count).
const { FEED_EVENT_TYPES, badgeFeedFields, feedEventCutoff, feedEventCeiling } = require('../feed-events');
...
async function addFeedEvent(uid, event) {
  ...
  // See lib/repo/json.js: an Abzeichen (#1389) is key + tier, or nothing.
  const badge = event.type === 'badge_earned' ? badgeFeedFields(event) : null;
  if (event.type === 'badge_earned' && !badge) return null;
  const eid = newId();
  const data = {
    type: event.type,
    title: badge ? badge.title : String(event.title || ''),
    coverUrl: badge ? null : event.coverUrl || null,
    at: new Date().toISOString(),
  };
  if (badge) data.tier = badge.tier;
  // See lib/repo/json.js for why this is the only widening, why it is typed here,
  // and why the key is ABSENT rather than 0 for the other two types.
  if (event.type === 'games_imported' && Number.isInteger(event.count) && event.count >= 0) {
    data.count = event.count;
  }
  await knex('feed_events').insert({ id: eid, uid, data: J(data) });
  // Prune this user's expired events (#1357) — `at` compared as TEXT, like the
  // moderation-log purge: always a UTC ISO string, and a cast would turn one bad
  // row into an error for the whole write. The uid index narrows it first.
  await knex('feed_events').where({ uid }).whereRaw("data->>'at' < ?", [feedEventCutoff()]).del();
  // Then anything beyond the safety ceiling (seq order == age).
  await knex('feed_events').where({ uid }).whereNotIn('id',
    knex('feed_events').where({ uid }).orderBy('seq', 'desc').limit(feedEventCeiling()).select('id')).del();
  return { id: eid, uid, ...data };
}

// See lib/repo/json.js for the `before` cursor's contract. Here "strictly older"
// is `seq` below the cursor row's; the NOT EXISTS arm is the gone-row fallback
// (rows dated before it), in the same statement so there is no read-then-query
// race with a concurrent prune.
async function listFeedEvents(uids, limit = 100, before = null) {
  if (!uids || !uids.length) return [];
  const q = knex('feed_events').whereIn('uid', uids);
  if (before) {
    const cursorRow = () => knex('feed_events').where({ id: before.id });
    q.where((w) => w
      .where('seq', '<', cursorRow().select('seq'))
      .orWhere((o) => o.whereNotExists(cursorRow().select(knex.raw('1')))
        .andWhereRaw("data->>'at' < ?", [before.at])));
  }
  const rows = await q.orderBy('seq', 'desc').limit(limit).select('id', 'uid', 'data');
  return rows.map(feedRow);
}

// The retention sweep (#1357) — see lib/repo/json.js. A full scan over the JSONB
// `at` once per scheduler tick, deliberately without a new indexed column: the
// table is bounded by the age limit and the per-account ceiling, and feed_events
// is global and un-scoped (no RLS), so this DELETE sees every row.
async function purgeExpiredFeedEvents(beforeIso) {
  return knex('feed_events').whereRaw("data->>'at' < ?", [beforeIso]).del();
}
...
// Retire / restore a member (#1006). See the JSON backend for the contract and
// for why this touches nothing but the two flags.
async function retireMember(tenant, rid, mid, retired, actorMemberId) {
  return tx(tenant, async (trx) => {
    const patch = { retired, retiredAt: retired ? new Date().toISOString() : null };
    const rows = await trx('members').where({ id: mid, round_id: rid, tenant_id: tenant })
      .update({ data: mergeData(patch) }).returning(['id', 'data']);
    if (!rows[0]) return null;
    await addActivity(trx, tenant, rid, retired ? 'member_retired' : 'member_restored', {
...
/* Hard-delete a member (#1006). See the JSON backend for the colour freeze,
   which is the non-obvious half: a member's avatar colour is derived from their
   POSITION in the round's member list when they have not picked one, so removing
   a row silently re-colours everyone after it.

   The ORDER here is the row order the read side returns, which is what the
   frontend indexes into — `listMembers`/`getRound` order by the same key, so the
   positions frozen below are the positions the app actually paints. */
async function deleteMember(tenant, rid, mid, actorMemberId) {
  return tx(tenant, async (trx) => {
    // ORDER BY seq, not by id: `seq` is insertion order and is what every read
    // path uses (getRound, listMembers), so it is the order the frontend indexes
    // into when it derives an unset avatar colour. Ordering by the random hex id
    // instead put the freeze loop on a different sequence than the one being
    // painted — caught by the Postgres CI job, where the seat being deleted
    // happened to sort last and nothing was frozen at all.
    const rows = await trx('members').where({ round_id: rid, tenant_id: tenant })
      .orderBy('seq').select('id', 'data');
    const i = rows.findIndex((r) => r.id === mid);
    if (i < 0) return false;
    for (let idx = i + 1; idx < rows.length; idx++) {
      if (MEMBER_COLORS.includes(rows[idx].data.color)) continue;
      await trx('members').where({ id: rows[idx].id, round_id: rid, tenant_id: tenant })
        .update({ data: mergeData({ color: MEMBER_COLORS[idx % MEMBER_COLORS.length] }) });
    }
    await trx('members').where({ id: mid, round_id: rid, tenant_id: tenant }).del();
    await addActivity(trx, tenant, rid, 'member_deleted', { name: rows[i].data.name }, actorMemberId);
    return true;
  });
}
...
async function createGame(tenant, rid, fields, actorMemberId) {
  return tx(tenant, async (trx) => {
    ...
    // Which MEMBERS own the box (#971) — see the JSON backend. Absent when none,
    // so an unmarked shelf's rows keep their shape (absent-key parity).
    if (Array.isArray(fields.ownerIds) && fields.ownerIds.length) data.ownerIds = fields.ownerIds;
  ...
  return tx(tenant, async (trx) => {
    ...
    for (const fields of games) {
      ...
      fresh.push({
        ...
        // Who owns the box (#971) — one selection for the whole batch. See the
        // JSON backend.
        ...(Array.isArray(fields.ownerIds) && fields.ownerIds.length ? { ownerIds: fields.ownerIds } : {}),
...
// Mirrors the JSON backend's method of the same name — see it for why an EMPTY
// `ownerIds` is stored as absence rather than as `[]`.
//
// It needs different code here rather than a different literal: `mergeData` is
// `data || patch`, which can add or overwrite a key but never remove one, so the
// clear merges the patch first and then subtracts the key. Subtracting AFTER the
// merge is load-bearing — the patch may legitimately carry other fields in the
// same request (a title edit alongside the clear), and those must still land.
async function updateGame(tenant, rid, gid, patch) {
  const clearsOwners = Array.isArray(patch.ownerIds) && !patch.ownerIds.length;
  const data = clearsOwners
    ? knex.raw("(data || ?::jsonb) - 'ownerIds'", [J(patch)])
    : mergeData(patch);
  const rows = await qt(tenant, (trx) =>
    trx('games').where({ id: gid, round_id: rid, tenant_id: tenant }).update({ data }).returning(['id', 'data']));
  return rows[0] ? withId(rows[0]) : null;
}
...
// Mirrors the JSON backend's method of the same name (#829) — see it for why the
// batch shares one `providerInfoAt` and why a missing id is skipped.
//
// ONE transaction rather than one per game: the win here is the round trips, not
// a file rewrite. The statements run sequentially inside it because a
// transaction holds ONE connection and cannot fire them concurrently
// (.claude/rules/postgres-backend.md) — the same shape updateCorpusEntries uses.
async function setGameProviderInfoMany(tenant, rid, updates) {
  const list = updates || [];
  const at = new Date().toISOString();
  return qt(tenant, async (trx) => {
    // The round is checked EXPLICITLY, unlike the single-game writer, which gets
    // "unknown round" for free from its own UPDATE returning no row. A batch
    // cannot: an update matching nothing is indistinguishable from a round that
    // is gone, so without this an unknown round would answer `{ updated: 0 }`
    // while the JSON backend answers null -- a divergence only the Postgres half
    // of the contract suite could ever see.
    const round = await trx('rounds').where({ id: rid, tenant_id: tenant }).first('id');
    if (!round) return null;
    let updated = 0;
    for (const u of list) {
      const patch = assignProviderInfo({ providerInfoAt: at }, u.info);
      updated += await trx('games')
        .where({ id: u.gameId, round_id: rid, tenant_id: tenant })
        .update({ data: mergeData(patch) });
    }
    return { updated };
  });
}
...
// Mirrors the JSON backend's helper of the same name — see it for the contract
// (preserved id + addedAt on a re-save). Kept as a deliberate second copy like
// `sameSource`, with the shared contract suite as the guard against drift. The
// deep COPY onto fresh ids is shared instead, in ./import-copy.js (#921).
...
async function deleteGame(tenant, rid, gid, actorMemberId) {
  return tx(tenant, async (trx) => {
    ...
    // Silent for a wish — see the JSON backend (#863).
    if (!game.wish) {
...
// Bulk retire (#832). See the JSON backend for the contract — every id must name
// a game of this round, an already-archived game is skipped rather than refused,
// and one count-bearing Chronik entry stands in for N rows.
//
// The rows are read `forUpdate` before being written, the same lock discipline
// moveGames and acquireWishExpansion use: two concurrent bulk retires over
// overlapping selections then serialize instead of each merging its patch onto
// the pre-retire `data` and one losing.
async function retireGames(tenant, rid, gameIds, actorMemberId) {
  return tx(tenant, async (trx) => {
    ...
    const rows = await trx('games')
      ...
      .forUpdate()
      .select('id', 'data');
    if (rows.length !== want.length) return 'unknown_game';

    // Already-archived games are skipped, so the count reports what CHANGED.
    const todo = rows.filter((r) => !r.data.retired).map((r) => r.id);
    if (todo.length) {
      await trx('games').where({ round_id: rid, tenant_id: tenant }).whereIn('id', todo).update({
        data: mergeData({
          retired: true,
          retiredAt: new Date().toISOString(),
          completed: false, completedAt: null, wish: false, wishAt: null,
        }),
      });
      await addActivity(trx, tenant, rid, 'games_retired', { count: todo.length }, actorMemberId);
    }
    return { retired: todo.length };
  });
}

// Mirrors the JSON backend's method of the same name (#972) — see it for the
// contract: replace-not-merge, a wish skipped rather than refused, and why a
// clear removes the key instead of storing `[]`.
//
// The clear drops to the `-` operator for the reason `updateGame` above gives:
// `mergeData` can never remove a key. Same shape moveGames uses to strip owners
// off a reparented game.
async function setGameOwners(tenant, rid, gameIds, ownerIds) {
  return tx(tenant, async (trx) => {
    const round = await trx('rounds').where({ id: rid, tenant_id: tenant }).first('id');
    if (!round) return null;
    const want = [...new Set(gameIds)];
    ...
    // Wishes are skipped, so the count reports what was actually stamped.
    const todo = rows.filter((r) => !r.data.wish).map((r) => r.id);
    if (todo.length) {
      await trx('games').where({ round_id: rid, tenant_id: tenant }).whereIn('id', todo).update({
        data: ownerIds.length ? mergeData({ ownerIds }) : knex.raw("data - 'ownerIds'"),
      });
    }
...
// Bulk tag delta (#1000). See the JSON backend for the contract — in particular
// why it ADDS and REMOVES rather than replacing, why a wish is not skipped, and
// why `updated` counts games that actually changed.
async function setGameTags(tenant, rid, gameIds, { add = [], remove = [] } = {}) {
  return tx(tenant, async (trx) => {
    const round = await trx('rounds').where({ id: rid, tenant_id: tenant }).first('id');
    if (!round) return null;
    const want = [...new Set(gameIds)];
    ...
    // Computed per row rather than as one UPDATE: the delta depends on what each
    // game already carries, so there is no single value to write. Batched into
    // two statements — one per resulting list — would need the lists grouped by
    // value, which is more machinery than a shelf-sized selection is worth.
    let updated = 0;
    for (const row of rows) {
      const before = row.data.tagIds || [];
      const next = [...new Set([...before.filter((x) => !remove.includes(x)), ...add])]
        .filter((x) => !remove.includes(x));
      if (next.length === before.length && next.every((x) => before.includes(x))) continue;
      await trx('games').where({ id: row.id, round_id: rid, tenant_id: tenant }).update({
        // An empty result REMOVES the key, so a cleared game is byte-identical
        // to one nobody ever tagged (absent-key parity, as setGameOwners does).
        data: next.length ? mergeData({ tagIds: next }) : knex.raw("data - 'tagIds'"),
      });
      updated += 1;
    }
    return { updated };
...
// Bulk delete (#832). See the JSON backend for the contract, including why this
// one does NOT refuse a game that is still on the shelf.
async function deleteGames(tenant, rid, gameIds, actorMemberId) {
  return tx(tenant, async (trx) => {
    ...
    await trx('games').where({ round_id: rid, tenant_id: tenant }).whereIn('id', [...ids]).del();
    await scrubGames(trx, tenant, rid, ids);
    // The count reports what was OWNED, the return value what was removed (#863).
    const reported = rows.filter((r) => !r.data.wish).length;
    if (reported) {
      await addActivity(trx, tenant, rid, 'games_deleted', { count: reported }, actorMemberId);
    }

    return { deleted: rows.length, images: rows.map((r) => r.data.image).filter(Boolean) };
  });
}

// Scrub removed games' ids out of the round's sessions and activity feed — drop
// them from gameIds + all votes, reset the choice if it was a chosen game,
// delete sessions that end up empty. Mirrors the JSON backend's helper of the
// same name; shared by deleteGame, deleteGames and acquireWishExpansion, all of
// which take rows out of `games`. Sequential awaits: one transaction is one
// connection.
//
// It takes a SET for the reason the JSON backend's does, and the cost is far
// sharper here: one call per deleted game would re-read and re-write every
// session of the round over the wire, so undoing a 200-game import would be
// 200 x (sessions) round trips inside a single transaction. One pass writes each
// session at most once.
async function scrubGames(trx, tenant, rid, gids) {
  ...
  await trx('activities').where({ round_id: rid, tenant_id: tenant })
    .whereIn(knex.raw("data->>'gameId'"), [...gids]).del();
}

async function scrubGame(trx, tenant, rid, gid) {
  await scrubGames(trx, tenant, rid, new Set([gid]));
}
...
async function moveGames(tenant, rid, targetRid, limits, gameIds) {
  return tx(tenant, async (trx) => {
    ...
    // Shared with copyGames and with the JSON backend (lib/repo/import-copy.js);
    // the rows carry the game under `data`, which the helper reads directly.
    const { remap, created, mergedTags } =
      mergeTagsInto(src.tags, moving.map((g) => g.data), targetTags, newId);
    ...
    for (const g of moving) {
      movedIds.add(g.id);
      // `ownerIds` names members of the SOURCE round and is DROPPED (#971) — see
      // the JSON backend for why it is not remapped like tags. jsonb's `-`
      // operator removes the key, which is what keeps the moved row's shape equal
      // to the JSON backend's `delete` (absent, not `[]`); it is a no-op on a row
      // that never carried one, so the expression is unconditional and the tag
      // remap composes onto it with `||`.
      const patch = {
        round_id: targetRid,
        seq: knex.raw("nextval(pg_get_serial_sequence('games', 'seq'))"),
        data: Array.isArray(g.data.tagIds)
          ? knex.raw("(data - 'ownerIds') || ?::jsonb",
            [J({ tagIds: g.data.tagIds.map((x) => remap.get(x)).filter(Boolean) })])
          : knex.raw("data - 'ownerIds'"),
      };
...
// Copy games of one round onto another EXISTING round's shelf (#916). See the
// JSON backend for the contract and for what the copy does and does not carry.
//
// Both round rows are locked in ONE id-ordered statement, for the reason
// moveGames documents: two concurrent transfers between the same pair of rounds
// in opposite directions serialize instead of deadlocking.
//
// Unlike the move there is no `seq` subtlety to get wrong — every copy is a fresh
// INSERT, so it takes a fresh `seq` by construction and lands after the target's
// own games in the source's shelf order, which is what the JSON backend's array
// push does.
async function copyGames(tenant, rid, targetRid, limits, gameIds) {
  return tx(tenant, async (trx) => {
    if (rid === targetRid) return 'same_round';
    const rows = await trx('rounds')
      .whereIn('id', [rid, targetRid])
    ...
    // Filtered in JS rather than with a `whereIn`, for moveGames' reason: the
    // 'unknown_game' refusal is a COUNT comparison and would need a second query.
    let copying = await trx('games')
      .where({ round_id: rid, tenant_id: tenant })
      .orderBy('seq')
      .select('id', 'data');
    if (gameIds) {
      const want = new Set(gameIds);
      copying = copying.filter((g) => want.has(g.id));
      if (copying.length !== want.size) return 'unknown_game';
    }
    const targetTags = target.tags || [];

    const { remap, created, mergedTags } =
      mergeTagsInto(src.tags, copying.map((g) => g.data), targetTags, newId);

    if (limits) {
      const [{ count }] = await trx('games').where({ round_id: targetRid, tenant_id: tenant }).count();
      if (Number(count) + copying.length > limits.maxGames) return 'quota_games';
      if (targetTags.length + created.length > limits.maxTags) return 'quota_tags';
    }

    const copiedGames = copying.length;
    if (created.length) {
      // `tags` is an ARRAY — J() it (see the header note on the JSONB footgun).
      await trx('rounds')
        .where({ id: targetRid, tenant_id: tenant })
    ...
    // Sequential awaits: one transaction runs on one connection.
    for (const row of copying) {
      const data = copyGame(row.data, newId(), remap, newId);
      const { id: gid, ...rest } = data;
      await trx('games').insert({ id: gid, round_id: targetRid, tenant_id: tenant, data: J(rest) });
    }

    if (copiedGames) {
      await addActivity(trx, tenant, rid, 'games_copied_out', {
        count: copiedGames, roundId: targetRid, roundName: target.name,
      });
      await addActivity(trx, tenant, targetRid, 'games_copied_in', {
        count: copiedGames, roundId: rid, roundName: src.name,
      });
    }

    return { copiedGames, mergedTags, createdTags: created.length };
  });
}
...
async function finishSession(tenant, rid, sid, { finished, winnerIds, ending }, events) {
  return withSession(tenant, rid, sid, (s) => {
    if (!finished) {
      s.finished = false;
      s.finishedAt = null;
      ...
      // Absent means "nothing chosen" (#1038), the same convention `guests`,
      // `teams` and `childSessionIds` use — so a session without an ending keeps
      // its blob byte-identical to one written before the feature existed.
      // DELETED rather than set to null on every other path, because this route
      // is re-POSTed by every winner-chip tap and must be able to clear one.
      if (ending) s.ending = ending;
      else delete s.ending;
    }
  }, events);
}
...
// Persist the multi-table proposals, FIRST WRITER WINS (#796). See the JSON twin
// for the reasoning; the atomicity it relies on is this backend's `FOR UPDATE` in
// withSession, which serializes two simultaneous first opens of the builder
// instead of letting the later one replace what the earlier reader is looking at.
...
// Link a split parent to its children (#796), first writer wins. The log entry is
// pushed inside the closure so only the claiming writer records it — see the JSON
// twin.
...
// The retired per-round design (see the JSON backend's setBackground for why it
// outlives its route, #1202).
...
// Set the round's colour marker (#1187) — the sibling of setBackground, and the
// contract the JSON backend's setMarker matches: the stored index back, or null
// when the round is missing or belongs to another tenant. `marker` is a plain
// smallint column, so no J() — that wrapper exists for the JSONB footgun and
// would store the literal string "3" here.
async function setMarker(tenant, rid, index) {
  return tx(tenant, async (trx) => {
    const row = await trx('rounds').where({ id: rid, tenant_id: tenant }).first('id');
    if (!row) return null;
    await trx('rounds').where({ id: rid, tenant_id: tenant }).update({ marker: index });
    return { marker: index };
  });
}
...
// Patch a tag's name and/or icon (#255 icon, #1004 rename). See the JSON backend
// for the contract and for why the name DEDUPE lives in the mutator rather than
// in the route — this is the backend that makes it load-bearing: FOR UPDATE
// serializes concurrent writes to the tags array, so a check outside the
// transaction would let two renames both pass and produce two tags with one
// name, a state addTag can never reach.
async function updateTag(tenant, rid, tagId, patch) {
  return tx(tenant, async (trx) => {
    const row = await trx('rounds').where({ id: rid, tenant_id: tenant }).forUpdate().first('tags');
    if (!row) return null;
    const tags = row.tags || [];
    ...
    if (p.name !== undefined) {
      ...
      const clash = tags.some(
        (tg) => tg.id !== tagId && tg.name.toLowerCase() === name.toLowerCase());
      if (clash) return 'name_taken';
      tag.name = name;
    }
    if (p.icon !== undefined) {
      if (p.icon) tag.icon = p.icon;
...
// Put a round's tags in a hand-chosen order (#1159). See the JSON backend for
// the contract and for why the permutation must be EXACT. This is the backend
// that makes the FOR UPDATE load-bearing: the whole array is rewritten, so a
// read outside the transaction would let a reorder clobber a create or a delete
// that landed in between — the array would simply be written back without it.
// `tags` is an ARRAY — J() it (see the JSONB footgun in the header).
async function reorderTags(tenant, rid, tagIds) {
  return tx(tenant, async (trx) => {
    const row = await trx('rounds').where({ id: rid, tenant_id: tenant }).forUpdate().first('tags');
    if (!row) return null;
    const tags = row.tags || [];
    const ids = Array.isArray(tagIds) ? tagIds.map(String) : [];
    if (ids.length !== tags.length) return 'tags_changed';
    const byId = new Map(tags.map((tg) => [tg.id, tg]));
    const next = [];
    for (const tagId of ids) {
      const tag = byId.get(tagId);
      // A repeated id reads as unknown — the first occurrence consumed it.
      if (!tag) return 'tags_changed';
      byId.delete(tagId);
      next.push(tag);
    }
    await trx('rounds').where({ id: rid, tenant_id: tenant }).update({ tags: J(next) });
    return next;
  });
}
...
// A round's named session filters (#1328). See the JSON backend for the
// contract. This is the backend that makes the shared rules' placement
// load-bearing: FOR UPDATE serializes writes to the array, so the uniqueness,
// the cap and the permutation are checked against the list that is then
// written — two concurrent saves cannot both pass the cap. `saved_filters` is
// an ARRAY — J() it (see the JSONB footgun in the header). NULL until the first
// save, so assemble() emits the key only once written (absent-key parity).
async function withSavedFilters(tenant, rid, op) {
  return tx(tenant, async (trx) => {
    const row = await trx('rounds').where({ id: rid, tenant_id: tenant }).forUpdate()
      .first('saved_filters as savedFilters');
    if (!row) return { missing: true };
    const out = op(row.savedFilters);
    if (out && typeof out === 'object') {
      await trx('rounds').where({ id: rid, tenant_id: tenant }).update({ saved_filters: J(out.list) });
    }
    return { out };
  });
}

async function createSavedFilter(tenant, rid, filter, limit = Infinity) {
  const r = await withSavedFilters(tenant, rid, (list) => insertSavedFilter(list, filter, limit, newId));
  if (r.missing) return null;
  return typeof r.out === 'string' ? r.out : r.out.entry;
}

async function updateSavedFilter(tenant, rid, fid, patch) {
  const r = await withSavedFilters(tenant, rid, (list) => renameSavedFilter(list, fid, (patch || {}).name));
  if (r.missing || !r.out) return null;
  return typeof r.out === 'string' ? r.out : r.out.entry;
}

async function reorderSavedFilters(tenant, rid, ids) {
  const r = await withSavedFilters(tenant, rid, (list) => permuteSavedFilters(list, ids));
  if (r.missing) return null;
  return typeof r.out === 'string' ? r.out : r.out.list;
}

async function deleteSavedFilter(tenant, rid, fid) {
  const r = await withSavedFilters(tenant, rid, (list) => removeSavedFilter(list, fid));
  return !r.missing && !!r.out;
}
