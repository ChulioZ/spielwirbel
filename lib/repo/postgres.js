  ...
  const accounts = await knex('users')
    ...
    .select(
      ...
      knex.raw(`count(*) FILTER (
        WHERE data->'identities' @> '[{"type":"passkey"}]'::jsonb)::int as with_passkey`),
      knex.raw("count(*) FILTER (WHERE coalesce(data->>'bggUsername', '') <> '')::int as with_bgg"),
    )
    .first();
  ...
  const rounds = await atx(async (trx) => {
    ...
    const games = await trx('games')
      ...
      .select(
        ...
        trx.raw(`count(*) FILTER (
          WHERE coalesce(data->'source'->>'provider', '') <> '')::int as linked`),
        ...
        trx.raw(`count(*) FILTER (
          WHERE coalesce(data->>'image', '') like '/uploads/%')::int as own_cover`),
        trx.raw(`count(*) FILTER (
          WHERE coalesce(data->>'image', '') <> ''
            AND coalesce(data->>'image', '') not like '/uploads/%')::int as provider_cover`),
        ...
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(case when jsonb_typeof(data->'ownerIds') = 'array'
            then data->'ownerIds' else '[]'::jsonb end) > 0)::int as with_owners`),
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(case when jsonb_typeof(data->'expansions') = 'array'
            then data->'expansions' else '[]'::jsonb end) > 0)::int as with_expansions`),
      )
      .first();
    ...
    const tags = await trx('rounds')
      ...
      .select(
        ...
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(coalesce(tags, '[]'::jsonb)) > 0)::int as rounds_with`),
      )
      .first();
    ...
    const sessions = await trx('sessions')
      ...
      .select(
        ...
        // not optional.
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(case when jsonb_typeof(data->'guests') = 'array'
            then data->'guests' else '[]'::jsonb end) > 0)::int as with_guests`),
        trx.raw(`count(*) FILTER (
          WHERE jsonb_array_length(case when jsonb_typeof(data->'teams') = 'array'
            then data->'teams' else '[]'::jsonb end) > 0)::int as with_teams`),
      )
      .first();
    ...
    const shelfUse = await trx('games')
      .whereRaw('tenant_id not like ?', [demoLike])
      .select(
        trx.raw("count(distinct round_id) FILTER (WHERE data->>'retired' = 'true')::int as retired"),
        trx.raw("count(distinct round_id) FILTER (WHERE data->>'completed' = 'true')::int as completed"),
        trx.raw("count(distinct round_id) FILTER (WHERE data->>'wish' = 'true')::int as wish"),
        ...
        trx.raw(`count(distinct round_id) FILTER (
          WHERE data->>'retired' = 'true'
             ...
             OR data->>'wish' = 'true')::int as any_shelf`),
      )
      .first();
    ...
    return {
      ...
      roundCounts: perTenant.reduce(
        (acc, r) => ({ total: acc.total + r.n, max: Math.max(acc.max, r.n) }),
        { total: 0, max: 0 },
      ),
      games: games.n,
