...
async function instanceMetrics() {
  ...
  /* #1174 — the tenants the Funktionsnutzung card leaves out, and the predicate
     ...
     WHY the exclusion reaches `adoption`, `designs` and `social` but never
  ...
  const rounds = await atx(async (trx) => {
    ...
    /* What rounds are wearing, as a RAW histogram keyed by a bounded
       discriminator — never a resolved name. lib/routes/marker.js stores the
       design id without checking it against the registry on purpose, so that the
       list never becomes a cross-boundary contract; the panel resolves labels
       and falls back to „unbekannt". */
    const designs = await trx('rounds')
      .whereRaw('tenant_id not like ?', [demoLike])
      // The histogram is rendered as an adoption share on the same card, so it
      // takes the same exclusion — a numerator filtered while its denominator
      // was not would report „Designs 140 %".
      .whereRaw(keptTenant, [EXCL])
      .select(
        trx.raw(`coalesce(
           background->>'id',
           case when background->>'type' = 'collage' then 'collage' end,
           background->>'page',
           'none') as key`),
        trx.raw('count(*)::int as n'),
      )
  ...
  return {
    ...
    designs: Object.fromEntries(rounds.designs.map((d) => [d.key, d.n])),
    social: {
