...
//
// That last one is the only place the two backends need different code rather
// than a different literal: `mergeData` is `data || patch`, which can add or
// overwrite a key but never remove one, so the clear drops to the `-` operator —
// the same shape moveGames already uses to strip owners off a reparented game.
