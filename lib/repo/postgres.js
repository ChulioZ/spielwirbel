...
const { countByWeek, historyStart } = require('../metrics-history');
...
async function instanceMetrics(now = new Date().toISOString()) {
  ...
  /* The weekly signup dates. `users` is UN-SCOPED (no RLS), so a plain knex read
     is correct here — unlike `sessions` below.

     Bounded by a TEXT compare and bucketed in JavaScript, deliberately: the
     obvious `date_trunc('week', (…)::timestamptz)` throws for the WHOLE query
     on one malformed value while the JSON backend drops that row, so the two
     backends would disagree by 500ing. See lib/metrics-history.js. */
  const accountDates = await knex('users')
    .whereRaw("coalesce(data->>'tenantId', '') not like ?", [demoLike])
    .whereRaw("data->>'createdAt' >= ?", [historyStart(at)])
    .select(knex.raw("data->>'createdAt' as created"));
  ...
  const rounds = await atx(async (trx) => {
    ...
    /* The weekly session series. INSIDE atx(), like every read in this block
       and unlike the accounts one above — `sessions` is RLS-scoped, and a plain
       query under a non-superuser role returns ZERO ROWS RATHER THAN AN ERROR.
       The panel would then draw a healthy-looking empty chart on production
       while every superuser-run test stayed green
       (.claude/rules/admin-kennzahlen-card.md, .claude/rules/admin-cross-tenant-escape.md §2). */
    const sessionDates = await trx('sessions')
      .whereRaw('tenant_id not like ?', [demoLike])
      .whereRaw("data->>'createdAt' >= ?", [historyStart(at)])
      .select(trx.raw("data->>'createdAt' as created"));
  ...
  return {
    accounts: {
      ...
      history: countByWeek(accountDates.map((r) => r.created), at),
    },
    rounds: {
    ...
    content: {
      ...
      sessionHistory: countByWeek(rounds.sessionDates.map((r) => r.created), at),
    },
