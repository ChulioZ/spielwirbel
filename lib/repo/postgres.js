  ...
  return atx(async (trx) => {
    ...
    const plays = await trx({ s: 'sessions' })
      ...
      .select(
        ...
        trx.raw(`count(*) FILTER (WHERE ${playedAt} >= ?)::int as plays_week`, [period.week]),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${playedAt} >= ?)::int as tenants_week`, [period.week]),
        trx.raw(`count(*) FILTER (WHERE ${playedAt} >= ?)::int as plays_month`, [period.month]),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${playedAt} >= ?)::int as tenants_month`, [period.month]),
        trx.raw(`count(*) FILTER (WHERE ${playedAt} >= ?)::int as plays_year`, [period.year]),
        trx.raw(`count(distinct s.tenant_id) FILTER (WHERE ${playedAt} >= ?)::int as tenants_year`, [period.year]),
        // ALL-TIME, feeding the Discover podium's play lift (#928): a game's
        // score there is a function of its own votes and plays over its whole
        // life, so a period would make the public number disagree with the one
        // the Regal prints. Unfiltered `count(*)` rather than a `>= ''` filter —
        // the JSON backend spells the empty cutoff because it shares one loop
        // over its window map; here the two forms are the same set, since every
        // row of this grouped read is already a finished play.
        trx.raw('count(*)::int as plays_all'),
        trx.raw('count(distinct s.tenant_id)::int as tenants_all'),
      );
