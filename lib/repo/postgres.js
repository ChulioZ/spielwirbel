...
// The legacy single-group tenant every pre-tenancy row belongs to (the SQL
// column default + lib/tenant.js DEFAULT_TENANT). The source of the #266 claim.
...
FROM _t`,
  ...
  // response. lastPlayed mirrors routes/rounds.js's old JS exactly: newest
...
async function createUser(fields) {
  ...
  // the deliberately HIDDEN e-mail one (see routes/account.js and the JSON
...
/*
 * The cross-tenant re-tenant WRITE, the counterpart to the atx read escape. `rtx`
 * runs a transaction with `app.retenant='on'` set, which the FOR SELECT + FOR
 * UPDATE escape pair (migration 20260724130000_retenant.js) admits so a bulk
 * UPDATE may re-label rows across tenants — the one thing the tenant policy makes
 * impossible under any single app.tenant_id. Both the read escape (so the WHERE
 * scan and the moved row are visible) and the write escape are needed; INSERT and
 * DELETE stay closed. Transaction-local (set_config(..., true)), so it dies at
 * COMMIT. Used ONLY by claimDefaultTenant; it is NOT a general write door — see
 * the migration and .claude/rules/retenant-rls-escape.md.
 */
function rtx(fn) {
  return knex.transaction(async (trx) => {
    await trx.raw("SELECT set_config('app.retenant', 'on', true)", []);
    return fn(trx);
  });
}

// Move every round-scoped row from the legacy 'default' tenant to `targetTenant`
// — the operator "claim the 'default' tenant" step (#266), so a freshly-registered
// owner account can finally see the pre-tenancy data once accounts mode is on.
// Global, absent from TENANT_METHODS; reached only from the admin route, which
// checks the target account exists, is empty, and is not itself 'default'. Returns
// the number of ROUNDS moved (children move with them); 0 means 'default' was
// already empty (nothing to claim). FK integrity is unaffected — children
// reference rounds by id, not tenant_id — so the table order is irrelevant.
async function claimDefaultTenant(targetTenant) {
  return rtx(async (trx) => {
    let rounds = 0;
    // Awaited one at a time: a single transaction runs on ONE connection and
    // cannot serve concurrent queries (see the tx note above). The
    // `where tenant_id = 'default'` is LOAD-BEARING, not decoration: under the
    // retenant escape every tenant's rows are visible, so an unqualified UPDATE
    // would move ALL of them — the WHERE, not the policy, is what scopes the move
    // to the source. Do not drop it.
    for (const table of ['rounds', 'members', 'games', 'sessions', 'activities']) {
      const n = await trx(table).where({ tenant_id: DEFAULT_TENANT }).update({ tenant_id: targetTenant });
      if (table === 'rounds') rounds = n;
    }
    return { rounds };
  });
}
 ...
 * from routes/contact.js, the read/decide side only from routes/admin.js.
 */
 ...
 * the caller's own id, since these are reached from user-facing routes/account.js.
...
// Add a member to an existing round (issue #207). See lib/repo/json.js for why
// this primitive exists and its contract; `fields` is { name, color?, userId? }
// with absent optional keys staying absent (jsonb parity). Returns the created
// member, or null if the round is missing / another tenant's.
async function createMember(tenant, rid, fields) {
  return tx(tenant, async (trx) => {
