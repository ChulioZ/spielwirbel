...
// One definition of what counts as a storefront link (#981) — see json.js.
const { storefrontMatch } = require('./storefront-match');
...
/* The storefront clean-up (#981). See the JSON backend for the contract and for
   why an uploaded cover is kept.

   READ cross-tenant under atx() — the question is "which rows anywhere still
   point at a retired storefront", which names no tenant. WRITE under
   qt(tenant, …), never atx(): the admin policy contributes SELECT only, and an
   UPDATE through it matches ZERO rows while reporting success
   (.claude/rules/admin-cross-tenant-escape.md, .claude/rules/rls-blocks-data-migrations.md). */
async function clearStorefrontLinks({ hosts, providerIds, dryRun }) {
  const run = { games: 0, covers: 0, sources: 0, uploads: 0, rounds: 0, tenants: 0 };
  const rows = await atx((trx) => trx('games')
    .whereRaw("data->>'image' is not null or data->'source' is not null")
    .select('id', 'round_id', 'tenant_id', 'data'));
  const perRound = new Map();
  for (const r of rows) {
    const m = storefrontMatch(r.data, hosts, providerIds);
    if (!m.cover && !m.source) continue;
    run.games += 1;
    if (m.cover) run.covers += 1;
    if (m.source) run.sources += 1;
    if (m.upload && m.source) run.uploads += 1;
    const key = `${r.tenant_id}\u0000${r.round_id}`;
    if (!perRound.has(key)) perRound.set(key, { tenant: r.tenant_id, rid: r.round_id, hits: [] });
    perRound.get(key).hits.push({ id: r.id, ...m });
  }
  run.rounds = perRound.size;
  run.tenants = new Set([...perRound.values()].map((v) => v.tenant)).size;
  if (dryRun) return run;

  for (const { tenant, rid, hits } of perRound.values()) {
    await tx(tenant, async (trx) => {
      for (const hit of hits) {
        // `mergeData` cannot REMOVE a key, so `source` is deleted with a jsonb
        // operator; `image` is set to null, which is the stored "no cover" value
        // every read already handles.
        let q = trx('games').where({ id: hit.id, round_id: rid, tenant_id: tenant });
        if (hit.cover && hit.source) {
          await q.update({ data: knex.raw("(data || ?::jsonb) - 'source'", [J({ image: null })]) });
        } else if (hit.cover) {
          await q.update({ data: mergeData({ image: null }) });
        } else {
          await q.update({ data: knex.raw("data - 'source'") });
        }
      }
      await addActivity(trx, tenant, rid, 'storefront_cleared', { n: hits.length });
    });
  }
  return run;
}
...
async function replaceImage(oldPath, newPath) {
  ...
  const targets = await atx((trx) =>
    trx('games').whereRaw("data->>'image' = ?", [oldPath]).select('id', 'tenant_id'));
  let changed = 0;
  for (const t of targets) {
    const rows = await qt(t.tenant_id, (trx) =>
      trx('games')
        .where({ id: t.id, tenant_id: t.tenant_id })
        .update({ data: mergeData({ image: newPath }) })
        .returning('id'));
    if (rows[0]) changed += 1;
  }
  return changed;
