 ...
 * listModeration/listUsers — issue #268; exportTenant/exportAccountData/
 * eraseAccount — issues #273/#397;
 ...
 * admin-gated routes (lib/routes/admin/) — never via req.repo, which is all an
 * ordinary request handler holds. See
 * .claude/rules/admin-moderation-surface.md.
 *
 * User feedback (createFeedback/listFeedback/deleteFeedback — issues #260/#389)
 * is global for the same reason: it is data ABOUT the app, addressed to the
 * operator, who reads it across every tenant. Also absent from TENANT_METHODS —
 * since #321 the write is reached on the module-level repo from lib/routes/contact.js
 * (the 'feedback' category of the public contact form) and the read/delete only
 * from the admin-gated lib/routes/admin/notices.js.
 *
 * Contact notices (createContactNotice/listContactNotices/deleteContactNotice/…
 * — issues #272/#389) follow the feedback pattern exactly: a stored abuse notice
 * / contact submission is addressed to the operator and usually comes from
 * someone who is not a user at all, so the store is global and absent from
 * TENANT_METHODS.
 *
 ...
 * user-facing routes (lib/routes/account.js) rather than only the admin ones, so
 ...
 *
 * Friendships (createFriendRequest/listFriendships/acceptFriendRequest/
 * deleteFriendshipById) and the Freundeskreis feed (addFeedEvent/listFeedEvents —
 * issue #325) are global too: both are keyed by ACCOUNT ids (the identity layer),
 * not a tenant, exactly like the inbox — so they are absent from TENANT_METHODS
 * and reached from the account routes (lib/routes/friends.js), which scope every query
 * to the authenticated caller. A friendship shares NO round data; only feed events
 * (a fixed title/coverUrl allowlist enforced by addFeedEvent) flow between friends.
 *
 * The BGG corpus (replaceCorpus/corpusStats/getCorpusEntries/listCorpusEntries/
 * listCorpusPending/updateCorpusEntries — issues #681, #682, #829) is global for the plainest reason of
 * the lot: it is not the app's data at all. A game's BGG rank, weight and player counts are public facts about
 * a published product, identical for every tenant, so scoping them would store
 * thousands of identical rows per tenant and ask BGG once per tenant. Absent from
 * TENANT_METHODS; written only from the admin-gated lib/routes/admin/corpus.js and the
 * scheduler's enrichment job.
 */
...
const TENANT_METHODS = [
  ...
  'reorderTags',
  'deleteTag',
  // Saved session filters (#1328) — round data like tags.
  'createSavedFilter',
  'updateSavedFilter',
  'reorderSavedFilters',
  'deleteSavedFilter',
  // Dismissed recommendations (#782) — round-scoped data like tags, so they are
  // reached through req.repo and never on the module-level repo.
