 ...
 */
/* The bounded discriminator a round's design is counted under (#941). Never a
   resolved NAME: `lib/routes/marker.js` stores `id` without checking it
   against the registry on purpose, so that the design list never becomes a
   cross-boundary contract — the panel resolves labels with `resolveDesign()`
   and falls back to „unbekannt". Keeping that here means an unknown id is
   reported honestly rather than silently folded into 'none'. */
function designKey(bg) {
  if (!bg || typeof bg !== 'object') return 'none';
  if (bg.type === 'collage') return 'collage';
  if (bg.id) return String(bg.id);
  if (bg.page) return String(bg.page);
  return 'none';
}
...
async function instanceMetrics() {
  ...
  /* #1174: the Funktionsnutzung card may leave the operator's own tenants out,
     ...
     EVERYTHING that card renders — `adoption`, `designs` and `social`, each
  ...
  for (const r of rounds) {
    ...
    if (counted) {
      ...
      const key = designKey(r.background);
      designHistogram[key] = (designHistogram[key] || 0) + 1;
  ...
  return {
    ...
    // A raw histogram of what rounds are wearing, keyed by a bounded
    // discriminator. THE SERVER DOES NOT RESOLVE DESIGN IDS: round-designs.js's
    // header states the stored id is deliberately not validated against the
    // registry so the list never becomes a cross-boundary contract, and the
    // panel resolves labels itself.
    designs: designHistogram,
    social: {
