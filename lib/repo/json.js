...
// Copy the provider-sourced fields (#717, widened by #724) from `src` onto
// `dst`. Values only ever ACCRETE: an absent, null or empty value is SKIPPED,
// never written. That gives two guarantees at once — "the provider has no data
// right now" cannot erase what an earlier fetch stored (the licence-granted copy
// we hold stays until a real value replaces it), and a free-text game's row
// keeps every one of these keys ABSENT rather than gaining a wall of nulls
// (absent-key parity, .claude/rules/postgres-backend.md).
//
// An empty categories/mechanics list is "BGG named none", which is why it is
// skipped like a null rather than stored as []. Mirrored in the Postgres backend
// as a deliberate second copy, like `sameSource`, with the shared contract suite
// as the guard against drift.
function assignProviderInfo(dst, src) {
  if (Number.isFinite(src.weight)) dst.weight = src.weight;
  if (typeof src.description === 'string' && src.description) dst.description = src.description;
  if (Number.isFinite(src.minPlaytime)) dst.minPlaytime = src.minPlaytime;
  if (Number.isFinite(src.maxPlaytime)) dst.maxPlaytime = src.maxPlaytime;
  if (Number.isFinite(src.minAge)) dst.minAge = src.minAge;
  if (Array.isArray(src.categories) && src.categories.length) dst.categories = src.categories;
  if (Array.isArray(src.mechanics) && src.mechanics.length) dst.mechanics = src.mechanics;
  if (Number.isFinite(src.rating)) dst.rating = src.rating;
  return dst;
}
...
// Remove every trace of a removed game's id from the round's sessions and its
// activity feed. Shared by deleteGame and acquireWishExpansion — both take a row
// out of `games`, and a session or a Chronik row still naming it is a dangling
...
function scrubGame(round, gid) {
  ...
  if (Array.isArray(round.activities)) round.activities = round.activities.filter((a) => a.gameId !== gid);
}
