...
async function createGame(tenant, rid, fields, actorMemberId) {
  ...
  // Provider-sourced info (#717), resolved by the route from the provider —
  // never client-supplied. All three keys stay absent when unresolved, so a
  // free-text game's row is byte-identical to what it always was.
  if (Number.isFinite(fields.weight)) game.weight = fields.weight;
  if (typeof fields.description === 'string' && fields.description) game.description = fields.description;
...
// Persist provider-sourced info (weight/description, #717) onto a game and
// stamp the attempt. Values only ever ACCRETE: a null means "the provider has
// no data (right now)", which must not erase what an earlier fetch stored —
// the licence-granted copy we hold stays until a real value replaces it.
// `providerInfoAt` is stamped on every call; it is what keeps a game the
// provider knows nothing about from being re-fetched on every view
// (lib/provider-info.js). Idempotent, so overlapping deploys may both run it.
async function setGameProviderInfo(tenant, rid, gid, info) {
  const round = live(tenant, rid);
  if (!round) return null;
  const game = round.games.find((g) => g.id === gid);
  if (!game) return null;
  if (Number.isFinite(info.weight)) game.weight = info.weight;
  if (typeof info.description === 'string' && info.description) game.description = info.description;
