...
// createGame, move, copy and the round import all leave behind, so a cleared
// game becomes indistinguishable from one nobody ever marked. (The single PATCH
// stores `[]` instead — both read as ownerless everywhere, and aligning the two
// would mean special-casing `ownerIds` inside the generic `updateGame`.)
//
