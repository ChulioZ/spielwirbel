...
const { countByWeek } = require('../metrics-history');
...
async function instanceMetrics(now = new Date().toISOString()) {
  ...
  return {
    accounts: {
      ...
      history: countByWeek(users.map((u) => u.createdAt), at),
    },
    rounds: {
      total: rounds.length,
    },
    content: {
      ...
      roundsWithRetired, roundsWithCompleted, roundsWithWish,
      sessionHistory: countByWeek(sessionDates, at),
    },
