  ...
  return {
    ...
    adoption: {
      ...
      sessionsWithVoteLink: new Set(data.sessionVoteLinks
        .filter((l) => !isDemoTenant(l.tenantId))
        .map((l) => `${l.roundId}\u0000${l.sessionId}`)).size,
      accountsWithPasskey: users.filter((u) => Array.isArray(u.identities)
        && u.identities.some((i) => i && i.type === 'passkey')).length,
      accountsWithBggUsername: users.filter((u) => !!u.bggUsername).length,
    },
