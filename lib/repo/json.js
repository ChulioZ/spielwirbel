...
// A row enriched BEFORE covers were parsed (#779): its attributes are present
// and fresh, but they predate `imageUrl`, so without this clause every existing
// row keeps the placeholder until it goes stale on its own — up to
// BGG_CORPUS_STALE_DAYS (30) after the deploy.
//
// It tests for the KEY'S ABSENCE, never for a null value, and that is the whole
// trap. `pickImage()` answers null for a game BGG has no thumbnail for, and that
// is a SETTLED answer: an `info.imageUrl == null` test would put every
// thumbnail-less game back at the head of the queue on every tick, forever —
// the asked-vs-answered failure updateCorpusEntries already carries the #736
// comment about. A row BGG answered nothing at all for keeps a stamped
// `enrichedAt` with no usable `info`, and must stay out for the same reason.
//
// The clause is therefore SELF-RETIRING: one enrichment pass writes the key
// (null or not), and the row never matches again. The Postgres backend spells
// the identical predicate in jsonb — change one and change the other.
    ...
    for (const s of round.sessions) {
      ...
      for (const perPerson of Object.values(s.votes || {})) {
        for (const [gameId, vote] of Object.entries(perPerson || {})) {
          if (!vote || typeof vote.rating !== 'number') continue;
          const key = linked.get(gameId);
          if (!key) continue;
          const row = rowFor(key);
          row.ratings.sum += vote.rating;
          row.ratings.count += 1;
