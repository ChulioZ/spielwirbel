...
// The legacy single-group tenant every pre-tenancy row belongs to (the source of
// the #266 claim; mirrors the Postgres backend's DEFAULT_TENANT).
...
async function createUser(fields) {
  ...
  // deliberately HIDDEN e-mail one — see routes/account.js.
...
// Move every round of the legacy 'default' tenant to `targetTenant` — the operator
// "claim the 'default' tenant" step (#266), so a freshly-registered owner account
// can finally see the pre-tenancy data once accounts mode is on. Children are
// nested in the round here, so re-pointing the round's tenantId moves everything.
...
// the number of ROUNDS moved; 0 means 'default' was already empty.
async function claimDefaultTenant(targetTenant) {
  let rounds = 0;
  for (const round of data.rounds) {
    if (tenantOf(round) === DEFAULT_TENANT) {
      round.tenantId = targetTenant;
      rounds += 1;
    }
  }
  if (rounds) saveData();
 ...
 * (feedback/moderation) it IS reached from user-facing routes (routes/account.js),
...
// be created by createRound — routes/members.js states outright that adding one
...
// missing (or belongs to another tenant). This slice adds the primitive; the
// invitation flow that calls it — and the "at most one member per user per round"
// invariant it must enforce — is a later slice of #207.
