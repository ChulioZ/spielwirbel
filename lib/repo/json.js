...
// The 0–5 tile a vote lands on. The public aggregate reports a HISTOGRAM rather
    ...
    for (const s of round.sessions) {
      ...
      for (const perPerson of Object.values(s.votes || {})) {
        for (const [gameId, vote] of Object.entries(perPerson || {})) {
          ...
          // disagree with the buckets it totals. `effectiveRating` admits any
          // finite number (it mirrors the SQL's `jsonb_typeof = 'number'`), and a
          // fractional one is not a tile the vote card can produce — the same
          // value `scoreRatings` skips rather than interpolating.
