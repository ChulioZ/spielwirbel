...
const fs = require('fs');
const { data, saveData, id, DATA_FILE } = require('../store');
...
// Calendar week/month/year boundaries for publicGameAggregates (#964).
...
// The saved-filter list operations (#1328), shared with the Postgres backend so
// the name/cap/permutation rules run beside each backend's own lock.
const {
  insertSavedFilter, renameSavedFilter, removeSavedFilter, permuteSavedFilters,
} = require('./saved-filters');

// The 1–5 tile a vote lands on. The public aggregate reports a HISTOGRAM rather
// than a rating sum so the SCORE CURVE stays in vote-score.js (#914) — see
// `scoreTally` there for why the sum could not stay.
const { TILE_VALUE, tileValue } = require('../../public/js/vote-score');
// The avatar palette, shared verbatim with the frontend (#420). Needed here so
// `deleteMember` can FREEZE the colours a removed row would otherwise shift —
// see its comment.
...
const { sessionChildIds, sessionEnding } = require('../../public/js/session-outcome');
...
// Which design an account wears, and the switch-back target (#1201) — the same
// resolver /me uses, so the operator's tile and the account agree.
...
const snapshot = (round) => {
  ...
  // Always present, may be null (#1187) — the same key contract `background`
  // has. A round created before markers existed simply has no stored index, and
  // the CLIENT resolves one from its retired design; emitting the key as `null`
  // rather than omitting it is what keeps this backend byte-comparable with
  // Postgres, whose reads name their columns and cannot omit it.
  copy.marker = round.marker ?? null;
  return copy;
};
...
//     sessionCount, playedCount, background, marker, lastPlayed:
...
// getRound: background and marker always present, tags/lastSessionFilters only
// when they have ever been written.
async function getRoundMeta(tenant, rid) {
  ...
  // Saved filters (#1328), same only-when-ever-written discipline.
  if (r.savedFilters != null) out.savedFilters = clone(r.savedFilters);
  return out;
}
...
// `importFromRoundId` optionally copies the active games from an existing round
// — of the same tenant only — with the full field set (#921), and the tags they
// carry. `owner` ({ name, userId }, #421) is the creator's own seat: PREPENDED
// ahead of the typed names, and the only member carrying a `userId`. When
// absent, a member's shape is byte-identical to before — { id, name }, no
// `userId` key (absent-key parity with Postgres).
//
// `limits` ({ maxTags }) is passed only when quotas are enforced, and only the
// tag copy can reach it: the games come from one round, so they are already
// bounded by that round's own games cap. See importGame below for why a tag over
// the cap is dropped silently rather than refusing the round.
async function createRound(tenant, { name, members, owner, importFromRoundId, limits }) {
  ...
  const round = {
    ...
    activities: [],
    background: null,
    // Assigned once, from the id, and STORED (#1187) — never recomputed on read.
    // That is what lets markerIndexFromId change later (a better hash, a
    // different marker count) without re-colouring every existing round, which a
    // derive-on-read default could not promise.
    marker: markerIndexFromId(rid),
  };

  const src = importFromRoundId ? live(tenant, importFromRoundId) : null;
  if (src) {
    const importing = src.games.filter((g) => !g.retired && !g.completed && !g.wish);
    const { tags, remap } = importTags(src.tags, importing, limits, id);
    if (tags.length) round.tags = tags;
    for (const g of importing) {
      const ng = importGame(g, id(), remap, id);
      round.games.push(ng);
...
async function deleteRound(tenant, rid) {
  ...
  // Legacy collage backgrounds are hosted uploads too (see lib/routes/marker.js).
...
/* Every stored path the app still references (#941): game covers plus account
   profile pictures. ONE method returning the union rather than two the caller
   has to remember to combine — an orphan report built from half the references
   would name live objects as deletable, which is the one way this feature could
   do harm.

   Includes hotlinked provider URLs, exactly as listGameImages does: the caller
   filters to `/uploads/` paths, and a union that quietly dropped them would be a
   second place that has to know the difference. */
async function referencedImages() {
  const out = new Set(await listGameImages());
  for (const u of data.users) if (u.avatar) out.add(u.avatar);
  return [...out];
}

/* How much room the store takes (#941). The JSON backend answers honestly with
   its own file's size — it has no database — so the panel can show a figure for
   a self-hosted instance too rather than a permanent dash. */
async function databaseSize() {
  const stat = await fs.promises.stat(DATA_FILE).catch(() => null);
...
async function roundContent(roundId) {
  ...
  return {
    ...
    // A saved filter's name (#1328) is user-authored text too, so it needs the
    // same takedown path — the sixth such field.
...
async function redactText({ kind, roundId, id: targetId }, replacement) {
  ...
  // Like a tag: the filter survives with its seats and tags, only its name is blanked.
  else if (kind === 'filter') { holder = (round.savedFilters || []).find((f) => f.id === targetId); field = 'name'; }
...
/* The 3-year retention purge (#311). Deletes moderation-log entries whose action
   date lies before `beforeIso`, EXCEPT the ERASURE_ACTIONS records: those are
   the Art. 17(3)(b)/(e) Löschnachweise, are e-mail-free by design, and are kept
   permanently. Returns how many were deleted.

   Mutates `data.moderationLog` in place, never reassigns it
   (.claude/rules/data-access-layer.md).

   An entry with NO `at` is kept, not deleted — the opposite of the vote-link
   sweep's fail-closed reading, and deliberately so: there the field authorizes a
   write, so a malformed row must lose its power; here it is a retention record,
   and deleting something because its date is unreadable destroys evidence the
   operator may owe an authority. A malformed row is a thing to notice, not to
   silently drop. */
async function purgeModerationLog(beforeIso) {
  let removed = 0;
  for (let i = data.moderationLog.length - 1; i >= 0; i -= 1) {
    const e = data.moderationLog[i];
    if (ERASURE_ACTIONS.includes(e.action)) continue;
    if (!e.at) continue;
    if (e.at < beforeIso) { data.moderationLog.splice(i, 1); removed += 1; }
  }
  if (removed) saveData();
  return removed;
}
 ...
 * admin-gated lib/routes/admin/notices.js. Not in TENANT_METHODS.
 */
...
// bare ceiling: an operator number that is bounded ships as used/limit, so the
// panel shows headroom rather than a figure nobody can judge.
 ...
 * passed by a call site can never reach a friend's feed. Retention is an AGE
 * limit since #1357 (lib/feed-events.js): expired rows are pruned on write and by
 * the scheduler's purgeExpiredFeedEvents sweep, with MAX_FEED_EVENTS kept only as
 * a high safety ceiling per account.
 */

// One set and one retention rule for both backends.
...
async function addFeedEvent(uid, event) {
  ...
  // An Abzeichen (#1389) carries its catalogue key + tier and no cover; an
  // invalid one is dropped like an unknown type (lib/feed-events.js).
  ...
  if (badge) row.tier = badge.tier;
  // The first field ever added to this allowlist (#1079; `tier` above is the second, #1389), and only for the type
  // that means anything by it. ABSENT rather than 0 for the other two: the row
  // is one JSON blob on both backends, so an always-present key would be a shape
  // difference between types and between rows written before and after the
  // change. Typed here, not trusted from the caller — a count is not personal
  // data, which is why the widening was acceptable at all, but "not personal
  // data" is a property of an integer and not of whatever arrives.
  if (event.type === 'games_imported' && Number.isInteger(event.count) && event.count >= 0) {
    row.count = event.count;
  }
  data.feedEvents.push(row);
  // Prune this user's expired events, then anything beyond the safety ceiling
  // (insertion order == age). In place: data.feedEvents is never reassigned.
  const cutoff = feedEventCutoff();
  for (let i = data.feedEvents.length - 1; i >= 0; i -= 1) {
    const e = data.feedEvents[i];
    if (e.uid === uid && e.at && e.at < cutoff) data.feedEvents.splice(i, 1);
  }
  const mine = data.feedEvents.filter((e) => e.uid === uid);
  const over = mine.length - feedEventCeiling();
  for (let i = 0; i < over; i++) {
...
// The retention sweep (#1357): delete every feed event dated before `beforeIso`,
// across all accounts, returning how many went. Idempotent, like the other
// scheduler sweeps. A row with no `at` is kept — it cannot be dated, and every
// row this backend writes carries one.
async function purgeExpiredFeedEvents(beforeIso) {
  let removed = 0;
  for (let i = data.feedEvents.length - 1; i >= 0; i -= 1) {
    const e = data.feedEvents[i];
    if (e.at && e.at < beforeIso) { data.feedEvents.splice(i, 1); removed += 1; }
  }
  if (removed) saveData();
  return removed;
}
...
//
// `before` (#1357) is the paging cursor: a { id, at } naming the last row the
// previous page CONSUMED, and the read returns only rows strictly older than it
// in store order (array position here, `seq` on Postgres). If that row has gone
// since — aged out, or its author erased — its timestamp stands in: rows dated
// before it. That can skip a sibling written in the same millisecond, and never
// repeats one.
async function listFeedEvents(uids, limit = 100, before = null) {
  const set = new Set(uids || []);
  if (!set.size) return [];
  let rows = data.feedEvents;
  if (before) {
    const i = rows.findIndex((e) => e.id === before.id);
    rows = i !== -1 ? rows.slice(0, i) : rows.filter((e) => e.at && e.at < before.at);
  }
  return rows
...
async function updateMember(tenant, rid, mid, patch) {
  ...
  // The owner selection this seat last used when adding a game (#971). An EMPTY
  // array is a real value here — "I took myself off the list" has to be
  // remembered too — so the guard is `!== undefined`, never truthiness.
  if (patch.ownerPreset !== undefined) member.ownerPreset = patch.ownerPreset;
  saveData();
  return clone(member);
}

/* Retire a member (#1006) — the same shape games have had since #250, and for
   the same reason: a member id is a key in `votes`, in `winnerIds` and in team
   lists, so removing the row would either orphan that history or rewrite it.
   Retiring touches NOTHING but the two flags, which is what makes every past
   session's participant list and every game's Spielwirbel-Score byte-identical
   afterwards.

   `sessionPeople()` keeps resolving a retired member on purpose — see the rule
   in the issue and the frontend helper `activeMembers()`, which is what the
   person-facing surfaces filter through instead. */
async function retireMember(tenant, rid, mid, retired, actorMemberId) {
  ...
  member.retiredAt = retired ? new Date().toISOString() : null;
  addActivity(round, retired ? 'member_retired' : 'member_restored', { name: member.name }, actorMemberId);
  saveData();
  return clone(member);
}

/* Hard-delete a member (#1006). Only ever called for a seat with no history at
   all — the route is what establishes that — because this really does remove the
   row.

   THE COLOUR FREEZE IS THE NON-OBVIOUS HALF. A member's avatar colour is derived
   from their POSITION in `round.members` when they have not picked one
   (`memberHex`, core.js), and the palette repeats every 8. So splicing a row out
   silently re-colours every member after it, on every screen including the
   historical ones. Writing each of their current colours down first makes the
   delete invisible to everyone but the person deleted. */
async function deleteMember(tenant, rid, mid, actorMemberId) {
  const round = live(tenant, rid);
  if (!round) return false;
  const i = round.members.findIndex((m) => m.id === mid);
  if (i < 0) return false;
  round.members.forEach((m, idx) => {
    if (idx > i && !MEMBER_COLORS.includes(m.color)) m.color = MEMBER_COLORS[idx % MEMBER_COLORS.length];
  });
  const [gone] = round.members.splice(i, 1);
  addActivity(round, 'member_deleted', { name: gone.name }, actorMemberId);
  saveData();
  return true;
}
...
async function createGame(tenant, rid, fields, actorMemberId) {
  ...
  // Which MEMBERS of this round own the box (#971). Absent when nobody has been
  // recorded, so an unmarked shelf's rows stay byte-identical and `ownedByParty`
  // reads them as always drawable. Validated against `round.members` by the
  // route; a wish never carries it, since the round does not own the game yet.
  if (Array.isArray(fields.ownerIds) && fields.ownerIds.length) game.ownerIds = fields.ownerIds;
...
async function createGames(tenant, rid, games, actorMemberId, limits, wish = false) {
  ...
  for (const fields of games) {
    ...
    fresh.push({
      ...
      // Who owns the box (#971) — the whole batch shares one selection, so the
      // key is absent on an import nobody assigned owners to. Never on a wish:
      // the round does not own the game yet.
...
// The generic field writer. One shape exception, and it is deliberate: an EMPTY
// `ownerIds` is stored as ABSENCE rather than as `[]`, so a game whose owners
// were cleared is byte-identical to one nobody ever marked — which is what
// createGame, moveGames, copyGames, the round import and setGameOwners (#972)
// all produce. Without it there are two stored shapes for one user action, and
// the one you get depends on which screen the user happened to reach it from.
//
// Mirrored in the Postgres backend, where it needs different code rather than a
// different literal; `test/support/repo-contract.js` is the guard against the
// two drifting, as it is for `sameSource` and `mergeExpansions`.
async function updateGame(tenant, rid, gid, patch) {
  ...
  if (Array.isArray(patch.ownerIds) && !patch.ownerIds.length) delete game.ownerIds;
  saveData();
  return clone(game);
}
...
// The deep copy onto fresh ids lives in ./import-copy.js since #921 — its only
// caller was createRound's import, and it belongs beside the rest of what a
// copied game carries.
...
// Set who owns several games at once (#972). Returns null (round missing),
// 'unknown_game', or { updated } — the number of games actually stamped.
//
// The set REPLACES what each game held, so an empty list is the clear. On a
// clear the key is REMOVED rather than stored as `[]`: that is the shape
// createGame, move, copy, the round import and the single `updateGame` all leave
// behind, so a cleared game is indistinguishable from one nobody ever marked.
//
// A WISH in the selection is SKIPPED — not stamped, not counted — because a wish
// is nobody's box yet (#971). Deliberately NOT the 400 the single PATCH gives:
// that one answers a detail page that shows no owner row at all, so a request
// carrying owners for a wish is a broken client. A bulk selection is a filtered
// sweep over a shelf, and refusing it whole because one wish slipped in would
// break the action exactly where it is most useful.
//
// No activity is written — see the route for the four bulk decisions.
async function setGameOwners(tenant, rid, gameIds, ownerIds) {
  ...
  let updated = 0;
  for (const game of picked) {
    if (game.wish) continue;
    if (ownerIds.length) game.ownerIds = [...ownerIds];
    else delete game.ownerIds;
    updated += 1;
  }
  saveData();
  return { updated };
...
// Add and/or remove tags across several games at once (#1000). Returns null
// (round missing), 'unknown_game', or { updated } — the number of games whose
// assignment actually MOVED.
//
// ADD AND REMOVE, never REPLACE, and that is the one semantic decision in this
// feature. Its sibling `setGameOwners` replaces, because a game has one owner
// set and there is a single right value to write. A 50-game selection carries
// 50 different tag sets, so replacing would silently strip every tag those games
// already had — the destructive answer, on the action whose whole purpose is
// organising a freshly imported shelf. Untouched tags stay untouched.
//
// `remove` wins over `add` for a value in both, which the tri-state chip cannot
// produce (one chip, one state) and a hand-rolled body can. Same precedence as
// the metadata exclude filters, and stated rather than left to statement order.
//
// A WISH is NOT skipped, unlike setGameOwners: a wish can carry tags today (the
// single PATCH takes `tagIds` for one) and organising the wish list is exactly
// the job this action exists for. Owners are different because a wish is nobody
// box yet.
//
// `updated` counts games that CHANGED, not games touched — so adding a tag the
// selection already carries reports 0 rather than 50, and the toast says
// something true.
//
// No activity: the single path (PATCH …/games/:gid with `tagIds`) writes none
// either, so a counted row here would announce in the Chronik something the
// per-game editor stays silent about (.claude/rules/bulk-paths-restate-the-single-path.md).
function applyTagDelta(game, add, remove) {
  const before = game.tagIds || [];
  const next = [...new Set([...before.filter((x) => !remove.includes(x)), ...add])]
    .filter((x) => !remove.includes(x));
  if (next.length === before.length && next.every((x) => before.includes(x))) return false;
  if (next.length) game.tagIds = next;
  else delete game.tagIds;
  return true;
}
...
async function setGameTags(tenant, rid, gameIds, { add = [], remove = [] } = {}) {
  ...
  let updated = 0;
  for (const game of picked) if (applyTagDelta(game, add, remove)) updated += 1;
  if (updated) saveData();
  return { updated };
}
...
async function moveGames(tenant, rid, targetRid, limits, gameIds) {
  ...
  // case-insensitive dedupe rule addTag uses (#238). Shared with copyGames and
  // with the Postgres backend (lib/repo/import-copy.js).
  const { remap, created, mergedTags } = mergeTagsInto(src.tags, moving, targetTags, id);
  ...
  for (const game of moving) {
    ...
    // Owners name members of the SOURCE round (#971), and the target has
    // different ones — so they are dropped rather than remapped, unlike tags
    // (which merge by name). Keeping them would silently filter the target's
    // draws on ids nobody there can match, i.e. hide the moved games from every
    // session while looking correct on the shelf.
    delete game.ownerIds;
...
// Copy games of one round onto ANOTHER EXISTING round's shelf (#916), merging the
// two rounds' tags by name exactly as moveGames does.
//
// Returns null when either round is missing, 'same_round' when they are the same,
// 'unknown_game' for an id the source round does not hold, one of
// 'quota_games'/'quota_tags' when `limits` is given and the copy would push the
// target past a cap, else { copiedGames, mergedTags, createdTags }.
//
// The three differences from moveGames are the whole feature, and each is load-
// bearing: every copied game is a FRESH row (new id, new expansion ids), the
// SOURCE round is not touched at all — no shelf filter, no session scrub, no vote
// deletion — and the copy carries no history, because ratings and votes are
// round-scoped and a copy is a different game.
//
// The cover PATH is shared rather than duplicated, exactly as createRound's
// import shares it: files are never rewritten, and one path referenced by several
// games is the shape isImageReferenced already answers for
// (.claude/rules/deletion-paths-must-free-cover-objects.md).
async function copyGames(tenant, rid, targetRid, limits, gameIds) {
  // Same order as moveGames: decided before the lookups, so the answer does not
  // depend on whether the id exists and Postgres can reach it identically.
  if (rid === targetRid) return 'same_round';
  const src = live(tenant, rid);
  const target = live(tenant, targetRid);
  if (!src || !target) return null;

  // The order comes from the SHELF, never from the request — same reason
  // moveGames documents, so both backends append the copies in one order.
  let copying = src.games;
  if (gameIds) {
    const want = new Set(gameIds);
    copying = src.games.filter((g) => want.has(g.id));
    if (copying.length !== want.size) return 'unknown_game';
  }
  const targetTags = target.tags || [];

  const { remap, created, mergedTags } = mergeTagsInto(src.tags, copying, targetTags, id);

  // Before any write, so a refusal changes nothing — and `tags` stays ABSENT on a
  // refused target that had none (absent-key parity with the Postgres NULL).
  if (limits) {
    if (target.games.length + copying.length > limits.maxGames) return 'quota_games';
    if (targetTags.length + created.length > limits.maxTags) return 'quota_tags';
  }
  ...
  for (const game of copying) target.games.push(copyGame(game, id(), remap, id));

  // ONE bulk entry per round, like the move's. The source's entry says a copy
  // left, never that games were lost — nothing left the shelf, which is also why
  // period-recap.js counts games_copied_in as growth and games_copied_out as
  // nothing at all.
  if (copiedGames) {
    addActivity(src, 'games_copied_out', { count: copiedGames, roundId: target.id, roundName: target.name });
    addActivity(target, 'games_copied_in', { count: copiedGames, roundId: src.id, roundName: src.name });
  }
...
// Set the round's retired per-round design. Returns { previous } (the prior
// background), or null if the round is gone. No route writes it since the flip
// (#1202) retired round designs; it stays in the repo contract because a stored
// `background` is still READ — a round that never picked a marker resolves to
// the marker its old design maps to (public/js/round-marker.js) — and the specs
// seed such legacy rounds through it.
...
// Set the round's colour marker (#1187): a design-neutral index 0-7, validated
// by the route. Returns the marker actually stored, or null if the round is gone
// or belongs to another tenant — indistinguishable, like every round-scoped
// method here. No activity entry: a marker is a label on the round, not
// something a co-owner needs an audit trail for, and the feed stays readable.
...
// Patch a tag's name and/or icon (#255 icon, #1004 rename). A key the patch
// omits is left alone; `icon: null` clears it back to the ABSENT key rather than
// an empty string. Returns the updated tag, `'name_taken'` when the new name
// collides with another tag of this round, or null when the round or the tag is
// gone.
//
// THE DEDUPE LIVES HERE, not in the route, and that is the whole shape of this
// method. `addTag` dedupes by name (trimmed, case-insensitive) and returns the
// existing tag, so two tags of one round can never share a name — except through
// a rename. Checking in the route puts the read and the write on either side of
// the Postgres backend's FOR UPDATE, so two concurrent renames both pass and
// produce a state `addTag` cannot: two tags, one name, different games,
// indistinguishable in every chip row.
//
// The tag's OWN entry is excluded from the check, which is what makes a pure
// case fix („kenner" -> „Kenner") reachable at all.
//
// Renaming touches nothing but the tag row: assignments are stored as `tagIds`
// on each game and remembered draw presets store `tagIds`/`excludeTagIds`, never
// names — which is precisely why this is cheaper than the delete-and-recreate it
// replaces (`deleteTag` unassigns the tag from every game that had it).
function applyTagPatch(tags, tagId, patch) {
...
// Put a round's tags in a hand-chosen order (#1159). Takes the FULL list of tag
// ids and requires it to be an exact permutation of the round's current ids;
// anything else returns 'tags_changed' and writes nothing.
//
// That strictness is the whole point, and it is the same race `applyTagPatch`
// guards for renames: a client holding a stale list would otherwise resurrect a
// tag another tab deleted, or silently drop one just created, because this
// mutator writes the array WHOLE rather than moving one entry. Rejecting sends
// the client back to the server, which is the only list that can be right.
//
// Nothing else moves: assignments live as `tagIds` on each game and as
// `tagIds`/`excludeTagIds` in remembered draw presets, always ids and never
// indices, so the order is free to change under them.
function permuteTags(tags, tagIds) {
  ...
  for (const tagId of ids) {
    ...
    // A REPEATED id lands here too: the first occurrence consumed the entry, so
    // the second reads as unknown. Without that, [a, a, b] against [a, b, c]
    // passes a length check and quietly clones one tag over another.
...
async function reorderTags(tenant, rid, tagIds) {
  const round = live(tenant, rid);
  if (!round) return null;
  const next = permuteTags(round.tags || [], tagIds);
  if (next === 'tags_changed') return next;
  round.tags = next;
  saveData();
  return clone(next);
}
...
async function updateTag(tenant, rid, tagId, patch) {
  const round = live(tenant, rid);
  if (!round) return null;
  const tag = applyTagPatch(round.tags, tagId, patch);
  if (!tag || tag === 'name_taken') return tag;
  saveData();
  return clone(tag);
}
...
/* ------------------------------ Saved filters ------------------------------ */
/*
 * A round's named session filters (#1328). Stored as an ARRAY on the round,
 * beside `lastSessionFilters`, and absent until the first one is saved
 * (absent-key parity with the Postgres backend's NULL column). Deleting the
 * last one leaves `[]`, as deleting the last tag does.
 *
 * The rules — unique name, the per-round cap, the exact permutation — live in
 * ./saved-filters.js and run INSIDE each mutator, never in the route: the
 * Postgres backend holds the list under FOR UPDATE, and a check outside it
 * could be raced past (the `updateTag` reasoning,
 * .claude/rules/data-access-layer.md).
 *
 * Each returns the touched entry (or the list), a string sentinel for a
 * refusal — 'name_taken', 'quota', 'filters_changed' — or null when the round
 * (or the filter) is gone.
 */
async function createSavedFilter(tenant, rid, filter, limit = Infinity) {
  const round = live(tenant, rid);
  if (!round) return null;
  const out = insertSavedFilter(round.savedFilters, filter, limit, id);
  if (typeof out === 'string') return out;
  round.savedFilters = out.list;
  saveData();
  return clone(out.entry);
}

// Only the name is patchable: what a filter HOLDS is replaced by saving a new
// one, which is how the setup screen expresses it.
async function updateSavedFilter(tenant, rid, fid, patch) {
  const round = live(tenant, rid);
  if (!round) return null;
  const out = renameSavedFilter(round.savedFilters, fid, (patch || {}).name);
...
async function reorderSavedFilters(tenant, rid, ids) {
  const round = live(tenant, rid);
  if (!round) return null;
  const out = permuteSavedFilters(round.savedFilters, ids);
  if (typeof out === 'string') return out;
  round.savedFilters = out.list;
  saveData();
  return clone(out.list);
}

// true/false (found) — a missing round reads like a missing filter, as
// deleteTag does.
async function deleteSavedFilter(tenant, rid, fid) {
  const round = live(tenant, rid);
  if (!round) return false;
  const out = removeSavedFilter(round.savedFilters, fid);
  if (!out) return false;
  round.savedFilters = out.list;
  saveData();
  return true;
 ...
 */
/* An adoption predicate, in one place: "this row uses the feature at all".
   Array.isArray first because half the fields it is asked about are ABSENT on a
   row that never used the feature (`guests`, `teams`, `expansions`, `tags`) —
   `.length` on undefined throws, and a truthiness test would count `{}`. */
const nonEmpty = (v) => Array.isArray(v) && v.length > 0;

/* NO `now` PARAMETER since #1124. Every time-dependent figure is gone — the two
   weekly series and the 30-day session window — so the method answers the same
   thing whenever it is called, and a date argument would be a parameter nothing
   reads. `publicGameAggregates` below still takes one; that is a different
   method with real calendar periods. */
/* How many PEOPLE rated something in this session (#1174).
 *
 * `votes[personId][gameId] = { rating }`, so a person key alone is not a vote:
 * the session's start writes one entry per joining seat and a seat that never
 * answered keeps an empty object. The funnel's „bewertet" stage asks for TWO,
 * because one is the host trying the flow out alone — which is precisely the
 * case the card exists to tell apart from a real evening.
 *
 * Not in public/js/session-outcome.js beside its `sessionHasVotes` sibling: no
 * screen asks this, and the Postgres backend restates it in SQL either way, so
 * putting it there would add a shared-scope global nothing on the frontend uses.
 */
function sessionRaterCount(session) {
...
/* Did this session record HOW it ended — winners, or one of the three endings
 * (#1038)? Via the shared `sessionEnding`, never a bare `winnerIds.length`:
 * that is the exact site public/js/session-outcome.js's header says fails
 * silently, and it also gets the guards for free (a cancelled or split session
 * carrying stale winners is not a recorded result, and an `ending` value the
 * allowlist does not know reads as unrecorded rather than as a result).
 */
function sessionHasResult(session) {
  const ending = sessionEnding(session);
  return ending !== null && ending !== 'unrecorded';
}
...
async function instanceMetrics() {
  ...
  /* #1174: the Funktionsnutzung card may leave the operator's own tenants out,
     so that its shares describe other people's usage. The exclusion reaches
     EVERYTHING that card renders — `adoption`, `designAdoption` and `social`, each
     with the matching denominator in `adoption` — and nothing else. A card
     whose numerator was filtered while its denominator was not would report
     shares over 100 %, which is worse than either choice; `content`, `rounds`,
     `accounts` and `peaks` therefore keep counting every real tenant, and
     lib/public-stats.js reads exactly those. */
  const excluded = new Set(excludedTenants());
  const counts = (tenantId) => !excluded.has(String(tenantId || ''));
  // The accounts the card's Konto-Funktionen shares are measured over. The
  // „Konten" tile itself is a BARE COUNT of everybody and stays on `users`.
  const adoptionUsers = users.filter((u) => counts(u.tenantId));
  // Tenants that hold at least one round, for „Konten ohne Runde" below. Built
  // from the same `rounds` list every other figure walks, so a round the demo
  // filter dropped cannot make an account look settled.
  const tenantsWithRound = new Set(rounds.map((r) => tenantOf(r)));
  ...
  // Rounds USING each shelf state, not games in it: "do groups reach for the
  // archive and the wishlist at all" is a different question from "how many
  // rows are archived", and the games total beside it already answers the
  // second one.
  let roundsWithRetired = 0;
  let roundsWithCompleted = 0;
  let roundsWithWish = 0;
  /* The UNION of the three, which the three cannot be added into: a round with
     an archived game AND a wish counts once here and twice there. It is the
     „did this land at all" headline the three are the breakdown of. */
  let roundsWithAnyShelf = 0;
  /* The adoption figures (#1124): how many of the things that exist actually use
     a feature. All counted in THIS walk rather than in passes of their own, so
     they cannot disagree with the denominators beside them about which rounds
     were counted. */
  let roundsWithTags = 0;
  let roundsWithSavedFilters = 0;
  let gamesLinked = 0;
  let gamesWithOwnCover = 0;
  let gamesWithProviderCover = 0;
  let gamesWithOwners = 0;
  let gamesWithExpansions = 0;
  let sessionsWithGuests = 0;
  let sessionsWithTeams = 0;
  /* The adoption DENOMINATORS (#1174). Separate accumulators rather than a
     reuse of `games`/`sessions`/`rounds.length` above, because those count
     every real tenant while these count only the ones the card describes —
     identical whenever ADMIN_EXCLUDE_TENANTS is unset, which is the default and
     what CI runs. */
  let adoptionRounds = 0;
  let adoptionGames = 0;
  let adoptionSessions = 0;
  /* The session funnel (#1174). Every stage is an INDEPENDENT share of
     `started`, never a step whose predecessor it implies: a direct-pick session
     is created `done: true` with a game already chosen and `votes: {}`, so it
     is closed and chosen while never having been rated. Reading the lines as a
     monotonically shrinking pipeline is therefore wrong, and the card labels
     them accordingly.

     SPLIT PARENTS ARE NOT COUNTED AT ALL — their tables are. A parent that was
     split across three tables holds a real vote, was never played and is not an
     abandoned evening either (public/js/session-outcome.js); counting it would
     book a loss against „gespielt" for a round that played three games. Its
     children are ordinary sessions and are counted like any other. */
  const funnel = {
    started: 0, rated: 0, closed: 0, chosen: 0, played: 0, result: 0, cancelled: 0,
  };
  // Rounds by how many sessions they finished: none / exactly one / two or more.
  // THE THREE BANDS SUM TO `adoptionRounds`, which is what makes it a composite
  // tile rather than three unrelated shares — „zwei oder mehr" is the one figure
  // that says whether the app became a habit anywhere.
  const roundsByFinished = { none: 0, one: 0, many: 0 };
  for (const r of rounds) {
    ...
    // Does this round appear on the Funktionsnutzung card at all (#1174)? Every
    // adoption numerator AND the denominator beside it hang off this one flag,
    // so the two cannot be filtered differently.
    const counted = counts(tenant);
    if (counted) adoptionRounds += 1;
    games += r.games.length;
    ...
    activeGames += r.games.filter((g) => !g.retired && !g.completed && !g.wish).length;
    if (counted) {
      adoptionGames += r.games.length;
      if (r.games.some((g) => g.retired)) roundsWithRetired += 1;
      if (r.games.some((g) => g.completed)) roundsWithCompleted += 1;
      if (r.games.some((g) => g.wish)) roundsWithWish += 1;
      if (r.games.some((g) => g.retired || g.completed || g.wish)) roundsWithAnyShelf += 1;
      if (nonEmpty(r.tags)) roundsWithTags += 1;
      if (nonEmpty(r.savedFilters)) roundsWithSavedFilters += 1;
      for (const g of r.games) {
        ...
        if (g.source && g.source.provider) gamesLinked += 1;
        /* The two cover kinds are mutually exclusive and neither is a superset:
           `/uploads/` is bytes this instance stores (upload or paste), anything
           else is hotlinked from the provider (#172). Their sum is the headline
           „carries any cover", which is why the predicate splits rather than
           counting covers twice. */
        const image = typeof g.image === 'string' ? g.image : '';
        if (image.startsWith('/uploads/')) gamesWithOwnCover += 1;
        else if (image) gamesWithProviderCover += 1;
        if (nonEmpty(g.ownerIds)) gamesWithOwners += 1;
        if (nonEmpty(g.expansions)) gamesWithExpansions += 1;
      }
    }
    ...
    for (const s of r.sessions) {
      ...
      if (!counted) continue;
      adoptionSessions += 1;
      if (s.finished === true) finishedHere += 1;
      // `guests` and `teams` are ABSENT keys on an ordinary session by design
      // (#532/#575), so the predicate is "present and non-empty" — a `.length`
      // on an assumed array would throw on every plain evening.
      if (nonEmpty(s.guests)) sessionsWithGuests += 1;
      if (nonEmpty(s.teams)) sessionsWithTeams += 1;
      if (sessionChildIds(s).length) continue; // a split parent: its tables count, it does not
      funnel.started += 1;
      if (sessionRaterCount(s) >= 2) funnel.rated += 1;
      if (s.done === true) funnel.closed += 1;
      if (s.chosenGameId) funnel.chosen += 1;
      if (s.finished === true) funnel.played += 1;
      if (sessionHasResult(s)) funnel.result += 1;
      if (s.cancelled === true) funnel.cancelled += 1;
    }
    if (counted) {
      if (finishedHere === 0) roundsByFinished.none += 1;
      else if (finishedHere === 1) roundsByFinished.one += 1;
      else roundsByFinished.many += 1;
    }
  }
  ...
  const sharedRounds = new Set(
    data.roundGrants
      .filter((g) => !isDemoTenant(g.ownerTenantId) && counts(g.ownerTenantId))
      .map((g) => g.roundId),
  );
  // Accounts belonging to an excluded tenant, for the friendship filter below —
  // a friendship row carries two ACCOUNT ids and no tenant, exactly like the
  // demo exclusion beside it.
  const excludedUserIds = new Set(
    data.users.filter((u) => !counts(u.tenantId)).map((u) => u.id),
  );
  ...
  return {
    accounts: {
      ...
      /* How many have uploaded a Konto-Bild (#841). `u.avatar` is falsy for
         null, undefined AND '' — the Postgres side must use
         `coalesce(data->>'avatar', '') <> ''` to agree, because
         `is not null` would count a cleared-to-empty-string row that this does
         not (#941; the takedown path writes `avatar: null`). */
      withAvatar: users.filter((u) => !!u.avatar).length,
    },
    rounds: {
      total: rounds.length,
    },
    /* `activeGames`, `members` and `sessionsFinished` no longer appear on the
       operator card — #1124 dropped their tiles as inventory the operator cannot
       act on — but they are NOT dead: lib/public-stats.js's COUNTERS read
       exactly these three for the public landing counters (#564). Deleting them
       blanks that block silently, because an absent field simply fails its
       threshold. `games` and `sessions` stay as the adoption denominators. */
    content: {
      games, activeGames, members, sessions, sessionsFinished,
    },
    adoption: {
      roundsWithRetired, roundsWithCompleted, roundsWithWish, roundsWithAnyShelf,
      roundsWithTags,
      roundsWithSavedFilters,
      gamesLinked, gamesWithOwnCover, gamesWithProviderCover,
      gamesWithOwners, gamesWithExpansions,
      sessionsWithGuests, sessionsWithTeams,
      /* Distinct (round, session) pairs, which is what the Postgres unique index
         on session_vote_links already guarantees — counted the same way on both
         backends so a stray duplicate row could not make them disagree. The
         table carries its tenant in the row rather than as a column, so the demo
         filter goes through `tenantId`. */
      sessionsWithVoteLink: new Set(data.sessionVoteLinks
        .filter((l) => !isDemoTenant(l.tenantId) && counts(l.tenantId))
        .map((l) => `${l.roundId}\u0000${l.sessionId}`)).size,
      accountsWithPasskey: adoptionUsers.filter((u) => Array.isArray(u.identities)
        && u.identities.some((i) => i && i.type === 'passkey')).length,
      accountsWithBggUsername: adoptionUsers.filter((u) => !!u.bggUsername).length,
      /* The card's own DENOMINATORS (#1174). Every share on „Funktionsnutzung"
         divides by one of these four rather than by `accounts.total`,
         `rounds.total` or `content.*`, which are instance-wide and would make a
         share exceed 100 % the moment a tenant is excluded. With
         ADMIN_EXCLUDE_TENANTS unset — the default, and what CI runs — each one
         equals its instance-wide twin. */
      accountsTotal: adoptionUsers.length,
      roundsTotal: adoptionRounds,
      gamesTotal: adoptionGames,
      sessionsTotal: adoptionSessions,
      /* „mit Konto-Bild" as an adoption share. `accounts.withAvatar` beside it
         stays instance-wide because it belongs to the accounts block; this is
         the one the card divides. */
      accountsWithAvatar: adoptionUsers.filter((u) => !!u.avatar).length,
      /* Registered and never started anything (#1174) — the first stage of the
         funnel, one level up. Measured by TENANT, which is the only link that
         exists: an account creates rounds under its own tenant (#136). Where a
         tenant is shared (#138's invites), an invited account reads as settled
         because its tenant holds a round — the card's note says so rather than
         pretending to a per-account answer the data cannot give. */
      accountsWithoutRound: adoptionUsers.filter(
        (u) => !tenantsWithRound.has(u.tenantId || DEFAULT_TENANT),
      ).length,
      // The BG Stats hand-off (#485), which is OFF by default — so this is a
      // real opt-in count rather than an artefact of the registration default.
      accountsWithBgStats: adoptionUsers.filter((u) => u.bgStats === true).length,
      funnel,
      roundsByFinished,
    },
    /* Which design accounts wear, and how many went back to Klassisch (#1201) —
       the per-user successor of #941's per-round histogram. One group per
       account; lib/account-design.js does the resolving and the counting for
       both backends. `=== true`, never `!== false`: an account predating the
       field carries no key and did not switch
       (.claude/rules/defaulted-account-fields-need-a-legacy-shape-spec.md). */
    designAdoption: tallyDesignAdoption(adoptionUsers.map((u) => ({
      design: u.design,
      answered: Boolean(u.designChooserSeen),
      switched: u.designSwitchedBack === true,
      n: 1,
    }))),
    social: {
      ...
      invitationsOpen: data.invitations
        .filter((i) => i.status === 'pending'
          && !isDemoTenant(i.ownerTenantId) && counts(i.ownerTenantId)).length,
      ...
      friendships: data.friendships.filter((f) => f.status === 'accepted'
        && !demoUserIds.has(f.requesterUserId) && !demoUserIds.has(f.addresseeUserId)
        && !excludedUserIds.has(f.requesterUserId)
        && !excludedUserIds.has(f.addresseeUserId)).length,
    },
 ...
 * distinct tenants own a game, how often it was played in the current calendar
 * week/month/year, and the rating mass it has collected. GLOBAL (cross-tenant),
 * so it stays out of TENANT_METHODS — but unlike every other global method it is reached
 ...
 *
 * `now` is an argument rather than a Date.now() call so the three play periods
...
async function publicGameAggregates(now = new Date().toISOString()) {
  // CALENDAR periods on the Europe/Berlin clock, not rolling day counts (#964):
  // the cards are labelled „diese Woche / diesen Monat / dieses Jahr", and a
  // window of the last 7/30/365 days describes a different period on every date
  // that is not a Monday / the 1st / January 1st. Derived in lib/calendar-periods.js
  // rather than here because the Postgres backend needs the identical boundary
  // and test/support/repo-contract.js compares the two.
  ...
  //
  // `all` is an ALL-TIME count and its cutoff is the empty string, which every
  // string sorts at or above — so it needs no branch of its own here and stays
  // provably the same walk as the three real windows. The Postgres side spells
  // it `count(*)`, which is the same set for the same reason: `playedAt` is
  // coalesced to '' there, so the filter admits every row. It feeds the
  // all-time „Dauerbrenner" card (#1035). It fed the Spielwirbel-Score's play
  // lift too until #1329, which gave the lift its own count (`plays.lift`,
  // below) because the Regal also counts plays that were never finished.
  const windows = { week: period.week, month: period.month, year: period.year, all: '' };
  ...
  const rowFor = (key) => {
    ...
    if (!row) {
      row = {
        ...
        owners: new Set(),
        plays: {
          week: { count: 0, tenants: new Set() },
          month: { count: 0, tenants: new Set() },
          year: { count: 0, tenants: new Set() },
          all: { count: 0, tenants: new Set() },
          lift: { count: 0, tenants: new Set() },
        },
        ratings: { tiles: TILE_VALUE.map(() => 0), count: 0, tenants: new Set() },
      };
      rows.set(key.id, row);
    }
    return row;
  ...
  for (const round of data.rounds) {
    ...
    for (const s of round.sessions) {
      // A PLAY is a finished session and the game it settled on, dated by
      // `createdAt` — when the evening WAS, which is what every other surface
      // shows (#1059). Deliberately NOT `finishedAt`: finishSession rewrites
      // that on every winner-chip tap, so a correction weeks later would drag
      // the play into the current week and month while the Chronik entry stays
      // put. No fallback either — every creation path writes `createdAt`, and
      // the empty string keeps a stamp-less row out of the calendar windows
      // while still counting in `all`.
      /* The LIFT count (#1329) is a different set: the Regal's `playCounts`
         (public/js/vote-score.js) — every NON-CANCELLED session with a
         `chosenGameId`, finished or not — so the Discover score's play lift is
         the shelf's by construction. The period counts above stay
         finished-only; they are "most played" tiles, not a score.

         A split evening is not counted twice: its parent never carries a
         `chosenGameId` (lib/routes/sessions.js refuses both `/choice` and
         `/split` once the other has happened), so only the child tables count,
         once each — exactly as `playCounts` reads them. */
      if (!s.cancelled && s.chosenGameId) {
        const key = linked.get(s.chosenGameId);
        if (key) {
          const row = rowFor(key);
          row.plays.lift.count += 1;
          row.plays.lift.tenants.add(tenant);
        }
      }
      if (s.finished === true && s.chosenGameId) {
        const key = linked.get(s.chosenGameId);
        if (key) {
          const playedAt = String(s.createdAt || '');
          const row = rowFor(key);
      ...
      for (const perPerson of Object.values(s.votes || {})) {
        for (const [gameId, vote] of Object.entries(perPerson || {})) {
          // Only a vote landing on an actual TILE is counted, so the histogram,
          // `count` and `tenants` are all over one set and `count` can never
          // disagree with the buckets it totals. The Postgres side spells the
          // same admission test in SQL, over the same 1–5 tiles — keep the two
          // together. A fractional rating is not a tile the vote card can
          // produce, and it is skipped here for the same reason `scoreRatings`
          // skips it rather than interpolating a step function.
          const rating = vote && typeof vote === 'object' ? vote.rating : null;
          if (tileValue(rating) === null) continue;
          const key = linked.get(gameId);
          if (!key) continue;
          const row = rowFor(key);
          row.ratings.tiles[rating] += 1;
          row.ratings.count += 1;
  ...
  return [...rows.values()].map((row) => ({
    ...
    plays: {
      week: { count: row.plays.week.count, tenants: row.plays.week.tenants.size },
      month: { count: row.plays.month.count, tenants: row.plays.month.tenants.size },
      year: { count: row.plays.year.count, tenants: row.plays.year.tenants.size },
      all: { count: row.plays.all.count, tenants: row.plays.all.tenants.size },
      lift: { count: row.plays.lift.count, tenants: row.plays.lift.tenants.size },
    },
    ratings: { tiles: row.ratings.tiles.slice(), count: row.ratings.count, tenants: row.ratings.tenants.size },
  }));
}
