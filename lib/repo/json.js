...
// One definition of what counts as a storefront link (#981), shared with the
// Postgres backend so the two can never disagree about which rows the operator
// is being asked to clear.
...
async function clearStorefrontLinks({ hosts, providerIds, dryRun }) {
  ...
  for (const round of data.rounds) {
    let touched = 0;
    for (const game of round.games || []) {
      const m = storefrontMatch(game, hosts, providerIds);
      ...
      touched += 1;
      if (dryRun) continue;
      if (m.cover) game.image = null;
      if (m.source) delete game.source;
    }
    if (!touched) continue;
    run.rounds += 1;
    tenants.add(tenantOf(round));
    // One entry per ROUND, not per game: a shelf with thirty storefront links
    // would otherwise bury its own Chronik under thirty identical lines.
    if (!dryRun) addActivity(round, 'storefront_cleared', { n: touched });
  }
