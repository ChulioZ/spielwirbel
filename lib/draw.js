...
//
// `memberIds` (#971) are the SEATS at the table — the ids the caller already
// resolved, not a headcount — and they drive the owner clause: a game whose
// recorded owners are all absent cannot be put on the table. It is a separate
// parameter from `playerCount` rather than derived from it because the two answer
// different questions and are computed differently: `playerCount` counts parties
// (guests in, teams flattened), while ownership is about seats only. Absent — a
// caller that predates this — the clause is a no-op, which is what keeps every
