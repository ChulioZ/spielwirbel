 ...
 *
 * The moderation read path (lib/routes/admin/) is deliberately NOT affected:
 ...
 * nothing for that.
 *
 * Not lib/provider-cache.js's Map, although its header says not to re-roll the
 * expiry logic: that cache promises "nothing personal is cached" and these keys
 * carry USER ids, and it has no eviction cap where this key space is one entry
 * per image on every <img> GET. Twenty lines is under the threshold at which a
 * dependency (lru-cache) would pay for itself.
 */
const TTL_MS = 60_000;
