...
const PRIVACY_REVISION = '2026-09-24';
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    /* DER TISCH AS THE FACE (#1198). These pages predate the app's tokens and
       ...
       <html data-design="tisch"> from FACE_DESIGN (public/js/designs.js). Until
       the flip (#1202) nothing below matches. Every value is pinned against
...
bestbewerteten und deines stärksten Spiels. <strong>Rundennamen, andere
...
„Feedback“ — ebenso über die einmalige Frage nach der ersten Session einer
Runde und nach dem ersten Import einer BoardGameGeek-Sammlung. Gespeichert werden
die Nachricht, der App-Bereich, aus dem du sie gesendet hast, die Sprache und,
falls du über eine dieser Fragen gekommen bist, welche es war; eine
E-Mail-Adresse nur, wenn du sie einträgst,
...
<ul>
  ...
  Hinweis auf diesem Gerät kein zweites Mal erscheint. Ebenso wird vermerkt,
  für welche Runden (nach ihrer ersten Session) und für welches Konto (nach dem
  ersten Sammlungsimport) dir die einmalige Frage nach Feedback schon gezeigt
  wurde — nur die Kennungen, damit sie nicht wiederkommt. Läuft diese Instanz
...
game. <strong>Round names, other members, individual sessions and individual
...
with the “Feedback” category preselected — as does the one-off question after a
round's first session and after your first BoardGameGeek collection import. We
store the message, the app area you sent it from, your language and, if you came
through one of those questions, which one it was; an e-mail address only if you
enter one, so we
...
<ul>
  ...
  note does not appear a second time on this device. Likewise, the device
  records for which rounds (after their first session) and for which account
  (after the first collection import) the one-off request for feedback has
  already been shown — identifiers only, so it does not come back. If this
  instance runs
