...
Rechtsgrundlage ist die Bereitstellung des Dienstes (Art. 6 Abs. 1 lit. b
DSGVO).</p>
<p>Sofern der Betreiber es aktiviert hat, veröffentlichen wir
<strong>instanzweite Gesamtzahlen</strong> auf der öffentlichen Startseite und
unter <code>/entdecken</code>: die Zahl der Konten, Runden, Spiele und gespielten
Abende sowie einige meistbesessene, meistgespielte und bestbewertete Spiele.
...
Test-Zugängen (Demo) zählen nicht mit. Rechtsgrundlage ist unser berechtigtes
Interesse, den Dienst nachvollziehbar darzustellen (Art. 6 Abs. 1 lit. f
DSGVO).</p>
...
<p>Hat der Betreiber die instanzweiten Statistiken (Abschnitt 5) aktiviert,
werden solche Cover auch auf der <strong>öffentlichen Startseite</strong> und
unter <code>/entdecken</code> angezeigt. Dann nimmt der Browser
...
(Art. 6(1)(b) GDPR).</p>
<p>Where the operator has switched it on, we publish <strong>instance-wide
totals</strong> on the public landing page and at <code>/entdecken</code>: the
number of accounts, rounds, games and nights played, plus a few of the
most-owned, most-played and best-rated games. These are
...
rankings at all. Rounds belonging to trial (demo) accounts are excluded. Legal
basis: our legitimate interest in describing the service transparently
(Art. 6(1)(f) GDPR).</p>
...
<p>Where the operator has switched on the instance-wide statistics (section 5),
such covers are also shown on the <strong>public landing page</strong> and at
<code>/entdecken</code>. Your browser then contacts
