...
<h2>9. E-Mail-Versand (Mailjet)</h2>
...
Meldungen) versenden wir über <strong>Mailjet SAS, Paris,
Frankreich</strong> — einen EU-Anbieter, mit dem ein
Auftragsverarbeitungsvertrag besteht. Die Verarbeitung findet in
EU-Rechenzentren statt (Google Cloud, Frankfurt und Belgien; Google als
Unterauftragsverarbeiter, Standardvertragsklauseln). Verarbeitet werden dabei
...
uns per E-Mail über Mailjet zugestellt und im Postfach des Betreibers gespeichert,
...
<ul>
  ...
  Instanz, solange die Registrierung noch nicht geöffnet ist — verwenden ein
  Sitzungs-Cookie („sid“, 30 Tage). Sind beide Verfahren zugleich aktiv
  (Konten hinter dem Gruppen-Passwort), werden beide Cookies gesetzt.</li>
...
Erklärung genannten Empfänger (Railway, Cloudflare, Mailjet, Heinlein Hosting,
ZERODOX — jeweils
...
<h2>9. E-mail delivery (Mailjet)</h2>
...
<strong>Mailjet SAS, Paris, France</strong> — an EU provider under a
data-processing agreement. Processing takes place in EU data centres (Google
Cloud, Frankfurt and Belgium; Google as a sub-processor, standard contractual
clauses). Recipient address, subject and message content are processed for
delivery (Art. 6(1)(b) GDPR).</p>
...
Mailjet and kept in the operator's mailbox as long as handling requires. The same
...
<ul>
  ...
  When both mechanisms are active at once (accounts behind the group
  password), both cookies are set.</li>
...
(Railway, Cloudflare, Mailjet, Heinlein Hosting, ZERODOX — each to the
