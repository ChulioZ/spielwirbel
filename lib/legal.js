...
// The face design (#1198), stamped onto <html> by layout() so the page picks its
// presentation on the server. Presentation only — no document text depends on it.
...
const PRIVACY_REVISION = '2026-09-26';
...
function layout(title, deHtml, enHtml) {
  return `<!DOCTYPE html>
<html lang="de" data-design="${FACE_DESIGN}">
<head>
  <meta charset="UTF-8" />
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    /* DER TISCH AS THE FACE (#1198). These pages predate the app's tokens and
       their Klassisch look above is frozen as it is — so unlike /faq there is no
       Klassisch token copy here, only Der Tisch's, applied when layout() stamps
       <html data-design="tisch"> from FACE_DESIGN (public/js/designs.js) — the
       face since the flip (#1202). Every value is pinned against
       Der Tisch's own resolved token by test/standalone-page-brand.test.js.
       PRESENTATION ONLY: not one sentence of any legal text is touched, so no
       revision constant moves (.claude/rules/keep-legal-docs-current.md). */
    ...
    /* Der Tisch's two faces (#1198), for the face block above. Declared always,
    ...
    /* „Eine Spalte Papier" (T12.2): the page is the walnut table, the body the
       printed column laid on it. html carries the wood because body is now the
       paper. Brass is not a colour on paper, so links take --accent-deep. */
    html[data-design="tisch"] { background: var(--page-bg); }
    ...
    :root[data-design="tisch"] h1,
    :root[data-design="tisch"] h2,
    :root[data-design="tisch"] h3 { font-family: var(--font-display); font-weight: 800; }
    :root[data-design="tisch"] a { color: var(--accent-deep); }
    :root[data-design="tisch"] .meta,
    :root[data-design="tisch"] .foot { color: var(--paper-ink-soft); }
    :root[data-design="tisch"] .lang-note { background: var(--paper-raised); border: 1px solid var(--paper-edge); border-radius: var(--radius-sm); }
    :root[data-design="tisch"] .highlight { border-color: var(--paper-edge); border-left-color: var(--accent-deep); border-radius: var(--radius-sm); }
    :root[data-design="tisch"] hr.split { border-top-color: var(--paper-edge); }
...
<p>Geht in deinem <strong>Browser</strong> etwas schief, sendet die Anwendung
einen knappen technischen <strong>Fehlerbericht</strong> an den Server — sonst
bliebe ein Fehler, der nur auf bestimmten Geräten oder in bestimmten Browsern
auftritt, dem Betreiber vollständig verborgen. Übertragen wird ausschließlich
ein fest vorgegebener Satz technischer Felder: <strong>die Art des Fehlers</strong>
aus einer festen Liste, <strong>die Fehlermeldung</strong> des Browsers (auf 200
Zeichen gekürzt), <strong>Name und Zeile der betroffenen Skriptdatei</strong>,
<strong>die Art des Bildschirms in anonymisierter Form</strong> (z. B.
„/round/:rid/chronik“ — Kennungen von Runden, Spielen, Mitgliedern und
Abstimmungs-Links werden ersetzt und gar nicht erst übertragen) sowie
<strong>die eingestellte Sprache</strong>. Serverseitig ergänzt wird
<strong>Browser-Engine und Hauptversion</strong> (z. B. „WebKit/605“) — nie die
vollständige Browser-Kennung. <strong>Nicht</strong> übertragen werden der
vollständige Aufrufstapel, Seiteninhalte, Formulareingaben sowie Runden-,
Session-, Mitglieds- oder Kontodaten. Die Berichte liegen im
<strong>Arbeitsspeicher</strong> des Prozesses (begrenzte Zahl der jüngsten
Einträge, beim Neustart geleert) und gehen zusätzlich in die oben beschriebenen
Anwendungs-Logs ein, für die dieselbe Aufbewahrungsgrenze der Plattform gilt.
Zweck ist der stabile Betrieb (Fehlersuche); Rechtsgrundlage ist unser
berechtigtes Interesse an einem funktionierenden Dienst (Art. 6 Abs. 1 lit. f
DSGVO). Eine zusätzliche Speicherung auf deinem Gerät findet dafür nicht statt.</p>
...
es zu keinem Zeitpunkt gespeichert. Zur Bestätigung der E-Mail-Adresse, für das
Zurücksetzen des Passworts und für die Bestätigung einer von dir gewünschten
Änderung deiner E-Mail-Adresse werden kurzlebige Token erzeugt (ebenfalls nur
gehasht gespeichert) und dir per E-Mail zugesandt. Änderst du deine Adresse,
speichern wir die neue Adresse bis zur Bestätigung — längstens 24 Stunden — und
benachrichtigen anschließend die bisherige Adresse über die Änderung. Rechtsgrundlage ist die
Durchführung des Nutzungsverhältnisses (Art. 6 Abs. 1 lit. b DSGVO).</p>
<p>Du kannst deinem Konto freiwillig ein <strong>Profilbild</strong> geben. Es
ist optional — ohne Bild zeigt der Dienst weiterhin nur ein farbiges Feld mit
deinen Initialen, und du kannst ein hochgeladenes Bild jederzeit im Kontobereich
wieder entfernen. Ein Profilbild ist häufig ein Bild einer Person und damit eine
eigene Kategorie personenbezogener Daten; lade deshalb nur Bilder hoch, an denen
du die Rechte hast, und keine Bilder anderer Personen ohne deren Einverständnis
(Abschnitt 5 der Nutzungsbedingungen). Sichtbar ist es für angemeldete Konten
dort, wo dein Konto ohnehin erscheint: auf deiner Profilseite, im Freundeskreis
und dessen Neuigkeiten sowie auf einem Sitzplatz in einer Runde, den du mit
deinem Konto verknüpft hast. Wir wandeln jedes hochgeladene Bild serverseitig in
ein einheitliches quadratisches Format um und <strong>entfernen dabei die
Metadaten der Bilddatei</strong> — insbesondere <strong>GPS-Koordinaten</strong>
und den Aufnahmezeitpunkt, die Handyfotos üblicherweise enthalten. Diese Angaben
werden also gar nicht erst gespeichert. Rechtsgrundlage ist die Durchführung des
Nutzungsverhältnisses (Art. 6 Abs. 1 lit. b DSGVO).</p>
...
(Abschnitt 13).</p>
<p>Optional kannst du für dein Konto einen oder mehrere <strong>Passkeys</strong>
einrichten und dich damit statt mit deinem Passwort anmelden. Wir speichern dazu
je Passkey eine <strong>Anmeldedaten-Kennung</strong>, einen
<strong>öffentlichen Schlüssel</strong>, einen Zähler, die vom Gerät gemeldeten
Übertragungswege sowie eine von dir vergebene <strong>Bezeichnung</strong> und
die Zeitpunkte des Anlegens und der letzten Nutzung.
<strong>Biometrische Daten erreichen unseren Server zu keinem Zeitpunkt</strong>:
Fingerabdruck oder Gesicht verlassen dein Gerät nicht, und der private Schlüssel
verbleibt im Schlüsselspeicher deines Geräts oder deines Passwortmanagers. Der
öffentliche Schlüssel dient ausschließlich der Prüfung deiner Anmeldung; mit ihm
lässt sich nichts entsperren. Passkeys sind eine <em>zusätzliche</em>
Anmeldemöglichkeit — dein Passwort bleibt bestehen. Du kannst jeden Passkey
jederzeit in deinen Kontoeinstellungen umbenennen oder entfernen; danach ist er
gelöscht. Rechtsgrundlage ist die Durchführung des Nutzungsverhältnisses
...
wir die eingegebenen Inhalte: <strong>Rundennamen, Spieltitel, Namen der zu
einem Spiel eingetragenen Erweiterungen, Mitgliedsnamen,
Gästenamen, die Zuordnung, welchem Mitglied ein Spiel gehört, Bewertungen/Stimmen,
Schlagworte (Tags), eine Aktivitäten-Chronik
...
<strong>Namen der Teilnehmenden dieser Session</strong> (jeweils mit der
Angabe, ob diese Person bereits abgestimmt hat) und kann eine Stimme
...
und wird gelöscht, in jedem Fall aber <strong>spätestens 30 Tage</strong> nach
dem Erzeugen: eine Abstimmung, die niemand beendet, soll den Link nicht
unbegrenzt gültig lassen. Rechtsgrundlage ist die Bereitstellung des Dienstes
(Art. 6 Abs. 1 lit. b DSGVO).</p>
...
abschließt, sowie das Erreichen einer neuen Stufe eines
<strong>Konto-Abzeichens</strong> (nur dessen Name und Stufe, z. B. „Sessions
100“; nicht, wenn du deine Spielbilanz für Freundschaften ausgeschaltet hast).
Dabei werden <strong>keine</strong> Mitgliedsnamen, Bewertungen,
...
der Allgemeinheit statt. Einzelne Feed-Hinweise werden <strong>nach 12
Monaten</strong> automatisch gelöscht; die Freundschaftsbeziehung selbst bleibt
bis zum Beenden der Freundschaft bzw. zur Löschung des Kontos gespeichert.
Rechtsgrundlage ist die Bereitstellung des Dienstes
(Art. 6 Abs. 1 lit. b DSGVO).</p>
...
<strong>Nutzernamen</strong>, dein <strong>Profilbild</strong> – falls du keines
hochgeladen hast, ein aus dem Nutzernamen automatisch erzeugtes Farbsymbol – und
den <strong>Monat der Registrierung</strong> („Mitglied seit“) sowie den Stand einer
...
Einschränkungen wie im vorigen Absatz.</p>
<p>Zusätzlich zeigt das Profil eine <strong>zusammengefasste Spielbilanz</strong>
über alle Runden hinweg, an denen dein Konto einen Mitglieds-Platz hat – eigene
wie mit dir geteilte Runden. Sie besteht ausschließlich aus Kennzahlen und
Spieltiteln: Zahl der gespielten Sessions, Zahl der Siege, Siegquote,
dein Durchschnitt der von dir vergebenen Bewertungen, die Zahl der
Runden und der gespielten Spiele sowie der <strong>Titel</strong> deines
bestbewerteten und deines stärksten Spiels; dazu vier daraus und aus dem Monat
der Registrierung abgeleitete <strong>Konto-Abzeichen</strong> (Sessions, Siege,
Runden, Jahre), die nicht gespeichert, sondern bei jedem Aufruf neu berechnet
werden. <strong>Rundennamen, andere
Mitglieder, einzelne Sessions und einzelne Bewertungen werden nicht
übermittelt.</strong> Diese Bilanz sehen nur du selbst und Konten, mit denen du
befreundet bist; für Freundschaften lässt sie sich in den Kontoeinstellungen
abschalten („Bilanz für Freundschaften sichtbar“, Voreinstellung: an). Nicht
befreundete Konten und offene Freundschaftsanfragen sehen sie nie. Deine
E-Mail-Adresse wird auf dem Profil <strong>nicht</strong> angezeigt.
Ein Profilbild ist freiwillig; du kannst es
jederzeit in den Kontoeinstellungen ändern oder entfernen. Das Profil setzt eine Anmeldung voraus; es ist
für nicht angemeldete Besucher und für Suchmaschinen nicht zugänglich.
Rechtsgrundlage ist die Bereitstellung des Dienstes (Art. 6 Abs. 1 lit. b
DSGVO).</p>
<p>Wir veröffentlichen <strong>instanzweite Gesamtzahlen</strong> auf der
öffentlichen Startseite und unter <code>/entdecken</code>: die Zahl der Runden,
der Spieler*innen (Mitglieds-Plätze in allen Runden), der Spiele im Regal und der
gespielten Sessions sowie einige Spiele, die besonders oft im Regal stehen,
besonders oft gespielt oder besonders gut bewertet wurden.
Diese Werte sind <strong>Aggregate über alle Runden hinweg</strong> und lassen
keinen Rückschluss auf eine Person, ein Konto, eine Runde oder eine einzelne
Bewertung zu. <strong>Selbst eingegebene Spieltitel werden dabei nie
veröffentlicht</strong> — angezeigt wird ausschließlich der Name, den der
Anbieter (BoardGameGeek) zu einem verknüpften Spiel führt; Spiele ohne
Anbieter-Verknüpfung erscheinen in diesen Ranglisten gar nicht. Runden von
Test-Zugängen (Demo) zählen nicht mit. Jede Kennzahl erscheint zudem erst
oberhalb eines Mindestwerts, unterhalb dessen sie gar nicht veröffentlicht wird.
Rechtsgrundlage ist unser berechtigtes Interesse, den Dienst nachvollziehbar
darzustellen (Art. 6 Abs. 1 lit. f DSGVO).</p>

<h2>6. Hochgeladene Bilder</h2>
<p>Selbst hochgeladene Bilder — <strong>Cover-Bilder</strong> von Spielen und
das optionale <strong>Profilbild</strong> eines Kontos (Abschnitt 4) — werden in
einem S3-kompatiblen
...
<p>Ein Bild wird gelöscht, sobald es nicht mehr gebraucht wird: wenn du ein
Cover entfernst oder das Spiel löschst, wenn du dein Profilbild entfernst oder
durch ein neues ersetzt, wenn wir es nach einer Meldung entfernen
(Abschnitt 13) und in jedem Fall mit der Löschung des Kontos (Abschnitt 4).</p>
...
<p>Solche Cover werden auch in den instanzweiten Statistiken (Abschnitt 5)
angezeigt, also auf der <strong>öffentlichen Startseite</strong> und
unter <code>/entdecken</code>, sowie auf der Abstimmungsseite, die ein geteilter
Abstimmungslink (Abschnitt 5) öffnet. Damit nimmt der Browser
<strong>auch ohne Anmeldung</strong> Verbindung zu <strong>BoardGameGeek</strong>
(<code>geekdo-images.com</code>) auf — bis dahin geschah das nur innerhalb der
angemeldeten App. Andere Anbieter sind davon nicht betroffen: in diesen
Ranglisten erscheinen ausschließlich Spiele mit BoardGameGeek-Verknüpfung.</p>
...
<h2>8. Suchanfragen an Spiele-Anbieter (Titel-Suche und Preise)</h2>
<p>Die Titel-Suche beim Anlegen eines Spiels fragt den Katalog von
<strong>BoardGameGeek</strong> ab. Diese Anfragen stellt <strong>unser
Server</strong> — übermittelt werden nur der eingegebene Suchbegriff und die im
Dienst gewählte Sprache, nie deine IP-Adresse oder andere Daten über dich.</p>
<p>Öffnest du ein Spiel auf deiner <strong>Wunschliste</strong>, das mit einem
Anbieter verknüpft ist, rufen wir dafür den aktuellen Preis bei
<strong>Brettspielpreise.de / BoardGamePrices</strong> ab. Auch diese Anfrage
stellt unser Server; dein Browser nimmt keine
Verbindung dorthin auf. Übermittelt werden ausschließlich die
<strong>Kennung des Spiels</strong> beim jeweiligen Anbieter sowie Zielland,
Währung und die im Dienst gewählte Sprache — keine Daten über dich, auch nicht
deine IP-Adresse. Ein Empfänger personenbezogener Daten entsteht dadurch nicht.
Den zuletzt abgerufenen Preis speichern wir — je Spiel, Zielland, Währung und
Ausgabe genau einen Wert, der bei jedem erfolgreichen Abruf überschrieben wird.
Er wird nur angezeigt, wenn der Anbieter gerade nicht erreichbar ist, und dann
immer mit Angabe seines Alters; nach spätestens sieben Tagen wird er gelöscht
und nicht mehr angezeigt. Gespeichert wird dabei ausschließlich die Kennung des
Spiels und der Preis — nichts über dich, und keine Preisverläufe. Neben dem
Preis zeigen wir einen gewöhnlichen Link zur Angebotsübersicht beziehungsweise
zur Store-Seite; erst wenn <em>du</em> ihn öffnest, erreicht deine IP-Adresse den
dortigen Anbieter — genau wie bei jedem anderen Link auch. Wir verdienen an
diesen Links nichts: es gibt keine Affiliate- oder Provisionsverknüpfung
irgendeiner Art.</p>
...
<ul>
  ...
  15 Minuten), damit hochgeladene Cover- und Profilbilder nur für angemeldete
  Nutzer abrufbar sind; selbst gehostete Instanzen mit Gruppen-Passwort verwenden
  ...
  kannst, ohne erneut gefragt zu werden. Hast du den einmaligen Hinweis „App
  installieren“ weggeklickt, wird auch das vermerkt, damit er nicht erneut
  erscheint. Ebenso wird vermerkt, wenn dir am Ende einer Abstimmung per Link
  einmalig angezeigt wurde, um welchen Dienst es sich handelt — damit dieser
  Hinweis auf diesem Gerät kein zweites Mal erscheint. Läuft diese Instanz
  <strong>ohne Konten</strong>, wird zusätzlich das von dir gewählte
  <strong>Design</strong> der Oberfläche auf dem Gerät vermerkt; mit Konto
  gehört diese Einstellung zum Konto und wird nicht auf dem Gerät abgelegt.</li>
...
<h2>15. Übergabe einer Partie an BG Stats</h2>
<p>Wenn du es in deinem Konto aktivierst, zeigen wir auf der Ergebnisseite einer
beendeten Session einen Link zu <strong>BG Stats</strong>
(bgstatsapp.com) an — einer App, in der viele Gruppen ihre gespielten Partien
führen. Die Funktion ist <strong>standardmäßig ausgeschaltet</strong>; ohne diese
Einstellung erscheint der Link nicht.</p>
<p>Der Link enthält die Daten der Partie: das gespielte Spiel, Datum und Uhrzeit
des Abschlusses, eine Kennung der Session sowie die <strong>Namen aller
Teilnehmenden</strong> (auch der Gäste) und die Angabe, wer gewonnen hat.
Bewertungen und Punkte werden nicht übergeben. Von uns aus wird dabei nichts
gesendet: Es sind keine BG-Stats-Inhalte eingebettet, und die Übermittlung
findet erst statt, wenn du den Link selbst antippst — dann direkt von deinem
Gerät aus. Der Anbieter von BG Stats verarbeitet diese Daten als
<strong>eigenständiger Verantwortlicher</strong> nach seiner eigenen
Datenschutzerklärung; hat er dort eine Verbindung zu deinem
BoardGameGeek-Konto eingerichtet, kann er die Partie zusätzlich dorthin
übertragen. Rechtsgrundlage für das Bereitstellen des Links ist unser
berechtigtes Interesse, den Dienst mit anderen Werkzeugen zusammenarbeiten zu
lassen (Art. 6 Abs. 1 lit. f DSGVO). Da du dabei auch Namen anderer Personen
weitergibst, entscheide bitte bewusst, ob das für deine Runde in Ordnung ist.</p>

<h2>16. Weitergabe von Daten</h2>
...
den Spenden-Link selbst öffnest (Abschnitt 12), BG Stats nur, wenn du eine
Partie selbst dorthin übergibst (Abschnitt 15). Die Katalog- und Preis-Anbieter
aus Abschnitt 8 erhalten überhaupt keine Daten über dich — dorthin gehen nur
dein Suchbegriff beziehungsweise die Kennung eines Spiels. Innerhalb des Dienstes werden
Inhalte anderen Konten nur wie in Abschnitt 5 beschrieben zugänglich (geteilte
Runden, der Freundeskreis-Feed, dein Profil und von dir geteilte
Abstimmungslinks) – teils nur den von dir ausdrücklich bestätigten Konten, teils,
wie dort jeweils beschrieben, allen angemeldeten Konten oder den Empfängern eines
von dir geteilten Links. Ein Verkauf oder eine werbliche Nutzung der Daten findet
nicht statt.</p>
...
<p>When something goes wrong in your <strong>browser</strong>, the application
sends a brief technical <strong>fault report</strong> to the server — otherwise
a fault that only occurs on certain devices or in certain browsers would stay
entirely invisible to the operator. Only a fixed set of technical fields is
transmitted: <strong>the kind of fault</strong> from a fixed list, <strong>the
browser's error message</strong> (truncated to 200 characters), <strong>the name
and line of the script file concerned</strong>, <strong>the kind of screen in
anonymised form</strong> (e.g. "/round/:rid/chronik" — identifiers of rounds,
games, members and voting links are replaced and never transmitted at all), and
<strong>your selected language</strong>. The server adds <strong>the browser
engine and its major version</strong> (e.g. "WebKit/605") — never the full
browser identification. <strong>Not</strong> transmitted: the full stack trace,
page contents, form input, or any round, session, member or account data. The
reports are held in the process's <strong>memory</strong> (a limited number of
the most recent entries, cleared on restart) and additionally form part of the
application logs described above, to which the same platform retention limit
applies. Purpose: stable operation (debugging); legal basis: legitimate interest
in a working service (Art. 6(1)(f) GDPR). No additional storage on your device
is used for this.</p>
...
Short-lived tokens for e-mail verification, password reset and confirming an
address change you have asked for are stored hashed and sent to you by e-mail.
If you change your address, we store the new one until it is confirmed — at most
24 hours — and then notify the previous address about the change. Legal basis: performance of the user relationship
...
<p>You may optionally give your account a <strong>profile picture</strong>. It
is optional — without one the service keeps showing a coloured tile with your
initials, and you can remove an uploaded picture at any time in the account area.
A profile picture is often a picture of a person and therefore its own category
of personal data, so upload only pictures you hold the rights to, and no pictures
of other people without their consent (section 5 of the Terms of Use). It is
visible to logged-in accounts wherever your account already appears: on your
profile page, in your friends list and its updates, and on a seat in a round you
have linked to your account. Every uploaded picture is re-encoded server-side
into one uniform square format, and <strong>the image file's metadata is
stripped</strong> in the process — in particular the <strong>GPS
coordinates</strong> and capture time that phone photos normally carry. Those
details are therefore never stored in the first place. Legal basis: performance
of the user relationship (Art. 6(1)(b) GDPR).</p>
...
<p>You may optionally set up one or more <strong>passkeys</strong> for your
account and sign in with them instead of your password. For each passkey we
store a <strong>credential ID</strong>, a <strong>public key</strong>, a
counter, the transports your device reports, a <strong>label</strong> you choose
and the times it was created and last used.
<strong>No biometric data ever reaches our server</strong>: your fingerprint or
face never leaves your device, and the private key stays in your device's or
password manager's keystore. The public key serves only to verify your sign-in;
it cannot unlock anything. Passkeys are an <em>additional</em> way to sign in —
your password remains in place. You can rename or remove any passkey at any time
in your account settings, after which it is deleted. Legal basis: performance of
the user relationship (Art. 6(1)(b) GDPR).</p>
...
<strong>round names, game titles, the names of any expansions recorded for a
game, member names, guest names, which member owns which game, ratings/votes,
...
<strong>names of that session's participants</strong> (each with whether that
person has already voted), and can cast one vote —
...
deleted — but in any case <strong>after 30 days at the latest</strong>: a vote
nobody ever ends must not leave the link valid indefinitely. Legal basis:
providing the service (Art. 6(1)(b) GDPR).</p>
...
a game to a shelf or finish a session, and reaching a new tier of an
<strong>account badge</strong> (only its name and tier, e.g. “Sessions 100”; not
if you have switched your play record off for friends). <strong>No</strong> member names, ratings,
...
there is no dissemination to the public. Individual feed notes are deleted
automatically <strong>after 12 months</strong>; the friendship itself is kept
until it is ended or the account is deleted. Legal basis: providing the service
(Art. 6(1)(b) GDPR).</p>
<p>Every account has a <strong>profile</strong> within the service, reachable
under its username. Signed-in accounts see the <strong>username</strong> there,
your <strong>profile picture</strong> — or, if you have not uploaded one, a
colour symbol derived from the username automatically — the <strong>month of
registration</strong> ("Member since"), and the state of any friend request
between you. That account's Freundeskreis feed is shown there only if you are
friends, under the same restrictions as the previous paragraph.</p>
<p>The profile additionally shows an <strong>aggregated play record</strong>
across every round in which your account holds a member seat — your own rounds
and rounds shared with you alike. It consists solely of figures and game titles:
the number of sessions played, the number of wins, the win rate,
the average of the ratings you have given, the number of rounds and of games
played, and the <strong>titles</strong> of your highest-rated and your strongest
game, plus four <strong>account badges</strong> derived from those figures and your
month of registration (Sessions, Wins, Rounds, Years), which are not stored but
recomputed on every view. <strong>Round names, other members, individual sessions and individual
ratings are never transmitted.</strong> This record is visible only to you and to
accounts you are friends with; it can be switched off for friends in your account
settings ("Show my record to friends", default: on). Accounts you are not friends
with, and pending friend requests, never see it. Your e-mail address is
<strong>not</strong> shown on the profile.
A profile picture is optional; you can change or remove it at any
time in your account settings. The profile requires a sign-in; it is not accessible to
signed-out visitors or to search engines. Legal basis: providing the service
(Art. 6(1)(b) GDPR).</p>
<p>We publish <strong>instance-wide totals</strong> on the public landing page
and at <code>/entdecken</code>: the number of rounds, of players (member seats
across all rounds), of games on shelves and of sessions played, plus a few games
that sit on especially many shelves, were played especially often, or are rated
especially well. These are
<strong>aggregates across all rounds</strong> and permit no conclusion about any
person, account, round or individual rating. <strong>Game titles you typed are
never published</strong> — only the name the provider (BoardGameGeek) holds for a
linked game is shown, and games with no provider link do not appear in these
rankings at all. Rounds belonging to trial (demo) accounts are excluded. Each
figure additionally appears only above a minimum, below which it is not
published at all. Legal basis: our legitimate interest in describing the service
transparently (Art. 6(1)(f) GDPR).</p>

<h2>6. Uploaded images</h2>
<p>Images you upload — game <strong>cover images</strong> and an account's
optional <strong>profile picture</strong> (section 4) — are stored in
...
<p>An image is deleted as soon as it is no longer needed: when you remove a
cover or delete the game, when you remove your profile picture or replace it
with a new one, when we remove it following a report (section 13), and in every
case when the account is deleted (section 4).</p>
...
<p>Such covers are also shown in the instance-wide statistics (section 5), i.e.
on the <strong>public landing page</strong> and at
<code>/entdecken</code>, and on the voting page a shared voting link (section 5)
opens. Your browser therefore contacts
<strong>BoardGameGeek</strong> (<code>geekdo-images.com</code>) <strong>even
without signing in</strong> — until then this only happened inside the
signed-in app. No other provider is affected: only games linked to BoardGameGeek
can appear in those rankings.</p>
...
<h2>8. Requests to game providers (title search and prices)</h2>
<p>The title search when adding a game queries the <strong>BoardGameGeek</strong>
catalogue. These requests are made by <strong>our server</strong> — only the
typed search term and the language selected in the app are transmitted, never
your IP address or any data about you.</p>
<p>When you open a game on your <strong>wish list</strong> that is linked to a
provider, we look up its current price at
<strong>Brettspielpreise.de / BoardGamePrices</strong>. That request is likewise
made by our server; your browser makes no
connection there. Only the <strong>game's id</strong> at that provider, plus the
destination country, the currency and the language selected in the app, are
transmitted — no data about you, and not your IP address. No recipient of
personal data arises from this. We do store the last price retrieved — exactly
one value per game, destination country, currency and edition, overwritten on
every successful lookup. It is shown only while the provider cannot be reached,
and then always with its age stated; after seven days at the latest it is deleted
and no longer shown. Only the game's id and the price are stored — nothing about
you, and no price history. Beside the price we
show an ordinary link to the offer overview or the store page; only when
<em>you</em> open it does your IP address reach that provider — exactly as with
any other link. We earn nothing from these links: there is no affiliate or
commission arrangement of any kind.</p>
...
<ul>
  ...
  uploaded cover images and profile pictures are only served to logged-in
  users; self-hosted
  ...
  so you can change your vote until voting ends without being asked again. If you
  dismissed the one-off "install the app" prompt, that is recorded too, so it
  does not appear again. The same applies once you have been shown the one-off
  note naming this service at the end of a link vote — it is recorded so that
  note does not appear a second time on this device. If this instance runs
  <strong>without accounts</strong>, the interface <strong>design</strong> you
  picked is stored on the device as well; with an account that preference
  belongs to the account and is not kept on the device.</li>
...
<h2>15. Sending a play to BG Stats</h2>
<p>If you enable it in your account, a finished session's results screen shows a
link to <strong>BG Stats</strong> (bgstatsapp.com), an app many groups keep their
play log in. The feature is <strong>off by default</strong>; without that setting
the link does not appear.</p>
<p>The link carries the play: the game, the date and time it was finished, an
identifier for the session, the <strong>names of everyone who took part</strong>
(guests included) and who won. Ratings and scores are not sent. Nothing is sent
by us: no BG Stats content is embedded, and the transfer happens only once you
tap the link yourself — directly from your device. The provider of BG Stats
processes that data as an <strong>independent controller</strong> under their own
privacy policy, and if you have connected it to your BoardGameGeek account there,
they may post the play on to it as well. The legal basis for offering the link is
our legitimate interest in letting the service work together with other tools
(Art. 6(1)(f) GDPR). Since you also pass on other people's names, please decide
deliberately whether that is appropriate for your round.</p>
...
(section 12), and BG Stats only when you send a play there yourself
(section 15). The catalogue and price providers in section 8 receive no data
about you at all — only your search term, or a game's id, goes there.
Within the service, content is made visible to other accounts only
as described in section 5 (shared rounds, the Freundeskreis feed,
your profile and the voting links you share) — partly only to the accounts
you have explicitly confirmed, partly, as described there, to all logged-in
accounts or to the recipients of a link you shared. Data is never sold or used
for advertising.</p>
