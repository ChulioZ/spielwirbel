...
//
/* `owners` indexes into the SEAT list the same way `winners`/`ratings` below do
   (0 = the visitor's own seat), resolved to member ids at seed time. It exists
   so the demo shows ownership at all (#1008): without it the results ranking's
   „Gehört …" line, the member page's „Spiele von …" section and the setup
   screen's "their owners are away" note are all unreachable for a demo visitor,
   i.e. three shipped features look like they do not exist.

   Two constraints, both asserted in test/demo-seed.test.js:
     - every index must be a real seat of THAT round — `ownedByParty` keeps a
       game only when an owner is in the joining party, so a stray index would
       quietly drop the game from every draw;
     - a WISH never carries owners. The round does not have that box yet, and
       the games route forces `ownerIds: []` on a wish for the same reason.
   Leaving a game out entirely is the ordinary case: most rounds record nothing,
   and a line on every row would be noise rather than news. */
...
const MAIN_SESSIONS = [
  ...
  },
  {
    ...
    gameIndexes: [3, 4, 6],
    chosenIndex: 4,
    /* „Just One" is COOPERATIVE, so nobody wins it individually — which is
       exactly the state that had no representation before #1038 and that the
       seed should therefore show. `winners` and `ending` are mutually exclusive
       (lib/routes/sessions.js enforces it), so this session records the ending
       and the one above keeps the winner: a demo visitor meets both. */
    winners: [],
    ending: 'lost',
...
const DEMO_TAGS = {
  classic: { icon: 'cards', de: 'Klassiker', en: 'Classic', es: 'Clásicos', fr: 'Classiques', it: 'Classici', nl: 'Klassiekers', pt: 'Clássicos', fi: 'Klassikot', ko: '명작' },
  family: { icon: 'users', de: 'Familie', en: 'Family', es: 'Familiar', fr: 'Famille', it: 'Famiglia', nl: 'Familie', pt: 'Família', fi: 'Perhe', ko: '가족' },
  party: { icon: 'sparkles', de: 'Party', en: 'Party', es: 'Fiesta', fr: 'Ambiance', it: 'Festa', nl: 'Feest', pt: 'Festa', fi: 'Bileet', ko: '파티' },
  duel: { icon: 'sword', de: 'Zu zweit', en: 'Two player', es: 'Para dos', fr: 'À deux', it: 'In due', nl: 'Met z’n tweeën', pt: 'Para dois', fi: 'Kaksinpeli', ko: '2인용' },
};
...
/* The three rounds a demo tenant is seeded with, in home-screen order.
   ...
   Each carries its own colour MARKER (#1187), and the three are distinct in
   every design — indexes 0, 1 and 2 are Tannen-, Kobalt- and Burgunderfilz
   under Der Tisch and Standard, Blaugrau and Salbei under Klassisch — so the
   lobby shows a visitor that rounds are told apart by colour. Until the flip
   (#1202) retired round designs, two of them wore a world instead.
...
const DEMO_ROUNDS = [
  { key: 'main', marker: 0, games: MAIN_GAMES, sessions: MAIN_SESSIONS },
  { key: 'duo', marker: 1, games: DUO_GAMES, sessions: [] },
  { key: 'group', marker: 2, games: GROUP_GAMES, sessions: [], split: GROUP_SPLIT },
];
...
const DEMO_TEXT = {
  ...
  pt: {
    ...
    ownerSeat: 'Mikko',
    rounds: {
      main: { name: 'Peliporukka (demo)', members: ['Anni', 'Eero', 'Sanna'] },
      duo: { name: 'Kaksinpeli (demo)', members: ['Eero'] },
      group: { name: 'Iso porukka (demo)', members: ['Anni', 'Eero', 'Sanna', 'Ville', 'Leena'] },
    },
    guests: ['Tuuli', 'Joonas'],
  },
  ko: {
    ownerSeat: '준호',
    rounds: {
      main: { name: '보드게임 모임 (데모)', members: ['지민', '현우', '서연'] },
      duo: { name: '2인용 (데모)', members: ['현우'] },
      group: { name: '큰 모임 (데모)', members: ['지민', '현우', '서연', '도윤', '하은'] },
    },
    guests: ['민준', '예진'],
  },
};
