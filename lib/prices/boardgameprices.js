...
function parseInfo(json, { lang, destination } = {}) {
  ...
  if (!best) return null;
  // Property 4 above: when the shown box includes the reader's language, that
  // match is the honest label; the item's own first language labels only the
  // most-offers fallback, where no reader-language edition existed.
  const want = wantedLang(lang);
  return {
...
function cacheKey(externalId, requested) {
  ...
  const lang = wantedLang(requested) || '-';
