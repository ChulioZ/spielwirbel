 ...
 *
 * One source, dispatched on the link the game already carries:
 *
 *   game.source.provider === 'bgg' -> ./boardgameprices  (the aggregator)
 *
 * There was a second — Steam's `price_overview` — until #744 retired the four
 * digital storefronts. A Steam-linked game is now simply a game whose provider
 * has no price source, which is an already-handled path rather than a new one.
 *
...
// A successful lookup that found no price — "nobody stocks this".
// Distinguishable from UNAVAILABLE (#707) because the client may state "no price
// available" only for a settled answer, never for an upstream that was merely
// unreachable. Frozen for the same shared-cache reason.
const NO_OFFERS = Object.freeze({ available: false, reason: 'no_offers' });
 ...
 * per source is also what keeps a second source answering while one is out.
 *
 * Process-local, like the cache and the rate limiters (#215's shared store was
 * closed unshipped) — during the few seconds a deploy overlaps two containers
 * each holds its own. That is fine: this is politeness and cost control, not
 * correctness.
 */
 ...
 *
 *  - It never REPLACES a live answer. The fallback branches read it only when
 *    the lookup is unavailable, and the second reader (#707's `storedPriceFor`,
 *    the `?stored=1` fast path) renders it only until the live answer lands,
 *    always age-labelled. A successful lookup that finds no price ("nobody
 *    stocks this") is a settled answer, and answering it with last week's price
 *    would contradict fresh data.
...
// The BGG language values of the edition the round picked its cover from (#742),
// or []. Read HERE, once, so the live lookup, the cache key and the stored
// fallback are all asking about the same box — the same reason the key itself is
// the source's own (.claude/rules/last-known-price-fallback.md §1).
//
// A game whose cover predates #742, or whose cover was pasted rather than picked,
// carries no edition and keeps exactly today's behaviour: the reader's locale
// decides.
function editionLanguagesOf(game) {
  const ed = game && game.edition && typeof game.edition === 'object' ? game.edition : null;
  return ed && Array.isArray(ed.languages) ? ed.languages : [];
}
...
function sourceFor(game) {
  ...
  return source ? { source, externalId, editionLangs: editionLanguagesOf(game) } : null;
}

// The stored last-known price alone (#707) — the instant answer behind the
// client's stale-while-revalidate read (`?stored=1`). It must NEVER fetch
// upstream, never touch the cooldown or the in-memory cache, and never write
// anything: it exists so the wish page can show something while the real
// `priceFor` call is still in flight, and a fast path with side effects would
// race the very lookup it fronts. `lastKnown` already enforces the
// PRICES_FALLBACK_MAX_AGE_DAYS ceiling and the `stale: true` labelling.
async function storedPriceFor(game, lang) {
  if (!pricesEnabled()) return UNAVAILABLE;
  const hit = sourceFor(game);
  if (!hit) return UNAVAILABLE;
  return (await lastKnown(hit.source.cacheKey(hit.externalId, lang, hit.editionLangs))) || UNAVAILABLE;
}
...
async function priceFor(game, lang) {
  ...
  const hit = sourceFor(game);
  if (!hit) return UNAVAILABLE;
  const { source, externalId, editionLangs } = hit;
  ...
  const key = source.cacheKey(externalId, lang, editionLangs);
  try {
    const payload = await cachedIf(
      ...
      async () => {
        ...
        const p = await source.price(externalId, lang, editionLangs);
        if (!p) return NO_OFFERS;
