...
async function remove(publicPath) {
  ...
  await fs.promises
    ...
    .catch((err) => {
      // ENOENT is the documented no-op above. Everything else (EACCES, EIO) is
      // a delete that did NOT happen, which leaves a cover the app believes it
      // removed — and telling the two apart is the whole point of the line
      // (#1148).
      if (err.code === 'ENOENT') return;
      logger.warn({
        event: 'storage_delete_failed', backend: 'disk',
        key: path.basename(publicPath), code: err.code || err.name,
      });
    });
}
...
async function size(publicPath) {
  ...
  const stat = await fs.promises
    ...
    .catch((err) => {
      logger.warn({
        event: 'storage_size_failed', backend: 'disk',
        key: path.basename(publicPath), code: err.code || err.name,
      });
      return null;
    });
...
async function read(publicPath) {
  ...
  return fs.promises
    ...
    .catch((err) => {
      logger.warn({
        event: 'storage_read_failed', backend: 'disk',
        key: path.basename(publicPath), code: err.code || err.name,
      });
      return null;
    });
...
async function usage() {
  const names = await fs.promises.readdir(UPLOAD_DIR).catch((err) => {
    logger.warn({ event: 'storage_usage_failed', backend: 'disk', code: err.code || err.name });
    return [];
  });
  ...
  for (const name of page) {
    const stat = await fs.promises.stat(path.join(UPLOAD_DIR, name)).catch(() => {
      unreadable += 1;
      return null;
    });
  ...
  // ONE line per call, carrying a count — never one per file. This loop runs up
  // to USAGE_MAX times, and the warn/error ring buffer the operator panel reads
  // holds 200 entries, so a per-file line would evict everything else in the
  // panel the moment the folder went unreadable (#1148).
  if (unreadable) {
