...
function createS3Storage(opts = {}) {
  ...
  async function remove(publicPath) {
    ...
    try {
      ...
      // Best effort for the CALLER, not silent for the operator: a delete that
      // fails leaves a cover the app believes it removed (#1148).
      logger.warn({
        event: 'storage_delete_failed', backend: 's3',
        key: path.basename(publicPath), message: err.message,
      });
    }
  }
  ...
  async function size(publicPath) {
    ...
    try {
      ...
      logger.warn({
        event: 'storage_size_failed', backend: 's3',
        key: path.basename(publicPath), message: err.message,
      });
      return null;
    }
  ...
  async function read(publicPath) {
    ...
    try {
      ...
      logger.warn({
        event: 'storage_read_failed', backend: 's3',
        key: path.basename(publicPath), message: err.message,
      });
      return null;
    }
  ...
  async function usage() {
    ...
    for (let page = 0; page < USAGE_MAX_PAGES; page += 1) {
      ...
      try {
        ...
        logger.warn({ event: 'storage_usage_failed', backend: 's3', message: err.message });
  ...
  async function serve(req, res) {
    ...
    try {
      ...
      // Not a missing object but a failing store: every cover in the app is
      // broken while this lasts, so it is ours and it is an error (#1148).
      logger.error({ event: 'storage_serve_failed', backend: 's3', key, message: err.message });
    ...
    out.Body.on('error', (err) => {
      logger.error({ event: 'storage_serve_failed', backend: 's3', key, message: err.message });
      if (res.headersSent) res.destroy(); else res.sendStatus(502);
    });
