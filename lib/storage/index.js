 ...
 * async remove(publicPath), async removeAll() -> count, and an Express `serve`
 * handler mounted at /uploads — so nothing else in the app changes when the
 * backend does. The default stays local disk, so local dev, existing installs,
 * and the test suite need no S3 (and the @aws-sdk/client-s3 dependency is only
 * loaded when S3 is configured).
 */
