...
module.exports = {
  ...
  // Guarded for the same reason as size(): basename() of a hotlinked provider
  // URL would read OUR object of that name and hand a stranger's bytes to a
  // caller that asked about theirs. Null ("not ours") is the honest answer.
  read: async (publicPath) => (isHostedImage(publicPath) ? backend.read(publicPath) : null),
  isHostedImage,
};
