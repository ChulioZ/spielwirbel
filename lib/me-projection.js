...
// The per-user design registry (#1184/#1186), required out of public/js on
// purpose — one list, no copy (.claude/rules/shared-constants-across-the-stack.md).
const { FACE_DESIGN, isSelectableDesign } = require('../public/js/designs');

// `undefined` for an account with no pending change, one whose record has
// expired, and one written before #1076 — three states the screen treats
// identically, and the only place that equivalence is decided.
const pendingEmailAddress = (user) => {
  const p = user && user.pendingEmail;
  if (!p || !p.email) return null;
  return Date.parse(p.expiresAt) > Date.now() ? p.email : null;
};

/* Which design this account wears (#1186), RESOLVED rather than echoed.

   Two different failures collapse into the same answer here, and both have to:

    - an account predating the field carries no key at all — the absent-key case
      this projection exists to normalise;
    - an account holding a design this instance does not offer. That is not
      hypothetical: `enabled` is resolved against NODE_ENV, so a design built and
      picked on a dev instance is a perfectly real stored value that production
      must not honour — and applyDesign() on the client is deliberately
      POLICY-FREE (public/js/design.js says why), so if this hands out an
      unoffered id the client will faithfully wear it.

   So the gate is here, on the way out, as an allowlist test. The other half is
   PATCH /me, which refuses the same set on the way in. */
const designOf = (user) => {
  const id = user && user.design;
  return isSelectableDesign(id, { production: process.env.NODE_ENV === 'production' })
    ? id : FACE_DESIGN;
};
...
const meProjection = (user) => ({
  ...
  /* The address a pending change is waiting on (#1076) — the ADDRESS ONLY, never
     the token hash or the timestamps. An EXPIRED record reads as null here too,
     so the screen never offers to resend a dead one; requesting again is what
     replaces it. Accounts predating this carry no key, which is exactly the
     absent-key case the projection exists to normalise. */
  pendingEmail: pendingEmailAddress(user),
  ...
  /* Whether accepted FRIENDS see this account's play statistics on its profile
     (#1089). Same `!== false` shape and the same reason: an account predating the
     field carries no key and must behave like one that has never touched the
     toggle — which for this preference is ON, because the aggregate is the thing
     the profile was given content for and a silent default of OFF would make the
     feature look unbuilt for every existing account. The subject always sees
     their own numbers regardless; this governs the friend branch only
     (lib/routes/profile.js). */
  statsVisible: user.statsVisible !== false,
  ...
  // The per-user design (#1186) — see designOf above for why it is resolved
  // rather than echoed.
  design: designOf(user),
  /* Which run of the first-start chooser this account has seen. `|| null` is
     what makes an account predating the field read as "has seen none", which is
     correct: it has not, and the chooser is the thing that tells it designs
     exist at all. Only the client compares this against DESIGN_CHOOSER_REVISION
     — the constant rides in public/js/designs.js, which the browser already
     holds, so sending it would be the server telling the client something it
     has (the same split the news dot uses, and the opposite of the terms pair
     above, which must not straddle two requests). */
  designChooserSeen: user.designChooserSeen || null,
});
