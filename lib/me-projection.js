...
// Which design an account wears (#1186) — resolved in lib/account-design.js,
// which the operator's design tile (#1201) reads too, so the two cannot
// disagree about what an account is wearing.
const { accountDesign } = require('./account-design');
...
/* Which design this account wears (#1186), RESOLVED rather than echoed — an
   absent key and a design this instance does not offer both fold into the face.
   lib/account-design.js carries the why; the other half is PATCH /me, which
   refuses the same set on the way in. */
const designOf = accountDesign;
