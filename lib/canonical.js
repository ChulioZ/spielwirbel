...
// Resolved per call so a live re-tune takes effect without a rebuild, and
// exported so anything that needs to name these hosts uses the SAME derivation
// this middleware acts on rather than re-deriving it from env and drifting.
// (The operator panel used to render them; #404 dropped that row.)
...
// The origin every MAILED link is built on — `APP_BASE_URL`, falling back to the
// local dev origin so a checkout with no env still produces a followable link.
// It lives here, beside the other "what is this instance's public address"
// answers, because two senders need it since #618 (lib/routes/account.js for
// verification/reset, lib/notify.js for the inbox notifications) and a copy in
// the second would drift from the first
// (.claude/rules/shared-constants-across-the-stack.md).
//
// Deliberately NOT derived from canonicalHost(): that one drives a 301 and has a
// safe default for hosts we do not control, whereas a wrong base here mails
// people a link into somebody else's origin. It stays explicitly configured.
//
// Mind the length of anything appended to it — a mailed link must fit on one
// quoted-printable line (.claude/rules/mailed-links-must-fit-one-qp-line.md).
function appBaseUrl() {
