...
| **`audit`** | Runs the full audit sweep — accessibility, legal, security, and Claude-file — in one pass, merges the results into one ranked report, and files issues only with your approval. |
...
merging). For dependency bumps, **`dependabot`** handles the batch. The four
**`*-audit`** skills (and the **`audit`** umbrella) run a research → self-critique
→ audit loop over accessibility, legal, security and the repo's own Claude files;
each keeps its criteria in a versioned `criteria.md` that changes only via a
reviewable PR.
