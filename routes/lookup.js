...
/*
 * Add-game lookup: proxies external game-database providers (BoardGameGeek for
 * now) so the browser never makes the cross-origin call itself. Mounted at
 * /api/lookup in lib/app.js.
 *
 *   GET /api/lookup/search?provider=bgg&q=catan
 *       -> { results: [{ providerId, title, year, thumbnail }] }
 *   GET /api/lookup/game?provider=bgg&id=13
