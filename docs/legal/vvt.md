...
| 3 | Runden-/Spieldaten | Nutzer + von ihnen eingetragene Mitglieder | Rundennamen, Spieltitel, Namen der zu einem Spiel eingetragenen Erweiterungen (#653), Mitgliedsnamen, Gästenamen (nur zu einer Session eingetragene Personen, #458), **Zuordnung, welchem Mitglied ein Spiel gehört** (#971 — Mitglieds-Ids derselben Runde am Spiel; steuert, welche Spiele bei einer Auslosung überhaupt in Frage kommen), Stimmen/Bewertungen, Tags, Aktivitäten | Kernfunktion des Dienstes | Art. 6 (1) b | Railway (DB) | USA (EU-Region; SCC) | bis Löschung durch Nutzer / Kontolöschung |
| 4 | Hochgeladene Bilder: **Cover-Bilder** von Spielen und das optionale **Profilbild** eines Kontos (#841) | Nutzer | Bilddateien. Beim Profilbild häufig ein **Bild einer Person** — eigene Datenkategorie, freiwillig, jederzeit entfernbar. Jedes hochgeladene Bild wird serverseitig neu kodiert; die **Datei-Metadaten (EXIF), insbesondere GPS-Koordinaten und Aufnahmezeitpunkt, werden dabei entfernt** und gar nicht erst gespeichert (Datenminimierung, Art. 5 (1) c) | Kernfunktion. Das Profilbild ist für **angemeldete** Konten sichtbar, wo das Konto ohnehin erscheint (Profilseite, Freundeskreis + dessen Neuigkeiten, verknüpfter Sitzplatz in einer Runde) — keine Veröffentlichung gegenüber der Allgemeinheit | Art. 6 (1) b | Cloudflare, Inc. (R2, AVV; DPF-zertifiziert) | USA (DPF/SCC) | bis Löschung des Spiels bzw. Entfernen/Ersetzen des Profilbilds, bei Entfernung nach Meldung (Zeile 13) und in jedem Fall bis Löschung des Kontos |
...
| 22 | Instanzweite öffentliche Statistiken (#564) | — (keine betroffenen Personen im Ergebnis) | **Im Ergebnis keine personenbezogenen Daten.** Veröffentlicht werden ausschließlich **Aggregate über alle Mandanten hinweg**: Gesamtzahlen (Mitglieds-Plätze über alle Runden — keine Konten —, Runden, Spiele, gespielte Sessions) sowie einzelne meistbesessene / meistgespielte / bestbewertete Spiele. Kein Bezug zu einer Person, einem Konto, einer Runde oder einer einzelnen Bewertung ist herstellbar; jede Kennzahl erscheint erst oberhalb eines konfigurierten Mindestwerts, und die Ranglisten verlangen zusätzlich mehrere **verschiedene Mandanten** hinter einem Wert. Der angezeigte **Spielname stammt ausschließlich vom Anbieter** (BoardGameGeek), nie aus dem vom Nutzer eingegebenen Titel — selbst eingegebene Titel können die Seite konstruktiv nicht erreichen. Demo-Mandanten sind ausgenommen | Den Dienst auf der öffentlichen Startseite und unter `/entdecken` nachvollziehbar darstellen | Art. 6 (1) f (berechtigtes Interesse an transparenter Darstellung des Dienstes); die Aggregation selbst verarbeitet Bestandsdaten aus Zeile 3 | Railway (DB) für die Aggregation; **BoardGameGeek (Geekdo, LLC)** für die Auflösung der Spielnamen — bereits Empfänger aus Zeile 9/15, der Abruf erfolgt **serverseitig** und zeitgesteuert, nicht pro Aufruf. **Neu:** die angezeigten Cover werden vom Browser **auch ohne Anmeldung** bei BoardGameGeek geladen (Hotlinking, Zeile 9) | USA (EU-Region; SCC) für die DB; USA für den Namensabruf bei BGG | Es wird nichts gespeichert: die veröffentlichte Momentaufnahme liegt nur im Arbeitsspeicher des Prozesses und wird bei jedem Neuaufbau ersetzt; mit `PUBLIC_STATS_ENABLED=false` entfällt sie vollständig |
