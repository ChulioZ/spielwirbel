...
- **Games** – each game has a title, a required player range (min–max), any
  ...
  **expansions the round owns** for it — ticked off the list BoardGameGeek
  ...
  extension stops a six-person evening from hiding the game you own. Each owned
  entry says which table sizes it actually unlocks for *that* game
  („Ermöglicht 6 Personen"), rather than repeating its own interval for the
  reader to compare in their head. A game can
...
- **The Spielwirbel-Score** – the figure every screen ranks a game by is not a
  ...
  shrunk toward a fixed neutral expectation in proportion to how thin the
  evidence is, so a game three people rated 5,5,5 on its one evening stops
  outranking a staple forty votes agree on — and a game the group keeps choosing
  lifts that expectation, which is what finally gives a round that runs its
  evenings by direct pick a ranked shelf at all (those sessions write no votes).
...
- **Entdecken** – *live by default; PUBLIC_STATS_ENABLED=false takes it down* (issue #564). Publishes
  ...
  played this week / month / year / ever, and the best-rated. The three periods are
  *calendar* ones rather than the last 7/30/365 days (issue #964), so the
