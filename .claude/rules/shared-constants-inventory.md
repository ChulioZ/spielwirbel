...
# The shared-constant instances — the running inventory

The *rule* is `.claude/rules/shared-constants-across-the-stack.md`: what the
shape is, why a backend file requires out of `public/js/`, and the two cases
where a copy is licensed instead. Read that one first — it is global, and it is
the half every session needs.

**This file is the list**, and it is scoped to the places that can create an
entry, because that is the only time anyone needs it. (`lib/**` already covers
`lib/routes/**`, where most of the requiring happens — a second glob for it
would be dead weight the scope test would have to keep checking.) It was split out
in #978: at 470 lines it was 18 % of the always-resident rules slice by itself
and growing ~30 % every six days, since adding a paragraph here is the *required*
edit for every new shared file.

## The instances

**The first is `public/js/member-colors.js`** (#420): `MEMBER_COLORS`, the member
avatar palette — the incident the rule beside this file is written from. Offered
by the colour swatches, validated by `PATCH …/members/:mid` in
`lib/routes/members.js`. It is the plain shape in its purest form, and the reason
it heads the list rather than living only in the rule's own example: the check
...
required by `lib/routes/sessions.js`. It is the sharpest case of the first three, because the server is
...
check reads only the `## The instances` section, so mentioning a module in this
file's own header — or in the rule beside it — does not satisfy it.
