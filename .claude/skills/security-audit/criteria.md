...
- **Enforced by:** `test/ci-workflow.test.js` (the `uses:` SHA-pin shape) + `test/docker.test.js` (the base-image pin), both since #982 closed #977 on 2026-09-08. The rest — whether the update channel still delivers — stays manual, and #994 records the way it can freeze while looking healthy
