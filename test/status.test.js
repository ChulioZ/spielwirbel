...
const VARS = [
  ...
  'SMTP_HOST', 'SMTP_USER', 'SMTP_PASS', 'MAIL_FROM', 'APP_BASE_URL', 'CANONICAL_HOST', 'REDIRECT_HOSTS',
...
test('mail reports outbox-only when unconfigured', async () => {
  const off = await withEnv(
    { SMTP_HOST: undefined, SMTP_USER: undefined, SMTP_PASS: undefined, MAIL_FROM: undefined, APP_BASE_URL: undefined },
    instanceStatus,
  );
  ...
  // A partial credential set delivers nothing, so the card must not call it
  // configured — the #440 failure mode this row exists to catch.
  const halfConfigured = await withEnv(
    { SMTP_HOST: 'smtp.example.test', SMTP_USER: 'u', SMTP_PASS: undefined, MAIL_FROM: 'no-reply@example.com' },
    instanceStatus,
  );
