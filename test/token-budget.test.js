...
const RULE_ALLOW = {
  '.claude/rules/tiles-vs-lists.md': 'judged 2026-09-13 — 155 lines, crossed by #1092 adding the Der Kreis entry: the first screen that needs BOTH shapes at once, which is the case the file\'s two halves only ever applied to two different screens before. It sat at 141, and the growth is the kind this file is FOR — a per-screen ledger whose entries ARE the decisions. No seam: the general rule (when a set of things is a grid and when it is a list) is unreadable away from the screen-by-screen table that applies it, and the table is unreadable away from the rule — every entry is an argument about the same two shapes. Splitting by screen would file each decision where nobody comparing two screens would find it, which is the comparison the file exists to make possible',
