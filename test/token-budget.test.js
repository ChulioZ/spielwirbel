...
const SOURCE_ALLOW = {
  ...
  'test/a11y-contrast.test.js': 'judged 2026-09-13 — 816 lines, crossed by #937 adding the seal-padlock pair and grown again by #1053. It is the `criteria.md` shape this file already excludes by extension: a CATALOGUE of independent contrast assertions over one stylesheet, each ~10 lines of measurement plus the reason the bar is what it is. The apparent seams (per-token sweeps vs. per-component pairs vs. the white/placeholder exemption sweeps) are false ones — all three read the SAME per-design token resolution (THEMES, sweep, name, AA_TEXT/AA_LARGE), which is the file\'s spine, so a split either duplicates it or invents a support module for four call sites. Worse, the exemption sweeps are defined by CONTRAST with the pairs above them ("no bare white outside the rules that justify one"), so filing them apart is how an exemption outlives the rule that licensed it. #1053 added a fourth kind — resolving the CASCADE for one element rather than naming a token pair — and that one DID have a seam: its ~100 lines of selector matching moved to test/support/css.js beside specificity(), which is the parsing this file already leans on. What stayed is four more measurements reading the same spine',
  ...
  'lib/demo-seed.js': 'judged 2026-09-06 — 736 lines, crossed by #953 resolving the provider metadata: ~500 of them are GENERATED rows (scripts/resolve-demo-covers.js prints the three game blocks, and a single game carries up to 53 mechanics on one line), which is the flat-data-table case public/js/lang/** is excluded outright for. The visible seam — the game tables vs. DEMO_TEXT — is a false one: DEMO_ROUNDS binds a round `key` to its games and DEMO_TEXT keys its round names and members off that SAME field, so filing them apart lets the two drift on precisely the field that joins them, and adding a round would mean editing two data files instead of one. The file already IS the data half of a split (lib/demo.js holds the logic), which its header states as the reason a cover refresh touches a file with no code in it',
  'test/demo.test.js': 'judged 2026-09-06 — 731 lines, crossed by #953 growing the seed from one round to three (the metadata stamp, the per-round design and the split are each asserted through the real write path, not just over the table). SPLIT once already, and along the seam that was real: the seed-table assertions are synchronous reads of an exported table needing no app, no supertest and no DATA_DIR, and they left as test/demo-seed.test.js. What remains is one lifecycle — gate, mint, bounds, event exclusion, ending, purge — sharing a ~45-line preamble that raises THREE separate ceilings on one endpoint (AUTH/RATE_LIMIT_MAX, MAX_LIVE_DEMOS_PER_IP, DEMO_RATE_LIMIT_MAX). Splitting further would copy that preamble, and a copy that raises two of three reads as exhaustive — which is exactly the failure .claude/rules/derived-loops-outgrow-fixed-test-ceilings.md was written up for, one file over',
  'test/round-worlds.test.js': 'recorded 2026-09-16 at 893 lines — grown by #1138, whose additions (a per-world alpha read, the rising-fade floor guard and the poster accent-name bar) all land in the stylesheet-contract half described below, which sharpens rather than blurs the seam. Previously recorded 2026-09-15 at 724 lines — crossed by #1085 turning the design picker\'s world cards into posters (the DOM markup, the crown rule and the contrast stand-down). SPLIT once already, by #1082, which took the SVG arc guard out to test/world-arc-guard.test.js. A second seam is visible and clean: the jsdom half (the data-world hook, the picker, the home tile — loadApp, ~210 lines) against the stylesheet-contract half (the token blocks, the nine slots, the media gates, the alpha budget — CSS/rulesOf/slotBody, ~500). #1082 and #1083 touched only the second. What argues against cutting it here is that #1085 straddles it in ONE feature — a card is markup plus the rule that paints it, and its poster spec asserts both — so the split needs a session that can judge which half each of the four inbound rule citations meant, which a UI slice mid-build cannot (.claude/rules/token-friendly-source-files.md). M-001\'s worklist item',
  ...
  'test/games.test.js': 'judged 2026-08-13 — one suite per subject (the games router), the admin/account shape; crossed by #742\'s stored-edition specs. The big sub-features already have their own files (game-expansions, game-cover-refresh, game-price-section), so what is left IS the one concern: creating and editing a game row. The edition specs in particular cannot leave — they assert what each way of changing a COVER does, so they need this file\'s addGame/PNG_BYTES kit and sit beside the cover specs they qualify',
  'test/filter-panel.test.js': 'judged 2026-08-28 — one suite for ONE shared control, crossed by #844 moving its body into an overlay. The visible seam (setup screen vs Regal) is exactly the one that must not exist: the whole point of #827 is that both screens carry the same control, so a per-screen split lets a case exist for one of them only — the reason support/repo-contract.js is allowlisted, one layer up. The #736 backfill specs are the other candidate and cannot leave either: they assert the SAME control appearing on both screens, through the same deferredApi kit. The opening kit itself did move out, to test/support/filter-panel.js, because three files need it',
  ...
  'lib/routes/account.js': 'judged 2026-09-08 at 973 lines — on the list since 2026-07-30 as `recorded`; the seam test was applied by the audit sweep and the answer is KEEP. The account surface has ALREADY been split four ways: invitations, friends, profile and passkeys are their own routers, mounted separately in lib/app.js:343-368. What is left is one credential lifecycle (register/verify/login/refresh/reset) plus the Art. 15/17 self-service pair that hangs off the same account identity — the cohesive-flow shape token-friendly-source-files.md names as the non-finding for views-session.js, not several independently editable concerns. Splitting the lifecycle itself would put the token issue and the token redemption in different files. Contrast lib/routes/admin.js, which reads as the same kind of entry and is not (#996)',
  ...
  'test/recommend.test.js': 'recorded 2026-09-22 at 2052 lines — grown by #1227\'s nine affinity-ladder cases (the anchor per tile x voter count, the A_RETIRED invariant swept over all 25 cells, the decay curve in literals, its two exclusions, and the profile-composition case that replaced the issue\'s own top-N overlap measurement — see recommendation-scoring.md §15 for why that one could not be used). The verdict below is unchanged: they are independent cases in one module\'s spec, and each carries the arithmetic it asserts, which is the part a reader needs. Previously recorded 2026-09-07 at 1573 lines — on the list since 2026-08-15, re-measured 2026-08-19 at 1331; 854 when crossed by #775. It is one module\'s spec and a catalogue of independent cases, the shape criteria.md files are excluded outright for: each case isolates ONE term against its exact weight (§1), so the length is the coverage. The per-case fixtures (shelfCorpus, tasteCorpus, standoutCorpus, twoAxisCorpus) are what a reader needs beside the case that uses them. Splits with lib/recommend.js if that one ever does',
...
/* `lib/routes/admin.js` LEFT this list in #996 rather than being re-judged: the
   seam test had already passed on it (16 self-labelled sections, four owned by
   disjoint docs), so it was split into lib/routes/admin/, every part of which is
   under budget. That is the outcome the list exists to produce — a `recorded`
   entry is a worklist item, not a permanent exemption.

/* The rest were `recorded` — "over budget, nobody has looked" — from 2026-07-30
...
const RULE_ALLOW = {
  '.claude/rules/account-profiles.md': 'judged 2026-09-15 \u2014 166 lines, crossed by #1089 giving the screen real content: \u00a72b (the self feed has no cutoff, and the demo may read its own profile) and \u00a74 (the play record is absent-never-empty, and the redaction lives in lib/user-stats.js rather than in the route). It sat at 127, and both sections record a spelling that reads correctly and is wrong \u2014 keying the demo refusal off `target.id === me` reopens a username oracle, which is what was written here first. No seam: every numbered section is a consequence of ONE placement decision (this route is mounted ahead of the /api tenant gate), and each is only legible beside that premise \u2014 \u00a71 and \u00a74 in particular are the same argument about what a route outside the gate must re-check for itself. The row-link section is the one candidate to move, since #1089 split the profile view out of views-friends.js and that section is about the Kreis rows; it stays because two other rules cite it here and it is 18 lines. M-001\u2019s worklist item',
  '.claude/rules/browser-pane-is-chromium-only.md': 'judged 2026-09-15 \u2014 160 lines, crossed by #1091 adding the non-persistent-data-store section, and grown again by the canvas-taint cross-link (a second claim the pane could not fail on, which is precisely what the “when to reach for the probe” list is for). It sat at 130, and the growth is the kind this file is FOR: the addition is a probe failure that cost a full 13-width sweep, reporting numbers that contradicted the stylesheet on disk because the app\u2019s own cache-first service worker had installed into WKWebView\u2019s default store and survived between runs. Five lines of genuine redundancy were trimmed first; going further would have cost a `why`, which is the outcome .claude/rules/token-friendly-source-files.md says not to buy with a number. No seam: the file is one claim (a layout verified in the pane is a Chromium claim) and the single procedure that answers it \u2014 the Swift probe, its four gotchas and now its caching trap are unreadable away from the claim that makes anyone build it, and the claim is useless without them',
  '.claude/rules/server-computed-calendar-periods.md': 'judged 2026-09-13 — 190 lines, crossed by #1080 adding §8: §3 already banned `now - N * 86400000` and the file was `paths:`-scoped away from the one frontend module that shipped three of them, so the new section is as much about the SCOPE list two screens up as about the arithmetic. It sat at 138. No seam: every section is a clause of one contract (a date on a screen is a calendar fact, never an elapsed span), and §8 is defined by contrast with §3 and §6 — a count of days versus a window boundary, and why a day-offset fixture cannot see either. Filing §8 apart would let a session read the boundary rule without the count instance that proves the boundary rule was not enough, which is the specific failure it records',
  '.claude/rules/tiles-vs-lists.md': 'judged 2026-09-16 — 207 lines. #1136 added 32: it CORRECTED the ordering half (a chronology in a row-major grid keeps its sequence, so the ban is on column-major, not on order as such) and rewrote the Freundeskreis entry as history the way Member and Game-detail already are. Re-judged rather than trimmed, because those 32 lines are the file\'s highest-value kind — a reading that was wrong, kept beside why. Previously judged 2026-09-14 at 175 lines, when TWO screens crossed it in the same batch: #1092 added the Der Kreis entry (the first screen needing BOTH shapes at once, where the file\'s two halves had only ever applied to two different screens) and #1074 rewrote the Member entry as history, the way the Game-detail one below it already is. It sat at 141. The growth is the kind this file is FOR — a per-screen ledger whose entries ARE the decisions, so the two that record "this reading was wrong and here is why" are the highest-value lines in it. No seam: the general rule (when a set of things is a grid and when it is a list) is unreadable away from the screen-by-screen table that applies it, and the table is unreadable away from the rule — every entry is an argument about the same two shapes. Splitting by screen would file each decision where nobody comparing two screens would find it, which is the comparison the file exists to make possible',
  '.claude/rules/css-text-assertions-strip-comments.md': 'judged 2026-09-15 — 159 lines, crossed by #1083 adding a four-line pointer to the lookup trap\'s own file. It sat EXACTLY at the ceiling, so the seam test was applied rather than the pointer dropped: the file is one learning (a regex over source text binds inside a comment) written out in the three media it bites in — CSS, HTML and JS — and each section is the same lesson with a different remedy, so splitting it would leave three files that only make sense read together. The lookup trap, which is a different concern, DID get its own file rather than a fourth section.',
  '.claude/rules/theme-derived-colors.md': 'recorded 2026-09-15 at 227 lines — first recorded 2026-09-12 at 181, crossed by #937 adding the non-flipping-fill section (the gold padlock, and why --gold-deep is not its ink), and grown since by the world slots themselves (#1082, #1083). A seam IS visible and it is clean: the "Worlds" section is about the artwork HOOK (a data-world attribute and nine slots) over round-designs.js, where the rest is about token derivation over styles.css. Not split here because #937 is a two-declaration contrast fix and a rule split is its own change — guessing which half each inbound citation meant, mid-fix, is how a pointer ends up wrong (.claude/rules/token-friendly-source-files.md). M-001\'s worklist item',
  '.claude/rules/reparenting-rows-between-rounds.md': 'judged 2026-09-08 — 165 lines, crossed by #971 recording that `ownerIds` is DROPPED rather than remapped by all three reparenting paths. It sat at 154, so the next field with a cross-round meaning was always going to trip it. No seam: every section answers ONE question — what happens to a game\'s row when it changes parent — and the copy/import half is defined by CONTRAST with the move (fresh rows vs. reparented ones, shelf state kept vs. reset, which quota gate applies), so filing them apart is how someone reimplements the `seq` re-mint or the tag remap for the sibling verb and gets it subtly wrong. The new bullet is specifically a field that behaves differently across the two, i.e. exactly the comparison a split would hide',
  ...
  '.claude/rules/setup-screens-two-column-layout.md': 'judged 2026-09-11 — 199 lines, crossed by #1015 adding §5 and extended by #1017 (§3 now also states what the two pool presentations ARE, since the pot replaced the tile grid and the overlapping strip): the two-column layout of #577 was NOT enough on its own, and the layout-only alternative was prototyped and measured failing. It sat at 130. No seam: this is ONE screen pair\'s layout contract and every section is a clause of it — §1 is the exemption, §2 the licence for choosing a width by content at all, §3 the two pool presentations that live inside it, §4 the DOM order the columns impose, and §5 is defined BY CONTRAST with §1-4 (widening a column cannot remove content), so filing it away is precisely how the next session re-derives the alternative that was already measured failing. The measured tables are what a trim would take, and they are the reason the file exists rather than a restatement of its heading',
  // judged 2026-08-01
  ...
  '.claude/rules/locale-set-is-data.md': 'judged 2026-08-23 — 157 lines, crossed by #534 recording that shipping French silently un-synthesized the three "locale registered at runtime" tests, which had pushed the code `fr`. The visible seam (§1 vacuity / §2 the i18n.js redeclaration / §3 the one-other plural cap) is a false one: all three are consequences of the SAME decision (#504 made the locale set data), and the new paragraph is specifically §1\'s trap re-entering through §1\'s own fix — it is unreadable away from the loop it qualifies. Adding a language is also exactly when all three are needed at once',
  '.claude/rules/popover-vs-sheet-editors.md': 'judged 2026-08-29 — 170 lines, crossed by #844 routing the filter panel through openEditor (the fourth editor, and the first from outside game detail) and adding the onClose hook every exit funnels through. A seam is visible — §4 is CSS sharing over styles.css while §1-3 are JS over core.js + views-round-detail.js — and it is a false one: §4 exists BECAUSE one builder fills two containers, which is §2\'s convention, and its "this rule stays popover-only" calls are the ones that decide the height caps in the neighbouring file. Whoever adds a fifth editor needs all of it in one read, which is the audience the file is written for',
  '.claude/rules/provider-info-is-a-field-set.md': 'judged 2026-09-12 — 178 lines, crossed by #1005 adding the suggested-players poll: a field whose guard accepts an EMPTY array, which is the exact inverse of the empty-arrays-are-absent paragraph two above it and is unreadable filed anywhere else. The visible seam (the completeness check vs. the live captures table vs. the rating-is-detail-only guarantee) is false in the direction that matters: the captures ARE the evidence for the field list, and #1005 is the third change to add a field by reading them first. Whoever widens the set again needs the whole file in one read, which is the audience it is written for',
  '.claude/rules/provider-metadata-is-a-filter-not-a-tag.md': 'judged 2026-09-12 — 261 lines, crossed again by #1003 (the tri-state exclude direction, whose whole content is the CONTRAST with the OR in the paragraph above it) and #1005 (the recommendation toggle, gated on the SCREEN rather than on the shelf — the first control here that is, and stated by contrast with the three that are not). Originally crossed by #844 recording that the presentation moved to an overlay and the count badge went with it. The visible seam (§1-3 semantics over draw-pool.js vs §5 presentation over filter-panel.js) is exactly the one that must not be cut: §5\'s entire job is to say that merging the PRESENTATION is not licence to merge the SEMANTICS above it, so it is defined by contrast with §1-3 and means nothing filed away from them — the same shape as active-games-filter-sites.md\'s §643 section. #1001 added the crossed playtime gating to §3 and the not-swapped contrast to §4, both of which are defined by the neighbouring paragraph they correct',
  ...
  '.claude/rules/recommendation-scoring.md': 'recorded 2026-09-22 at 775 lines — grown by #1227\'s §15, a formula (`(score - 1) / 2`) that survived #893 replacing the scale underneath it, which is this file\'s own premise seen at its sharpest: nothing went red, and the one calibration point the two scales SHARE stayed correct, so the ladder read fine exactly where a reader spot-checks it. It also records a measurement from the issue that did NOT reproduce, and why a top-N overlap is the wrong instrument for a change of degree. Same seam and same verdict as before. Previously recorded 2026-09-07 at 565 lines — on the list since 2026-08-15, re-measured 2026-08-19 at 424 after #778 added §12 (plays as a profile input) — 274 when grown by #775, which extended §11 with the taste-reason exclusion and the spike-distribution fixture trap (a z-score over one candidate above a floor of zeros is value-INDEPENDENT, so the obvious fixture ties and the spec goes vacuously green). Same seam and same reason as below. Originally recorded 2026-08-14 at 207 lines, crossed by #772, which added the reason-ranking finding (§11: three of the six reason types were structurally unreachable because the weight constants, not the candidate, decided the ordering) and two fixture-vacuity traps in §1 that were each measured going green against a deliberate break. A seam IS visible: §7–8 are the corpus snapshot\'s cost and TTL over lib/corpus-cache.js + lib/corpus.js, while the rest is scoring and reason semantics over lib/recommend.js. Not split here because #772 put the corpus-size INVARIANCE constraint in §7 and it is what binds §1\'s rescale and §11\'s statistics — profile-relative, never candidate-relative — so filing it away from the scoring half is precisely how the cheaper, broken implementation gets written next time. M-001\'s worklist item',
  '.claude/rules/shared-constants-inventory.md': 'judged 2026-09-12 — SPLIT (#978): shared-constants-across-the-stack.md was 470 lines and `scope: global`, i.e. 18 % of the always-resident rules slice on its own, growing ~30 % every six days because adding a paragraph here is the REQUIRED edit for every new shared file. The seam taken is not the one the old entry recorded (the licensed-copy sections): it is the INVENTORY, which is only ever needed while editing public/js/** or lib/**, against the rule itself, which every session needs and now fits in 130 lines with no entry at all. What is left over budget is a list of fifteen independent entries — the token-budget file excludes criteria.md for exactly that shape — and it must stay one list: its whole value is that a session asking "has this been done before" finds every instance in one place, so splitting it by file set is what lets an instance be missed',
};
