...
const RULE_ALLOW = {
  ...
  '.claude/rules/bgg-collection-import.md': 'judged — four silent traps plus the placement decisions of ONE import path, from the provider parse through the bulk write to the picker\'s two lists. It sat exactly at 150 and #625 pushed it 12 over. The visible seam (provider hops vs. the UI "Smaller things") is a false one: the picker\'s constraints are consequences of the route shape above them (a single list carrying a `present` flag), so a session touching either half needs both open',
