...
const RULE_ALLOW = {
  ...
  '.claude/rules/provider-metadata-is-a-filter-not-a-tag.md': 'judged 2026-08-29 — 164 lines, crossed by #844 recording that the presentation moved to an overlay and the count badge went with it. The visible seam (§1-3 semantics over draw-pool.js vs §5 presentation over filter-panel.js) is exactly the one that must not be cut: §5\'s entire job is to say that merging the PRESENTATION is not licence to merge the SEMANTICS above it, so it is defined by contrast with §1-3 and means nothing filed away from them — the same shape as active-games-filter-sites.md\'s §643 section',
