...
test('every colour token a design declares is measured by one of the checks above', () => {
  ...
  const MEASURED = new Set([
    ...
    '--ink-2', '--box', '--box-ink', '--box-ink-soft', '--vermilion', '--on-vermilion',
  ...
  for (const t of THEMES) {
    ...
    for (const m of block.all.matchAll(/(^|[;{\s])(--[a-z0-9-]+)\s*:/gm)) {
      ...
      if (/^--(radius|text|w|dur|ease|font|rail|dock|space|target|shadow|display|leading|kicker|rule|ring-width|ring-offset)/.test(tok)) continue;
