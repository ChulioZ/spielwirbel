...
/* ---- Found while building the above (#957), not in the issue ----
   ...
   `showResults` applies the round's look itself, deliberately — a results URL is
   shared and cold-loaded, and #940 fixed exactly that screen rendering such a
   visit without it. But its multi-table branch returns to `showTableBuilder`
   BEFORE that line, so the sibling screen for a SPLIT evening never got the fix.
   Until the flip (#1202) the look was the round's design; since then it is the
   round's colour marker, and the same cold-load path has to apply it. */
test('a cold-loaded split screen wears the round\'s own marker', async (t) => {
  const dom = loadApp({ locale: 'de' });
  t.after(() => dom.close());
  dom.set('roundCan', () => false);

  const dressed = { ...round([parent, PLAYED, OPEN]), marker: 6 };
  await dom.call('showTableBuilder', dressed, parent);

  const root = dom.document.documentElement;
  assert.equal(root.style.getPropertyValue('--marker'), designMarkers('klassisch')[6].color);
  assert.equal(root.hasAttribute('data-marked'), true);
});
