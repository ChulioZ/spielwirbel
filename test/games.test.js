...
// --- Provider source + server-side cover download (issue #41) ---
...
test('POST games downloads a cover from an allowlisted host', async () => {
  ...
  try {
    ...
    const res = await addGame(round.id, {
      title: 'Catan',
      imageUrl: 'https://cf.geekdo-images.com/original/catan.png',
    });
    assert.equal(res.status, 201);
    assert.match(res.body.image, /^\/uploads\/[0-9a-f]+\.png$/);
    // File really landed under the isolated uploads dir.
