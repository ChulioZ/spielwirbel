 ...
 * /kontakt.html page. No network ever — with MJ_APIKEY_PRIVATE unset lib/mail.js
 * captures messages in its in-memory outbox (the Mailjet path, when exercised, is
...
afterEach(() => {
  ...
  for (const k of ['CONTACT_TO', 'MJ_APIKEY_PRIVATE', 'MJ_APIKEY_PUBLIC', 'MAIL_FROM', 'NODE_ENV',
...
test('feedback works in production with mail unconfigured — never the 502 guard (#321)', async () => {
  ...
  // No MJ_APIKEY_PRIVATE / MAIL_FROM → mail.isConfigured() is false, which 502s a
...
test('in production with mail unconfigured it fails loud (502) instead of black-holing', async () => {
  process.env.NODE_ENV = 'production';
  process.env.CONTACT_TO = 'ops@example.com';
  // No MJ_APIKEY_PRIVATE / MAIL_FROM → mail.isConfigured() is false.
  const before = outbox.length;
  const res = await request(app).post('/api/contact').send(valid);
...
test('a send failure returns 502 with the fallback email — but keeps the stored notice', async () => {
  process.env.MJ_APIKEY_PRIVATE = 'test-key';
  process.env.MJ_APIKEY_PUBLIC = 'proj-123';
  process.env.MAIL_FROM = 'no-reply@example.com';
  process.env.CONTACT_TO = 'ops@example.com';
  global.fetch = async () => ({ ok: false, status: 500 }); // Mailjet error → mail.send rejects
  const countBefore = await repo.countContactNotices();
